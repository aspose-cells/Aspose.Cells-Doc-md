...
| :-             | :-              |
| Parameters     | The Parameters collection contains the report parameters within the report. Parameters can be passed to queries, used in filters, or other functions that alter the report appearance based on the parameters. |
...
| Comparison        | <                | Less than, for example 4 < 3, which is false. |
|                   | <=               | Less than or equal, for example 4 <= 3, which is false. |
|                   | >                | Greater than, for example 4 > 3, which is true. |
|                   | >=               | Greater than or equal, for example 4 >= 3, which is true. |
|                   | =                | Equal, for example 4 = 3, which is false. |
|                   | <>               | Not equal, for example 4 <> 3, which is true. |
...
|                   | Xor              | Performs a logical exclusive‑or on two Boolean expressions, or a bitwise exclusive‑or on two numeric expressions. |
...
| Bit Shift         | >>               | Performs an arithmetic right shift on a bit pattern. |
