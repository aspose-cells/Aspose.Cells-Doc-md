...
The DeviceInfo section in **rsreportserver.config** takes the following parameters that affect Aspose.Cells' behavior:
...
- **SimplePageFooters**: when the value is **true**, the report footer item is rendered as a Microsoft Excel page footer. The default value is **true**.
...
- **PutoutFooter**: when **true** (default), the report footer item is exported. When **false**, it is not exported. Only supports the Excel 2007 XLSX (data only) extension.
