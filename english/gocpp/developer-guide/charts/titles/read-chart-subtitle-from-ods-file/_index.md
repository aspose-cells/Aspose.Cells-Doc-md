---  
title: Read Chart Subtitle from ODS File with Golang via C++  
linktitle: Read Chart Subtitle from ODS File  
description: Learn how to use Aspose.Cells for C++ to read the chart subtitle from an OpenDocument Spreadsheet (ODS) file. Our guide will demonstrate how to extract and access the subtitle of a chart for further analysis or display.  
keywords: Aspose.Cells for C++, Read Chart Subtitle, OpenDocument Spreadsheet, ODS File, Chart Extraction, Data Analysis.  
type: docs  
weight: 160  
url: /go-cpp/read-chart-subtitle-from-ods-file/  
---  

## **Read Chart Subtitle from ODS File**  

Aspose.Cells provides the ability to read chart subtitles in ODS files by using the [**Chart.SubTitle**](https://reference.aspose.com/cells/go-cpp/chart/getsubtitle/) property. The following sample code loads the [sample ODS file](89620481.ods) and reads the chart subtitle using the [**Chart.SubTitle**](https://reference.aspose.com/cells/go-cpp/chart/getsubtitle/) property and prints it in the console window. Please see the console output of the code given below for reference.  

## **Sample Code**  

{{< gist "aspose-cells-gists" "b414abd53259bbc47d2c3c0fe985395b" "Examples-Go-CPP-ReadChartSubtitleFromOdsFile.go" >}}  

## **Console Output**  

{{< highlight java >}}  

Chart Subtitle: Sample Chart Subtitle  

{{< /highlight >}}