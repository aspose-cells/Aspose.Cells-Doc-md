...
description: How to insert hyperlinks into an Excel file with the Aspose.Cells library without MS Excel using C++.
...
A hyperlink is used to create a link between two entities. Everyone is familiar with the use of hyperlinks, especially on websites.  
...
In the above example, a hyperlink is added to a URL in an empty cell, **A1**. In such cases, if a cell is empty, the URL address is also added to that empty cell as its value. If the cell is not empty and a hyperlink is added, the value of the cell appears as plain text. To make it look like a hyperlink, apply the appropriate formatting settings to that cell.
