---
title: Add Image Hyperlinks with Golang via C++
linktitle: Add Image Hyperlinks
type: docs
weight: 140
url: /go-cpp/add-image-hyperlinks/
description: Learn how to add image hyperlinks using the Aspose.Cells for C++ API.
keywords: Add Image Hyperlinks, Insert Image Hyperlinks
---

{{% alert color="primary" %}} 

Hyperlinks are useful for accessing information on other worksheets or on websites. Microsoft Excel lets users add hyperlinks to text in cells and to images. Image hyperlinks can make navigating a worksheet easier, for example, as next and previous buttons, or as logos that link to particular sites. This document explains how to insert image hyperlinks in a worksheet using Aspose.Cells.

{{% /alert %}} 

Aspose.Cells allows you to add hyperlinks to images in spreadsheets at runtime. It is possible to set and modify the link's screen tip and address. The following sample code illustrates how to add an image hyperlink into a worksheet.

{{< gist "aspose-cells-gists" "b414abd53259bbc47d2c3c0fe985395b" "Examples-Go-CPP-AddImageHyperlinks.go" >}}