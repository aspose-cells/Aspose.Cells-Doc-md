...
The [**SheetRender**](https://reference.aspose.com/cells/go-cpp/sheetrender/) class represents a worksheet to render as images. It has an overloaded method, [**ToImage**](https://reference.aspose.com/cells/go-cpp/sheetrender/toimage/), that can convert a worksheet to image file(s) with different attributes or options. It returns a `System.Drawing.Bitmap` object and you can save the image file to disk or stream. Several image formats are supported, for example BMP, PNG, GIF, JPG, JPEG, TIFF, EMF.
