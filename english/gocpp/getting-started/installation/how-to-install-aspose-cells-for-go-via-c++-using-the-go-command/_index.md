...
From Go 1.16, you can use the `go install` command directly to install the Aspose.Cells for Go via C++ package. The command allows for the global installation of command‑line tools without affecting existing project dependencies.

```go
go install github.com/aspose-cells/aspose-cells-go-cpp/v25@latest
```

When you use the `go get` command to install the Aspose.Cells for Go via C++ package, a `go.mod` file must exist in the current directory or any parent directory. See [Installation of Aspose.Cells for Go via C++ package and running your code in your project](#Installation-in-your-project).
...
1. Create a directory for your project and a `main.go` file within it. Add the following code to `main.go`.
...
4. Set your `PATH` to point to the shared libraries for Aspose.Cells for Go via C++ in your current command shell. Replace `your_version` with the version of Aspose.Cells for Go via C++ you are running.
