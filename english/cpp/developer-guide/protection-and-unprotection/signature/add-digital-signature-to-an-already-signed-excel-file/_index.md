...
Please note that while adding a digital signature to an already signed Excel document, if the original document was generated by Aspose.Cells, it works properly. However, if the document was created by other engines (e.g., Microsoft Excel), Aspose.Cells for C++ cannot preserve the exact file structure after loading and re‑saving, which may invalidate existing signatures.
...
int main()
{
    ...
    U16String comments = u"Aspose.Cells added a new digital signature to an existing digitally signed workbook.";
