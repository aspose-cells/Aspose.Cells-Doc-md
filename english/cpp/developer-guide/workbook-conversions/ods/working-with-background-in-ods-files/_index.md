...
Background can be added to sheets in ODS files. The background can either be a colored background or a graphic background. The background is not visible when the file is opened, but if the file is printed as PDF, the background appears in the generated PDF. The background is also visible in the print preview dialogue.
...
Aspose.Cells provides the [**OdsPageBackground**](https://reference.aspose.com/cells/cpp/aspose.cells.ods/odspagebackground/) class to manage background in ODS files. The following code sample demonstrates the use of [**OdsPageBackground.GetGraphicData()**](https://reference.aspose.com/cells/cpp/aspose.cells.ods/odspagebackground/getgraphicdata/) property to add a graphic background to the ODS file. Please see the [output ODS](90112030.ods) file generated by the code for reference.
...
Vector<uint8_t> GetDataFromFile(const U16String& file)
{
    ...
    uint8_t* buffer = new uint8_t[static_cast<size_t>(fileSize)];
    fileStream.read(reinterpret_cast<char*>(buffer), fileSize);
    fileStream.close();

    Vector<uint8_t> data(buffer, static_cast<size_t>(fileSize));
    delete[] buffer;
