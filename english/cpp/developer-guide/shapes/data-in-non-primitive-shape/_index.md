...
Sometimes you need to access data from a shape that is not built‑in. Built‑in shapes are called primitive shapes; shapes that aren’t are called non‑primitive. For example, you can define your own shapes using different curved, connected lines.
...
int main()
{
    ...
    if (shape.GetAutoShapeType() == AutoShapeType::NotPrimitive)
    {
        ...
        // Access information of each individual path
        for (int i = 0; i < shapePathCollection.GetCount(); ++i)
        {
            ShapePath shapePath = shapePathCollection.Get(i);

            // Access path segment list
            ShapeSegmentPathCollection pathSegments = shapePath.GetPathSegmentList();
