---  
title: Reading and Writing Query Tables of a Worksheet with C++  
linktitle: Reading and Writing Query Tables of a Worksheet  
type: docs  
weight: 40  
...
## Reading and Writing Query Tables of a Worksheet  

The following sample code reads the first `QueryTable` of the first worksheet and then prints both of the `QueryTable` properties. It then sets the `QueryTable.PreserveFormatting` to `true`.  
...
int main()
{
    ...
    std::cout << convert_u16_to_string(
        wb.GetWorksheets().Get(0).GetQueryTables().Get(0).GetResultRange().GetAddress().GetData()
    ) << std::endl;
