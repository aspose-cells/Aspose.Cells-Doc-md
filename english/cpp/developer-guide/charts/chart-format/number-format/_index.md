...
You can set the values format code of chart series using the [Series.GetValuesFormatCode()](https://reference.aspose.com/cells/cpp/aspose.cells.charts/series/getvaluesformatcode/) property. This property is not only useful for series that are based on a range inside the worksheet but also works well for series created with an array of values.

## **Set the Values Format Code of Chart Series**
The following sample code adds a series to an empty chart that initially has no series. It adds the series using an array of values. After adding the series, it formats it with the code `$#,##0` using the [Series.GetValuesFormatCode()](https://reference.aspose.com/cells/cpp/aspose.cells.charts/series/getvaluesformatcode/) property, and the number `10000` becomes `$10,000`. The screenshot shows the effect of the code on the [sample Excel file](51740712.xlsx) and the [output Excel file](51740713.xlsx) after execution.
