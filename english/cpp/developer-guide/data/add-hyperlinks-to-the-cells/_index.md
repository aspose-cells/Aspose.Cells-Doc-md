---
title: Add Hyperlinks to the Cells
type: docs
weight: 30
url: /cpp/add-hyperlinks-to-the-cells/
ai_search_scope: cells_cpp
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

## **Add Hyperlinks to the Cells**
The following sample code shows how to add a hyperlink inside a cell of the worksheet. You can also add a hyperlink in a range of cells in a similar way. Please check the [output Excel file](23167002.xlsx) generated with this code and the following screenshot showing the output Excel file in Microsoft Excel.

![todo:image_alt_text](add-hyperlinks-to-the-cells_1.png)

## **Sample Code**
{{< gist "aspose-cells-gists" "6f7d9819d85793c3a3b5d040af42e1a9" "Examples-CellsCPP-Data-AddHyperlinksToCells-new.cpp" >}}
{{< app/cells/assistant language="cpp" >}}
