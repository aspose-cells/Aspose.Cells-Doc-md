...
Aspose.Cells.GridDesktop has a context menu which contains all the commonly used commands. The control allows you to hide/show menu items. Moreover, it is possible to add new menu items with event handlers to the menu.

{{% /alert %}} 
## **Introduction**
The ContextMenuManager class is used to manage the context menu items. The GridDesktop.ContextMenuManager attribute gets an instance of the ContextMenuManager object. For example, the ContextMenuManager.MenuItemAvailable_Copy attribute gets or sets a value indicating whether the context menu item **Copy** is available or not. Similarly, we have all the corresponding attributes for different context menu items.
...
To perform this task, we first take a look at the default context menu the GridDesktop has.

**GridDesktop's default menu** 
...
After executing the above code, a new menu item can be seen in the context menu. A message will also appear when a cell is clicked.
