...
This list doesn't include those events that are inherited by Aspose.Cells.GridDesktop from the Control class.

{{% /alert %}} 

|**Events**|**Description**|
| :- | :- |
|BeforeCalculate|Occurs before calculating a formula in the workbook.|
|BeforeLoadFile|Occurs before a workbook is loaded from a file.|
...
|CellCheckedChanged|Occurs when the Checked property of a cell's checkbox is changed.|
|CellSelectedIndexChanged|Occurs when the SelectedIndex property of a cell combo‑box is changed.|
|CellClick|Occurs when a Grid cell is clicked.|
|CellDoubleClick|Occurs when a Grid cell is double‑clicked.|
|CellKeyPressed|Occurs when a key is pressed while a cell has focus. If you want to create an event handler for the CellKeyPressed event, set the Handled property of the CellKeyEventArgs argument to **true** to prevent the GridDesktop control from handling the key event.|
...
|FailLoadFile|Occurs when the workbook fails to load.|
|FinishCalculate|Occurs after calculating a formula in the workbook.|
...
|RowColumnHiddenChanged|Occurs when the hidden status of a row or column is changed.|
...
2. When the event is double‑clicked, an event handler is created by Visual Studio.NET. The following is designer‑generated code that shows an event handler for the GridControl control.
...
Have a look at the event handler that Visual Studio has added to the GridDesktop control's CellClick event. It will look something like the code below.
