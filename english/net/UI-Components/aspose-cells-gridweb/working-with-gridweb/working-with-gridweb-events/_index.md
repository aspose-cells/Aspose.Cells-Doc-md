...
description: This article introduces how to work with different kinds of events in GridWeb.
...
|CellError |Occurs when a cell's input value has an error. |
...
|ColumnInserted |Occurs when a user inserts a column into a worksheet using client‑side menu. |
...
|RowInserted |Occurs when a user inserts a row into a worksheet using client‑side menu. |
...
|AjaxCallFinished |Fires when the AJAX update of the control has finished (the EnableAJAX property shall be set to true). |
|CellModifiedOnAjax |Fires when the cell is modified in an AJAX call. |
...
To perform a specific operation when a specific event is triggered, we have to create an event handler. An event handler performs the desired task when a certain event is triggered. The steps illustrated below show how to handle a simple grid event using Visual Studio.
...
2. Add code to perform some action inside the event handler.  

   Here, a single line of code that saves the grid content to an Excel file when the **Save** button is clicked has been added.  

   To get more information, move the cursor over the code; you will find that Visual Studio is intelligent enough to add an event handler to the GridWeb's SaveCommand event.  
