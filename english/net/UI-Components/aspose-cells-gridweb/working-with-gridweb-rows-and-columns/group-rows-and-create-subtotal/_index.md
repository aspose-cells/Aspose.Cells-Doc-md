...
3. Select the desired cells in the rows.  
4. Group the rows.  
...
The following example groups the rows from the 2nd row to the 10th row.
...
You can create levels of organization while grouping a set of rows. You can group rows within already grouped rows. The following example shows nesting grouped rows.
...
1. Select the rows in the worksheet to ungroup.  
2. Ungroup the rows.  

The following example ungroups the rows from the 2nd row to the 10th row.
...
The following example generates subtotals that calculate the non‑numeric values grouped by the second column in the worksheet.
