...
It is possible to restrict any kind of client‑side operations on rows and columns by restricting the options available in the context menu. This can be done by setting the `EnableClientColumnOperations` and `EnableClientRowOperations` properties of the GridWeb control to **false**. It is also possible to restrict users from freezing rows and columns by setting the GridWeb control's `EnableClientFreeze` property to **false**.
