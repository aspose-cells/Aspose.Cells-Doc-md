...
Now, it's time to add some code to the Page_Load event of the Web Form. You can double‑click on the Web Form in design view and VS.NET IDE will automatically take you to the Page_Load event handler where you would need to use the Attributes collection of the Button for overriding its OnClick event.

{{% alert color="primary" %}} 

ASP.NET Button control is a server‑side control. Whenever it is clicked, a server‑side event is triggered, but if you want to use this Button control to execute some code on the client side (normally using JavaScript) then you need to modify its OnClick attribute under the Page_Load event. Aspose.Cells.GridWeb provides some functions that can be invoked in JavaScript to deal with the GridWeb control from the client side. In the example below, we have used the **updateData** and **validateAll** functions (which are client‑side functions) of GridWeb to update and validate Grid data.
...
Now, you can compile and run your application by pressing Ctrl+F5 and the page will open in a new browser window. Let's add some values to GridWeb and press the **Submit Grid Data to Server** button; you will see a postback occurring to update and validate GridWeb data.
## **Conclusion**
{{% alert color="primary" %}} 

Aspose.Cells.GridWeb offers great flexibility for working with GridWeb controls from either the server‑side or the client‑side. Developers have a wide range of options to use the GridWeb control in different kinds of scenarios to provide better solutions to their customers.
