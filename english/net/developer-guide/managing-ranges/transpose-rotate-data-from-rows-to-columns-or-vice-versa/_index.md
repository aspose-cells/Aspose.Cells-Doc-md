...
url: /net/transpose-rotate-data-from-rows-to-columns-or-vice-versa/
...
The article then presents three different ways to transpose this data using Aspose.Cells for .NET, each suited to a different Excel version and use case.
...
After transposition this anchor range holds the rotated data. The first row reads (empty, **Europe**, **Asia**, **North America**) and the first column reads (empty, **Qtr 1**, **Qtr 2**, **Qtr 3**, **Qtr 4**). Each original column of sales becomes a row in the transposed range.
...
3. Place the dynamic array formula on cell **A6**, just below the source range, by calling `cells["A6"].SetDynamicArrayFormula("=TRANSPOSE(A1:D5)", new FormulaParseOptions(), true)`.
4. The `new FormulaParseOptions()` argument uses default `FormulaParseOptions` settings, and the third argument `true` tells Aspose.Cells to treat the formula as a dynamic array and to evaluate it so the spilled values are written to the workbook.
5. Save the workbook with `workbook.Save(outputFile)`.
Cell **A6** holds the formula `=TRANSPOSE(A1:D5)` and Excel spills the result automatically into the region **A6:E9**, a 4-row by 5-column block equal to the transposed data.
...
2. Retrieve the first worksheet and access its `Cells` collection.
3. Call `cells["A6"].SetArrayFormula("=TRANSPOSE(A1:D5)", 4, 5)`. The second argument `4` is the number of rows of the destination array and the third argument `5` is the number of columns.
