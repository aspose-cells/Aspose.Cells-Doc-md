...
The following sample code embeds the formula in the smart marker field named **TestFormula**, and its data source name is **MyDataSource**, so the complete field with the formula parameter looks like `&=MyDataSource.TestFormula(formula)`. After execution of the code, the [final output Excel file](46465047.xlsx) will have formulas in cells from **A1** to **A5**.
