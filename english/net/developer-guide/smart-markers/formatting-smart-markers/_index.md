...
### **Introduction**
While working with the Smart Markers grouping data feature, sometimes you need to add your own custom labels to the summary row. You also want to concatenate the column's name with that label, e.g., “Sub Total of Orders”. Aspose.Cells provides the Label and LabelPosition attributes, so you may place your custom labels in the Smart Markers while concatenating with the subtotal rows in grouping data.
