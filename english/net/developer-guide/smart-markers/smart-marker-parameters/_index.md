...
If you have multiple parameters, separate them with commas, but no spaces: `parameterA,parameterB,parameterC`
...
In the dynamic formula marker, “-1” denotes the offset to the current row in the B and C columns respectively, which will be used for the division operation; the **skip** parameter is one row. Moreover, we should specify the following character as a separator to apply further parameters in dynamic formulas:
...
It’s very easy to process the smart markers. The following example code shows how to use dynamic formulas in Smart Markers. We load the [template file](templateDynamicFormulas.xlsx), create test data, and process the markers to fill data into the cells.
...
Sometimes you need to use dynamic formulas and variables in SmartMarkers. For dynamic formulas, please add the special character **~** as a separator. Aspose.Cells makes it possible to use dynamic formulas and variables in SmartMarkers. Please check the [template file](template.xlsx), the [JSON file](employees-data.json) and the screenshot of the output Excel file generated with the following code.
