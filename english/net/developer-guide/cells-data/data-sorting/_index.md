...
Excel provides a feature to sort data based on the background color. The same feature is provided using Aspose.Cells, where the DataSorter’s [**SortOnType**](https://reference.aspose.com/cells/net/aspose.cells/sortontype).CellColor can be used in [**AddKey()**](https://reference.aspose.com/cells/net/aspose.cells/datasorter/methods/addkey) to sort data based on the background color. All the cells that contain the specified color in the **AddKey()** function are placed on the top or bottom according to the **SortOrder** setting, and the order of the rest of the cells is not changed at all.
