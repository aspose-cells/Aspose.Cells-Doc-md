...
Aspose.Cells for .NET exposes two equivalent ways to register a filter field. The high-level API is `PivotTable.AddFieldToArea(PivotFieldType.Page, "fieldName")`, which takes the source-column name and adds the field in a single call. The lower-level API is `PivotTable.PageFields.Add(PivotField)`, which is used when you already hold a `PivotField` reference and want to add the same field instance to the filter area. Both APIs end up populating the same `PageFields` collection, and the remainder of this article demonstrates how to choose between them and how to drive each filtering mode.
