...
description: Aspose.Cells is a .NET library for working with spreadsheet files that supports many different cell number settings. This article will introduce how to use the Aspose.Cells library to manage the number settings of cells so that users can adjust the number format in the spreadsheet as needed.  
...
Aspose.Cells offers some built-in number formats to configure the display formats of numbers and dates. These built-in number formats can be applied by using the **Number** property of the **Style** object. All built-in number formats are given unique numeric values. Developers can assign any desired numeric value to the **Number** property of the **Style** object to apply the display format. This approach is fast. The built-in number formats supported by Aspose.Cells are listed below.
...
If you use the **Custom** property to set the number format, any previous format set using the **Number** property is overridden and vice versa.

{{% /alert %}}

## **Advanced topics**
- [Check Custom Number Format when Setting Style.Custom Property](/cells/net/check-custom-number-format-when-setting-style-custom-property/)
- [List of Supported Number Formats](/cells/net/list-of-supported-number-formats/)
