...
- **Index** – the index of the color in the palette that the custom color will replace (must be between 0 and 55).
...
{{% alert color="primary" %}}
The palette holds only 56 colors. When you add a custom color to the palette, the palette is changed and any element in the file formatted with the previous color is also changed. Therefore, you should be very careful when modifying the palette. Moreover, this limitation applies only to XLS (Excel 97‑2003) files; there is no such limitation for XLSX or later Excel file formats (2007, 2010, 2013, etc.).
{{% /alert %}}
