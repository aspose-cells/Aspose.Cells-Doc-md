...
Aspose.Cells also allows you to set the **Formula Calculation Mode** using the `FormulaSettings.CalculationMode` property. You can assign it a value from the `CalcModeType` enumeration, which has one of the following values:

- `CalcModeType.Automatic`
- `CalcModeType.AutomaticExceptTable`
- `CalcModeType.Manual`

The following sample code first creates a workbook, then sets the formula calculation mode to **Manual**, and saves the workbook as an output Excel file on disk.
