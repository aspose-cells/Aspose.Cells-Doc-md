...
When **UAC** mode is enabled, every program that you run will be given only "standard user" access to the system, even when you are logged in as an administrator. **Windows Vista** has the built-in ability to automatically reduce the potential of security **breaches** in the system. It does that by automatically enabling this feature called **User Account Control** (or **UAC** for short). The **UAC** forces users that are part of the local administrators group to run as if they were regular users with no administrative privileges. Although **UAC** clearly improves the security on **Windows Vista**, under some scenarios you might want to disable it, for example when giving demos in front of an audience (demos that are not security‑related, for example). Some home users might be tempted to disable **UAC** because of the additional resource usage of their system.
## **Steps Involved for Successful Installation of the Component**
- Please make sure that IIS is installed on your Vista before installing **Aspose.Cells**. It is compulsory because the **Aspose.Cells** installer needs to create a virtual folder on IIS for the deployment of the Web Demos (ASP.NET Demos).
- You need to disable **UAC** (User Account Control). You have to make sure that you have **administrative privileges** with full control of the system before installing **Aspose.Cells**. Otherwise you might get an error #2869 while installing **Aspose.Cells** using its installer.

Following are some ways to achieve this.
### **Using Command Line**
1. Search for `cmd.exe` in your Windows directory, then right‑click on it and select **Run as Administrator**.
2. Now, run the following command in the command prompt: `msiexec /i <your path>/Aspose.Cells.msi` and press **Enter**.
### **Using Control Panel**
- Click **Start**
- Click **Control Panel**
