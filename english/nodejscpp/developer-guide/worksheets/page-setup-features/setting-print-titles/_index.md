---  
title: How to Set Print Titles with Node.js via C++  
...
To set print titles in a specified worksheet, first load the [sample file](input.xlsx). Then, use `Worksheet.getPageSetup()` to obtain the **PageSetup** object and set its properties. Setting these properties to a range string will define the print titles.  
...
To clear the print titles in a specified worksheet, first load the [sample file](input.xlsx). Then, use `Worksheet.getPageSetup()` to obtain the **PageSetup** object and set its properties to empty strings.  
