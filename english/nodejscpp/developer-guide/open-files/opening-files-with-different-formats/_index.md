...
It is not always necessary to use the default parser settings for opening the CSV files. Sometimes importing a CSV file does not create the expected output, like date format not appearing as expected or empty fields handled differently. For this purpose, **TxtLoadOptions.PreferredParsers** is available to provide a preferred parser to parse different data types as per the requirement. Following sample code demonstrates the usage of a preferred parser. 
...
oTxtLoadOptions.setHasFormula(true);

// Initialize preferred parsers
const preferredParsers = [
    ...
    {
        ParseObject: (value) => new Date(value),
        GetFormat: () => "dd/MM/yyyy"
    }
];
oTxtLoadOptions.setPreferredParsers(preferredParsers);
...
oExcelWorkBook.save(path.join(outputDir, "outputsamplePreferredParser.xlsx"));
