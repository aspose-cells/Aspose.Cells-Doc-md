---
title: Using Sparklines and Setting 3D Format with Node.js via C++
...
## **Using Sparklines**
Microsoft Excel 2010 can analyze information in more ways than ever before. It allows users to track and highlight important data trends with new data‑analysis and visualization tools. Sparklines are mini‑charts that you can place inside cells so that you can view data and charts on the same table. When sparklines are used properly, data analysis is quicker and more to the point. They also provide a simple view of information, avoiding overcrowded worksheets with a lot of busy charts.
...
Sparklines help you see trends, for example, the win‑or‑loss record for a softball league. Sparklines can even sum up the entire season of each team in the league.
...
// Read the Sparklines from the template file (if it has any)
...
for (let i = 0; i < sparklinesCount; i++) {
    ...
    const sparklineCount = group.getSparklines().getCount();
    for (let j = 0; j < sparklineCount; j++) {
        const sparkline = group.getSparklines().get(j);
        // Display the individual Sparklines and the data ranges
...
## **Setting 3D Format**
You might need 3D charting styles so you can obtain the exact results for your scenario. Aspose.Cells for Node.js via C++ does provide the relevant API to apply Microsoft Excel 2007 3D formatting.
