---  
title: Setting Dynamic Array Formulas with Node.js via C++  
linktitle: Setting Dynamic Array Formulas  
description: How to use the Aspose.Cells library to calculate dynamic array formulas in Excel using Node.js via C++. Load, calculate, and save Excel files easily.  
keywords: Dynamic Array Formulas, Dynamic Array Formulas in Excel, Set dynamic array formulas Node.js via C++, Calculation of dynamic array formulas Node.js via C++, operate dynamic array formulas in Excel.  
type: docs  
weight: 70  
url: /nodejs-cpp/calculation-of-dynamic-array-formulas/  
...
4. **Benefits:**  
   <br>  
   Array formulas allow you to perform complex calculations in a single formula, which can improve efficiency and simplify your worksheets. They can handle large data sets and perform calculations that would otherwise require multiple intermediate steps.  

5. **Limitations:**  
   <br>  
   Array formulas can be more difficult to understand and troubleshoot than regular formulas. They can slow down worksheet performance, especially if used extensively or with large data sets.  
...
## **What Is an Excel Dynamic Array Formula**  
...
5. **Improved Performance:**  
   <br>  
   Dynamic array formulas can improve performance compared to traditional array formulas, especially when working with large data sets or complex calculations.  

6. **Compatibility:**  
   <br>  
   Dynamic array formulas are available in Excel 365 and Excel 2021. They may not be supported in older versions of Excel.  

7. **Examples of Dynamic Array Formulas:**  
   <br>  
...
In Excel, both array formulas and dynamic array formulas are used to perform calculations on multiple values simultaneously, but they differ in functionality and implementation.

### **Array Formula Features**  
...
3. Array formulas have limitations in their ability to spill results into adjacent cells. They typically return a single result, although that result might be an array.  
4. They have been around for a long time and are supported in all versions of Excel.  

### **Dynamic Array Formula Features**  
...
4. These formulas can return multiple results (a range of cells) directly without the need for an explicit array formula.  
5. They introduce functions like **FILTER**, **SORT**, **UNIQUE**, etc., which handle arrays natively and return results in a dynamic array format.  

In summary, dynamic array formulas are a more modern and convenient way to work with arrays in Excel, providing automatic spilling of results and simplifying the process compared to traditional array formulas. However, they are only available in the newer versions of Excel that support dynamic arrays.
...
5. **Interact with the Spilled Range:**  
   <br>  
...
6. **Update the Formula:**  
   <br>  
   If you need to modify the dynamic array formula, simply edit the formula in the original cell where it was entered. After editing, press **Enter** again to confirm the changes. Excel will automatically update the spilled range if necessary.  

7. **Clear the Spilled Range:**  
   <br>  
...
Please see the following sample code that loads the [sample Excel file](dynamicArrayFormula.xlsx) containing some dummy data. After loading the file, call the [Cell.setDynamicArrayFormula(string, FormulaParseOptions, boolean)](https://reference.aspose.com/cells/nodejs-cpp/cell/#setDynamicArrayFormula-string-formulaparseoptions-boolean-) method to set a dynamic array formula and the [Workbook.refreshDynamicArrayFormulas(boolean)](https://reference.aspose.com/cells/nodejs-cpp/workbook/#refreshDynamicArrayFormulas-boolean-) method to refresh dynamic array formulas before calculating the workbook. Finally, save the workbook as the [output Excel file](out.xlsx).
...
f16.setDynamicArrayFormula(
    ...
    new AsposeCells.FormulaParseOptions(),
    false
);
