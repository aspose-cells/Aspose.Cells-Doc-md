...
description: Learn how to manage formulas of Excel files through Aspose.Cells for Node.js via C++. Aspose.Cells can simply get, set, and calculate formulas of Excel files.
...
Built-in functions or formulas are provided as ready‑made utilities to reduce developers' effort and time. See [a list of built-in functions](/cells/nodejs-cpp/supported-formula-functions/) supported by Aspose.Cells. The functions are listed in alphabetical order. More functions will be supported in the future.
...
Use the [**Cell**](https://reference.aspose.com/cells/nodejs-cpp/cell) class's **setFormula()** method to add a formula to a cell. **Complex formulas**, for example
...
Aspose.Cells provides features to register add‑in functions using [**Worksheets.registerAddInFunction(string, string, boolean)**](https://reference.aspose.com/cells/nodejs-cpp/worksheetcollection/#registerAddInFunction-string-string-boolean-). Afterwards, when we set `cell.setFormula("=YourAddInFunction()")`, the output Excel file contains the calculated value from the add‑in function.
...
It is possible to apply an array formula to a cell by calling the [**Cell**](https://reference.aspose.com/cells/nodejs-cpp/cell) class's **setArrayFormula(string, number, number)** method. The method takes the following parameters:
...
Add an **R1C1** reference‑style formula to a cell with the [**Cell**](https://reference.aspose.com/cells/nodejs-cpp/cell) class's **setR1C1Formula()** method.
