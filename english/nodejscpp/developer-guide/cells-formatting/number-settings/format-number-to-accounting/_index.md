---
title: How to Format Numbers to Accounting
type: docs
weight: 10
url: /nodejs-cpp/how-to-format-number-to-accounting/
description: This article will introduce how to format numbers to accounting using Aspose.Cells for Node.js via C++ API.
...
4. **Zero Values Representation**: In accounting format, zero values are often represented as a dash (‑) instead of a numeral zero (0). This practice can help distinguish between zero values and cells that are simply empty or have not been filled in, adding to the clarity of the data presented.
...
2. **Go to the Home Tab**: On the **Home** tab in the Ribbon, locate the **Number** group.  
3. **Select Accounting Format**: Click the dropdown and choose **Accounting Number Format**. This automatically applies the default accounting format to your selected cells.
...
## **How to Format Numbers to Accounting in Aspose.Cells for Node.js via C++**
To format numbers to accounting format in Aspose.Cells for Node.js via C++, you can use the `Style` object associated with a cell or range of cells. The `Style` object allows you to set various formatting options, including number formats. The accounting format typically has a format code that can vary depending on the specific requirements (e.g., whether to show currency symbols, decimal places, etc.).
