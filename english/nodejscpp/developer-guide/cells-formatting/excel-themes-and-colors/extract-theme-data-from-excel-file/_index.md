---  
title: Extract Theme Data from Excel File  
linktitle: Extract Theme Data from Excel File  
description: Learn how to extract theme data from Excel files using Aspose.Cells for Node.js via C++. Obtain style and formatting information effectively.  
keywords: Aspose.Cells, Excel File, Theme Data, Style, Format, Node.js via C++  
type: docs  
weight: 120  
url: /nodejs-cpp/extract-theme-data-from-excel-file/  
ai_search_scope: cells_nodejscpp  
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"  
---  
  
{{% alert color="primary" %}}  
  
Aspose.Cells allows users to extract theme‑related data from an Excel file. For example, you can extract the theme name applied to a workbook and the theme color applied to a cell or the borders of the cell, etc.  
  
You can apply a theme to your workbook using Microsoft Excel via the **Page Layout > Themes** command.  
  
{{% /alert %}}  
  
## JavaScript code to extract theme data from Excel file  
  
The following sample code extracts the theme name applied to the source workbook and then extracts the theme color applied to cell A1 and the theme color applied to the bottom border of the cell.  
  
{{< gist "aspose-cells-gists" "c7b55cbeb75eaaae989115230a7619eb" "Cells-Formatting-ThemesAndColors-ExtractThemeData.js" >}}  
  
{{< app/cells/assistant language="nodejs-cpp" >}}
