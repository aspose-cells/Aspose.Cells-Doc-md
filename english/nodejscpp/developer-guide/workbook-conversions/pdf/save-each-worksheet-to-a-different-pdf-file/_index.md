...
{{% alert color="primary" %}}  
Aspose.Cells supports converting XLS files (that contain images, charts, etc.) to PDF documents. Aspose.Cells for Node.js via C++ can work independently to convert a spreadsheet to PDF, and you do not need to use Aspose.PDF for Node.js via C++ for the conversion. The conversion does not require any temporary files, as the whole process can be done in memory.  
{{% /alert %}}  

## **Save Each Worksheet to a Different PDF File**  
If you need to save each worksheet in your template Excel file to generate different PDF files, you can achieve this easily. You may try setting one sheet index to the [**PdfSaveOptions.SheetSet**](https://reference.aspose.com/cells/nodejs-cpp/pdfs_saveoptions/#sheetSet) option at a time to render to PDF.  
