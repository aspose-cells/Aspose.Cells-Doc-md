...
range.get(3, 1).setValue("Philippines");
...
Call the **worksheets** collection's **getRangeByName(string)** method to get a range by the specified name. This method takes the name of the named range and returns the specified named range as an instance of the **Range** class. The following example shows how to access a specified range by its name.  
...
Call the **worksheets** collection's **getNamedRanges()** method to get all named ranges in a spreadsheet. The **getNamedRanges()** method returns an array of all named ranges in the **worksheets** collection.  
...
Aspose.Cells provides the **range.copy(Range, PasteOptions)** method to copy a range of cells with formatting into another range.  
...
// Copy the first range into the second range.
range2.copy(range1);
