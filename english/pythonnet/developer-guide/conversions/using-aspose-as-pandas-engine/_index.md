...
keywords: Using Aspise.Cells for Python via .NET as a Pandas Excel Engine.
...
In `pandas/io/excel/_base.py`, find the `class ExcelFile` class, and add the following import line after the existing `from pandas.io.excel._xlrd import XlrdReader`:

```python
from pandas.io.excel._asposecells import AsposeCellsExcelReader
```

Then add the following code in `_engines: Mapping[str, Any]`:
