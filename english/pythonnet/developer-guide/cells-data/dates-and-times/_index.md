...
description: Learn how to manage Dates and Times through the Aspose.Cells for Python via .NET API.
...
This example shows that the serial values stored for the same date in different date systems are different.  
...
This example sets a `DateTime` value in cells A1 and A2, applies a custom format to A1 and a number format to A2, checks the value types of the two cells, and then outputs the `DateTime` value and formatted string.
