...
The following code demonstrates the usage of this property using the [sample Excel file](5115030.xlsm), which you can download from the provided link. The same Excel file has a valid signature, but when we modify its VBA code, save the workbook, and then recheck, we find its signature has become invalid.
