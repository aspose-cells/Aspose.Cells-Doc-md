---
title: Disable Downlevel Revealed Comments while saving to HTML
type: docs
weight: 20
url: /python-net/disable-downlevel-revealed-comments-while-saving-to/
ai_search_scope: cells_pythonnet
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

## **Possible Usage Scenarios**

When you save your Excel file to HTML, Aspose.Cells for Python via .NET reveals downlevel conditional comments. These conditional comments are mostly relevant to older versions of Internet Explorer and are irrelevant to modern web browsers. You can read about them in detail at the following link.

- [Conditional comment - Downlevel-revealed conditional comment](https://en.wikipedia.org/wiki/Conditional_comment#Downlevel-revealed_conditional_comment)

Aspose.Cells for Python via .NET allows you to eliminate these Downlevel Revealed Comments by setting the **HtmlSaveOptions.disable_downlevel_revealed_comments** property to **true**.

## **Disable Downlevel Revealed Comments while saving to HTML**

The following sample code shows the usage of **HtmlSaveOptions.disable_downlevel_revealed_comments** property. The screenshot shows the effect of this property when it is not set to **true**. Please download the [sample Excel file](50528257.xlsx) used in this code and the [output HTML](50528258.zip) generated by it for reference.

![todo:image_alt_text](disable-downlevel-revealed-comments-while-saving-to-html_1.png)

## **Sample Code**

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "HTML-DisableDownlevelRevealedCommentsWhileSavingToHTML.py" >}}
{{< app/cells/assistant language="python-net" >}}
