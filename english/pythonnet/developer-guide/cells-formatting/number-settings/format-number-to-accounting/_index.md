...
description: This article introduces how to format numbers to accounting using Aspose.Cells for Python via .NET API.  
...
2. **Open Format Cells Dialog**:  
   ...
   - Go to the `Home` tab on the Ribbon, look for the `Number` group, and click the small arrow in the bottom‑right corner to open the `Format Cells` dialog box. Alternatively, you can press `Ctrl + 1` as a shortcut to open the `Format Cells` dialog.  
