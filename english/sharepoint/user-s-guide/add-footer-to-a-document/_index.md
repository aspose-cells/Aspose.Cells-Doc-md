---
title: Add Footer to a Document
type: docs
weight: 70
url: /sharepoint/add-footer-to-a-document/
ai_search_scope: cells_sharepoint
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

If you want to add a footer to an Excel file, please choose the “Add Footer with Aspose.Cells” option in the document’s Edit Control Block (ECB).

![todo:image_alt_text](add-footer-to-a-document_1.png)

The following options are available during the add footer setup:

![todo:image_alt_text](add-footer-to-a-document_2.png)

**Section**

Add footer position: Left Section, Center Section, Right Section.

**Footer Script**

It represents the footer formatting script. Script commands:  

- `&P` – Current page number  
- `&N` – Page count  
- `&D` – Current date  
- `&T` – Current time  
- `&A` – Sheet name  
- `&F` – File name without path  
- `&"<FontName>"` – Font name, for example: `&"Arial"`  
- `&"<FontName>, <FontStyle>"` – Font name and font style, for example: `&"Arial,Bold"`  
- `&<FontSize>` – Font size. If this command is followed by a plain number to be printed in the footer, it will be separated from the font height with a space character.  
- `&G` – Image script  

For example: `&"Arial,Bold&8Footer Note`.
