...
In this article, we will learn how to freeze left columns. When you have a huge amount of data in a row, you may be unable to see the left columns when horizontally scrolling the worksheet. You can freeze and lock the first column(s) so that you can see that frozen portion even when the rest of the data is being scrolled. You can easily see headers in the left columns.  

## **Freeze Columns In Excel**  

**![Freeze left column(s) in Excel](freeze-columns.png)**  

1. If you want to freeze left column(s), then first select the column to the right of the column that needs to be frozen.  
...
As you can see, the 1st column is frozen, and the first column is always locked on the left side of the view when you scroll horizontally.
