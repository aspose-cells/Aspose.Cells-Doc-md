...
You can set the values format code of chart series using the [Series.valuesFormatCode](https://reference.aspose.com/cells/javascript-cpp/series/#valuesFormatCode--) property. This property is not only useful for the series that is based on the range inside the worksheet but also works well for the series created with an array of values.

## **Set the Values Format Code of Chart Series**
The following sample code adds a series to an empty chart that has no series initially. It adds the series using an array of values. Once it adds the series, it formats it with the code `$#,##0` using the [Series.valuesFormatCode](https://reference.aspose.com/cells/javascript-cpp/series/#valuesFormatCode--) property, and the number 10000 becomes `$10,000`. The screenshot shows the effect of the code on the [sample Excel file](51740712.xlsx) and the [output Excel file](51740713.xlsx) after execution.
