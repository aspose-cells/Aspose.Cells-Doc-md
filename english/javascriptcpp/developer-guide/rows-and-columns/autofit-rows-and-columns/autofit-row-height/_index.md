---
title: AutoFit Row Height Automatically When Loading a File with JavaScript via C++
...
The height of the row automatically matches the font size of the content, but when the height of the cached row does not match the height of the content in the file, MS Excel will automatically adjust the row height when loading the file, while Aspose.Cells for JavaScript via C++ will not automatically adjust it to improve performance. If you need to use the Aspose.Cells API to automatically match row heights when loading files, you can achieve this through the parameter [autoFitterOptions(AutoFitterOptions)](https://reference.aspose.com/cells/javascript-cpp/loadoptions/#autoFitterOptions-autofitteroptions-) in your code.
