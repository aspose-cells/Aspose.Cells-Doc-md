...
If a column contains text and only a few cells are blank, and a filter is required to select only those rows where blank cells are present, **AutoFilter.matchBlanks(number)** and **AutoFilter.addFilter(number, string)** functions can be used as demonstrated below.  
