...
All worksheets contained in the GridWeb control are stored in a GridWorksheetCollection of the GridWeb control. It is simple to access a particular worksheet by its sheet index.
...
Developers can remove a specific worksheet by specifying its sheet index using the GridWorksheetCollection's removeAt method as demonstrated below in the example code snippet.
...
To add a worksheet with a specific name to the GridWeb control instead of using the default naming scheme, call an overloaded version of the add method that takes a string SheetName. For instance, the example below adds a worksheet named Invoice.
...
Renaming a worksheet can be very useful when working with many worksheets in GridWeb and you decide to change their names to make them more meaningful. For example, a worksheet containing an invoice can be renamed Invoice instead of Sheet1. This topic describes this simple but useful feature.
...
Labels can be used in formulas that refer to data on the same worksheet; if you want to represent a range on another worksheet, you may use a name. **Named ranges** are one of the most powerful features of Microsoft Excel.
...
To add a comment to a worksheet, please follow the steps below:
...
3. Pass the reference to the Comment collection to access the comment.  
...
The example below adds an image URL hyperlink to a worksheet.
...
To sort data from a top‑to‑bottom orientation:

1. Add the Aspose.Cells.GridWeb control to your Web Form.  
2. Access the worksheet that you want to sort.  
3. Sort the range of data in any order (ascending or descending). Be sure to select the top‑to‑bottom orientation.
...
1. Add the Aspose.Cells.GridWeb control to your Web Form.  
2. Access the worksheet that you want to sort.  
3. Sort the range of data in any order (ascending or descending). Be sure to select the left‑to‑right orientation.
...
If your search criterion is not found, a dialog is displayed.
...
| ------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| 1       | Match case               | Indicates whether to use case‑sensitive searching.                                                                                             |
...
| 5       | Find in Formulas/Values  | When **Formulas** is selected, the control will match the formula or unformatted value of the cells if the formula or the unformatted value is present. When **Values** is selected, the control will only match the displayed value of the cells. |

### **Using Replace**

To replace text or values:

1. Open the Find/Replace dialog box by pressing **CTRL+F**, or right‑click a cell and select **Find** before clicking **Replace**.  
...
Aspose.Cells GridWeb now supports adding and removing hyperlinks from the client side. For this, the API provides the "addCelllink" and "delCelllink" functions. The following code snippets demonstrate adding and removing hyperlinks from the client side in GridWeb.

### Sample Code

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "examples-worksheets-add-remove-hyperlink-from-client-side-1.jsp" >}}

You may also link to a sheet using the following code snippet.
...
- **updateCellFontSize**: Params – font size, e.g., ‘12pt’  
- **updateCellFontName**: Params – font name, e.g., ‘System’  
...
Aspose.Cells GridWeb now supports adding and removing sheets by using buttons in the toolbar. For the buttons to be visible on the front end, you need to set **GridWeb1.ShowAddButton** to **true**. The following code snippet demonstrates adding Add/Remove buttons to the GridWeb toolbar.
