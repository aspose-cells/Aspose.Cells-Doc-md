...
description: Learn how to use Excel Camera in Aspose.Cells for Java to create a dynamic picture linked to a cell range that refreshes with the source data and preserves all source formatting.
linktitle: Excel Camera
keywords: Aspose.Cells, Java, Excel Camera, dynamic picture, linked picture, Picture.Formula, UpdateSelectedValue, CreateRange, ToImage, ByteArrayInputStream
type: docs
weight: 90
url: /java/excel-camera/
...
- `Picture.setFormula(String value)` — sets the A1-style reference to the source range that the Camera mirrors, such as `"A1:F10"`.
...
The following code creates a workbook, adds an empty picture anchored at row 10 column 6, links it to the source range `A1:F10` through `setFormula`, refreshes the embedded image data, and saves the workbook.
