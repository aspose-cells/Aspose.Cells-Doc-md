...
PHP developers can use Aspose.Cells for Java in PHP applications. To work with Aspose.Cells for Java and PHP, use PHP version 5 (known as PHP5). PHP4 can also be used to access Aspose.Cells for Java, but it is more complex than using PHP5. 
...
These wrapper classes can be found in the PHP directory of the aspose.cells.zip archive in the form of a PHP file. 
## **Using PHP4**
...
The PHP wrapper classes provide some static methods to create PHP classes for the corresponding Java class, in the ClassFactory with the signature `createXXX()`. If the constructors are overloaded, all corresponding methods in the ClassFactory are defined as `create` + serial number + class name, for example: `createXXX()`, `create1XXX(args…)`, `create2XXX(args…)`, and so on. 
