---
title: Custom sorting in Pivot Table
type: docs
weight: 30
url: /java/custom-sorting-in-pivot-table/
ai_search_scope: cells_java
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

## **Custom sorting in Pivot Table**
By using the Aspose.Cells API, you can sort Pivot Tables on field values. The following code snippet loads the sample Excel file and adds three pivot tables. The first pivot table does not have custom sorting, the second pivot table is sorted on the "SeaFood" row field values, and the third pivot table is sorted on the "28/07/2000" column field values.

Sample source file and output files can be downloaded from here for testing the sample code:

[Source Excel File](SamplePivotSort.xlsx)

[Output Excel File](95584328.xlsx)

[Output PDF File](95584329.pdf)

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-AsposeCellsExamples-PivotTables-PivotTableCustomSort-1.java" >}}
{{< app/cells/assistant language="java" >}}
