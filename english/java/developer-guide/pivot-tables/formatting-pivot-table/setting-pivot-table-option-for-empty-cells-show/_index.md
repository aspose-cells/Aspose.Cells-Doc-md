---
title: Setting Pivot Table Option - For Empty Cells Show
type: docs
weight: 30
url: /java/setting-pivot-table-option-for-empty-cells-show/
ai_search_scope: cells_java
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

## **Introduction**

You can set different pivot table options using Aspose.Cells. One such option is "For empty cells show". Setting this option means that all empty cells in the pivot table are displayed with a specified string.

To find and set this option in Microsoft Excel:

1. Select a pivot table and right-click it.
1. Select **PivotTable Options**.
1. Select the **Layout & Format** tab.
1. Select the **For empty cells show** option and specify a string.

![todo:image_alt_text](setting-pivot-table-option-for-empty-cells-show_1.png)

## **How to Setting Pivot Table Option - For Empty Cells Show**

Aspose.Cells provides the [**PivotTable.setDisplayNullString()**](https://reference.aspose.com/cells/java/com.aspose.cells/pivottable#setDisplayNullString-boolean-) and [**PivotTable.setNullString()**](https://reference.aspose.com/cells/java/com.aspose.cells/pivottable#setNullString-java.lang.String-) properties for setting the "For empty cells show" pivot table option.

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-main-java-com-aspose-cells-examples-articles-SettingPivotTableOption-SettingPivotTableOption.java" >}}
{{< app/cells/assistant language="java" >}}
