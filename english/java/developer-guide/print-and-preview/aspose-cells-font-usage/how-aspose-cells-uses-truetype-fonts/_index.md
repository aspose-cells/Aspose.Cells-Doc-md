...
A spreadsheet may be formatted using various fonts such as Arial, Times New Roman, Verdana, and others. When Aspose.Cells renders a spreadsheet, it attempts to select the fonts that are used in the spreadsheet. However, there are situations when the exact font may not be available, so Aspose.Cells has to substitute a similar font instead.

Below is the process that Aspose.Cells follows behind the scenes.
...
Aspose.Cells attempts to find TrueType fonts on the file system automatically. Most of the time you can rely on Aspose.Cells' default behavior to find TrueType fonts, but sometimes you may need to specify folders that contain the TrueType fonts using the `FontConfigs.setFontFolder` factory method.
...
Keep in mind when copying any fonts that most fonts are copyrighted. First locate the license of a font beforehand and verify it can be freely transferred to another machine.  

{{% /alert %}}

|**Problem** |**Reason** |**Solution** |
| :- | :- | :- |
|The layout and fonts in the rendered document are different from the original. |You are using Aspose.Cells on Linux or macOS where TrueType fonts are not present by default, so Aspose.Cells cannot locate fonts on your computer. |Copy TrueType font files from a Windows machine or install a TrueType font package. Use the `FontConfigs.setFontFolder` factory method to specify the location of the font files.|

{{% alert color="primary" %}}

Refer to the detailed articles on  
