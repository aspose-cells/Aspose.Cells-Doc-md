---
title: Retrieving External Data Sources Details
type: docs
weight: 10
url: /java/retrieving-external-data-sources-details/
ai_search_scope: cells_java
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

{{% alert color="primary" %}} 

Sometimes, you might want to retrieve information about external data sources. For example, you might want to see the SQL connection data. This information can include any types of data required to connect to an SQL server, for example the server URL, user name, underlying table name, SQL query, query type, the location of the table, etc.

Aspose.Cells **provides** some useful calls for retrieving such details from corporate databases linked to template Excel files.

{{% /alert %}} 
## **Retrieving External Data Sources Details**
The following example shows how to get database **connections** and other details. The example utilizes a simple Excel file that contains links to external data **sources** (SQL Server).

When the code runs, the connection details are printed to the console.

**SQL connection information** 

![todo:image_alt_text](retrieving-external-data-sources-details_1.png)

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-main-java-com-aspose-cells-examples-articles-RetrievingExternalDataSourcesDetails-RetrievingExternalDataSourcesDetails.java" >}}

{{< app/cells/assistant language="java" >}}
