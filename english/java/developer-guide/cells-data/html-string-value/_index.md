---
title: Adding HTML Rich Text inside the Cell
linktitle: Html String Value
type: docs
weight: 90
url: /java/adding-html-rich-text-inside-the-cell/
ai_search_scope: cells_java
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

{{% alert color="primary" %}}

Aspose.Cells supports converting Microsoft Excel oriented HTML into XLS/XLSX format. It means, the HTML generated by Microsoft Excel can be converted back to XLS/XLSX format using Aspose.Cells.

Similarly, if there is some simple HTML, Aspose.Cells can convert it into HTML Rich Text. Aspose.Cells provides [**Cell.setHtmlString()**](https://reference.aspose.com/cells/java/com.aspose.cells/cell#setHtmlString-java.lang.String-) property which can take such a simple HTML and convert it to formatted cell text.

{{% /alert %}}

## **Example**

The following screenshot shows the output Excel file generated with Aspose.Cells. As you can see, it shows HTML formatted rich text added inside the cell A1 using the [**Cell.setHtmlString()**](https://reference.aspose.com/cells/java/com.aspose.cells/cell#setHtmlString-java.lang.String-) property.

![todo:image_alt_text](adding-html-rich-text-inside-the-cell_1.png)

This is the sample code that generated the output Excel file as shown inside the above screenshot.

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-main-java-com-aspose-cells-examples-articles-AddImageHyperlinks-AddImageHyperlinks.java" >}}

## Related Articles
- [Display Bullets by setting Cell Value using HTML](/cells/java/display-bullets-by-setting-cell-value-using/)
- [Get HTML5 string from Cell](/cells/java/get-html5-string-from-cell/)

{{< app/cells/assistant language="java" >}}
