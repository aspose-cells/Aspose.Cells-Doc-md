...
| :- | :- |
| FORMAT_TYPE | Can be set to "EXCEL97TO2003" or "EXCEL2007" to generate Microsoft Excel 97‑2003 or Excel 2007 format files. |
...
| IS_WHITE_PAGE_BACKGROUND | A Boolean value specifying whether the page background should be white or the default XLS background color. What the XLS background color is may vary depending on the XLS viewer properties or the operating system color scheme. |
...
| MAXIMUM_ROWS_PER_SHEET | An integer value specifying the maximum number of rows allowed to be shown in a sheet. When set, a new sheet is created for the remaining rows to be displayed. Negative values or zero mean no limit has been set. |
| IS_IGNORE_GRAPHICS | Flag for ignoring graphical elements and exporting text elements only. |
| IS_COLLAPSE_ROW_SPAN | Flag for collapsing row span and avoiding merging cells across rows. |
| IS_IGNORE_CELL_BORDER | Flag for ignoring the cell border. |
| IS_USE_EXCEL_CHART | Flag for using an editable chart in Microsoft Excel format rather than static pictures. The default value is true. |
