...
url: /python-java/transpose-rotate-data-from-rows-to-columns-or-vice-versa/
...
The article then presents three different ways to transpose this data using Aspose.Cells for Python via Java, each suited to a different Excel version and use case.
...
3. Place the dynamic array formula on cell **A6**, just below the source range, by calling `cells.get("A6").setDynamicArrayFormula("=TRANSPOSE(A1:D5)", None, True)`.
...
## **Related Articles**
- [SmartMarker Single Cell Array Rendering | Aspose.Cells for Python via Java](/cells/python-java/smartmarker-array-single-cell-rendering-arrayassingle-extradelimiter/)
