...
Les segments sont utilisés pour filtrer rapidement les données. Ils peuvent être utilisés pour filtrer les données à la fois dans un tableau ou un tableau croisé dynamique. Microsoft Excel vous permet de créer un segment en sélectionnant un tableau ou un tableau croisé dynamique, puis en cliquant sur le*Insertion > Trancheuse*Aspose.Cells pour Python via Java fournit la méthode Worksheet.getSlicers().add() pour créer un slicer.
