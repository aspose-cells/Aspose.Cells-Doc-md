---
title: Comment formater un nombre en comptabilité avec Golang via C++
linktitle: Comment formater un nombre en comptabilité
type: docs
weight: 10
url: /fr/go-cpp/how-to-format-number-to-accounting/
description: Cet article expliquera comment formater un nombre en comptabilité en utilisant l’API Aspose.Cells for C++.
...
## **Comment formater un nombre en comptabilité dans Aspose.Cells for C++**
Pour formater des nombres en format comptable dans Aspose.Cells for C++, vous pouvez utiliser l'objet `Style` associé à une cellule ou une plage de cellules. L'objet `Style` vous permet de définir diverses options de mise en forme, y compris les formats numériques. Le format comptable a généralement un code de format qui peut varier en fonction des exigences spécifiques (par exemple, afficher ou non des symboles monétaires, des décimales, etc.).

Voici un exemple de base pour appliquer un format numérique comptable à une cellule dans Aspose.Cells for C++ :

1. **Référence Aspose.Cells** : Assurez-vous que vous avez référencé Aspose.Cells for C++ dans votre projet. Vous pouvez l'obtenir via NuGet ou le site Aspose.
