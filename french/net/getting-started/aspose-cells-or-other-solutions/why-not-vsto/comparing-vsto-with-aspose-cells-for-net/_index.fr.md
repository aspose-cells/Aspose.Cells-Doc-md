...
Cet article compare l'utilisation de VSTO (Visual Studio Tools for Office) avec d'autres approches pour le développement de solutions basées sur Microsoft Office. En particulier, il examine Aspose.Cells et fournit une comparaison du fonctionnement des deux solutions. L'article donne aux développeurs des informations qu'ils peuvent utiliser pour analyser, comparer et évaluer différentes options avant d'adopter une solution.
...
- Consultez la [documentation Aspose.Cells](https://docs.aspose.com/cells/net/).
- Voyez comment cela fonctionne dans les [démonstrations en ligne](https://github.com/aspose-cells/Aspose.Cells-for-.NET).
- Essayez-le : [téléchargez](https://downloads.aspose.com/cells/net) une version d'évaluation gratuitement.
...
1. Un fichier Microsoft Excel (TempBook.xls) est utilisé comme modèle.
   Le classeur contient quelques feuilles de calcul avec quelques cellules remplies de données.
1. Le code d'exemple place 1000*20 enregistrements sur la première feuille de calcul dans le fichier Excel modèle.
   La feuille de calcul est remplie de données constantes (fausses) dans les cellules.
