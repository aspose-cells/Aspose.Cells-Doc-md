...
À partir de la v8.0.0 de Aspose.Cells for .NET, nous avons fourni les options d'utilisation de la mémoire pour des considérations de performances. La propriété MemorySetting est maintenant disponible dans les classes LoadOptions et WorkbookSettings.
##### **Exemple**
...
{{% alert color="primary" %}} 
 ...
 Veuillez consulter l'article détaillé sur[Optimisation de la mémoire lors de l'utilisation de fichiers volumineux](/cells/fr/net/optimizing-memory-usage-while-working-with-big-files-having-large-datasets/).
...
À partir de maintenant, les utilisateurs peuvent utiliser la propriété booléenne ShowFormulas offerte par Worksheet pour changer la vue de la formule à la valeur d'une feuille de calcul donnée.
...
Nous avons essentiellement corrigé l'erreur de frappe dans le nom de la propriété pour SeriesCollection.SecondCatergoryData. Vous pouvez utiliser la propriété SeriesCollection.SecondCategoryData maintenant, alors que la propriété d'origine SeriesCollection.SecondCatergoryData a été marquée comme obsolète.
