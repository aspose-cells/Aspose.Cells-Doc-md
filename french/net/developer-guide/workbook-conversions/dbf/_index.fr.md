...
{{% alert color="primary" %}}
Aspose.Cells offre une prise en charge complète pour la lecture et l'écriture des fichiers DBF (dBASE). Vous pouvez charger des fichiers dBASE III et dBASE IV existants dans un objet Workbook, manipuler les données à l'aide de la riche API d'Aspose.Cells, puis enregistrer le classeur au format DBF pour une utilisation avec des applications de bases de données existantes.

## **Introduction**
Le DBF (DataBase File) est un format de fichier de base de données hérité, introduit à l'origine par dBASE au début des années 1980. Malgré l'ancienneté du format, les fichiers DBF sont encore largement utilisés dans de nombreux secteurs pour stocker des données structurées, notamment dans la comptabilité, les SIG et d'autres applications spécialisées. Aspose.Cells vous permet d'intégrer ces fichiers hérités de manière transparente dans les flux de travail modernes de feuilles de calcul .NET.
...
- Créer de nouveaux fichiers DBF à partir de zéro ou en transformant des données issues d'autres formats de feuilles de calcul.
...
- **dBASE III** — La variante d'origine la plus largement prise en charge du format DBF.
...
### Supported Features
La bibliothèque fournit une prise en charge complète pour les opérations suivantes :
...
Pour lire un fichier DBF, vous devez créer une instance de `LoadOptions`, définir sa propriété `LoadFormat` sur `LoadFormat.Dbf`, puis la transmettre au constructeur `Workbook` avec le chemin du fichier. Une fois chargées, les données deviennent accessibles via la collection `Worksheets`, où vous pouvez parcourir les cellules, extraire des valeurs ou manipuler les données selon vos besoins.
...
L'exemple suivant montre comment créer un nouveau fichier DBF à partir de zéro. Il remplit une feuille de calcul avec des exemples de données contenant différents types de données (chaînes, nombres et dates) pour illustrer la façon dont les types de champs sont gérés lors de l'exportation au format DBF.
