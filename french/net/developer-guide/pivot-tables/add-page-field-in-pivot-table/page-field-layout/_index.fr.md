...
description: Apprenez à contrôler la disposition de la zone des champs de page dans un tableau croisé dynamique à l'aide d'Aspose.Cells for .NET, notamment en définissant l'ordre d'affichage, le nombre de retours à la ligne et l'ordre des champs de page en haut du tableau croisé dynamique.
linktitle: Modifier la disposition des champs de page dans un tableau croisé dynamique
keywords: Aspose.Cells, bibliothèque .NET, feuille de calcul, tableau croisé dynamique, champ de page, ordre des champs de page, nombre de retours à la ligne des champs de page, déplacer le champ de page
type: docs
weight: 191
...
- `PivotTable.PageFieldWrapCount` définit combien de champs de page sont placés par ligne ou colonne avant le retour à la ligne.
...
// Configurer la disposition de la zone des champs de page : placer les champs de page horizontalement d'abord, retour à la ligne après chaque 2
...
Dans le troisième scénario, nous conservons ce jeu de données et cette affectation de champs, définissons une disposition neutre (`OverThenDown` avec un nombre de retours à la ligne de `2`), puis démontrons l'opération `PageFields.Move`. L'appel `Move(0, 1)` déplace le champ de page à l'index 0 (`Year`) vers la position 1, et le champ de page qui était à la position 1 (`Region`) se décale vers la position 0. Après cet appel, `Region` est le premier champ de page et `Year` est le second. Le mode de retour à la ligne et d'ordre reste inchangé, donc la bande est toujours rendue horizontalement côte à côte — seul l'ordre des deux listes déroulantes a été inversé.
...
- [Actualiser le tableau croisé dynamique](/cells/fr/net/refresh-pivot-table/) — explique `RefreshData` et `CalculateData`, qui sont requis après le réordonnancement des champs de page.
- [Appliquer un style au tableau croisé dynamique](/cells/fr/net/apply-style-to-pivot-table/) — montre comment mettre en forme le tableau croisé dynamique rendu une fois la bande des champs de page disposée.
