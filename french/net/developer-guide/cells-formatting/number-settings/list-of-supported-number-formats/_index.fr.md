---
title: Liste des formats de nombre pris en charge
type: docs
weight: 30
url: /fr/net/list-of-supported-number-formats/
---
## **Aspose.Cells**
 Le composant Aspose.Cells propose des formats de nombres intégrés pour configurer les formats d'affichage des nombres et des dates. Ces formats numériques intégrés peuvent être appliqués à l'aide de la**Nombre** propriété de la**Style** objet. Tous les formats numériques intégrés reçoivent des valeurs numériques uniques. Les développeurs peuvent attribuer n'importe quelle valeur numérique souhaitée au*Nombre* propriété de**Style** objet et donc le format d'affichage est appliqué. Cette approche est plus rapide. Les formats numériques intégrés pris en charge par le composant Aspose.Cells sont indiqués ci-dessous :

|**Évaluer**|**Taper**|**Formater la chaîne**|
|:- |:- |:- |
|0 | Général| Général|
|1 | Décimal|0 |
|2 | Décimal|0.00 |
|3 | Décimal|# ,##0
|
|4 | Décimal|# ,##0.00
|
|5 | Devise|$#,##0;$-#,##0 |
|6 | Devise|$#,##0;$-#,##0 |
|7 | Devise|$#,##0.00;$-#,##0.00 |
|8 | Devise|$#,##0.00;$-#,##0.00 |
|9 | Pourcentage|0% |
|10 | Pourcentage|0.00% |
|11 | Scientifique| 0.00E+00|
|12 |Fraction|# ?/?
|
|13 |Fraction|# */*
|
|14 | Date| m/j/aa|
|15 | Date| j-mmm-aa|
|16 | Date| d-mmm|
|17 | Date| mmm-aa|
|18 | Temps| h:mm AM/PM|
|19 | Temps| h:mm:ss AM/PM|
|20 | Temps| hmm|
|21 | Temps| h:mm:ss|
|22 | Temps| j/j/aa h:mm|
|37 | Devise|# ,##0;-#,##0
|
|38 | Devise|# ,##0;-#,##0
|
|39 | Devise|# ,##0.00;-#,##0.00
|
|40 | Devise|# ,##0.00;-#,##0.00
|
|41 | Comptabilité|_ * #,##0_ ;_ * "_ ;_ @_ |
|42 | Comptabilité|_ $* #,##0_ ;_ $* "_ ;_ @_ |
|43 | Comptabilité|_ * #,##0.00_ ;_ * "??_ ;_ @_ |
|44 | Comptabilité|_ $* #,##0.00_ ;_ $* "??_ ;_ @_ |
|45 | Temps| mm:ss|
|46 | Temps| h :mm:ss|
|47 | Temps| mm:ss.0|
|48 | Scientifique|## 0.0E+00
|
|49 | Texte|@ |
## **Aspose.Cells Grille Suite**
Comme nous savons qu'il existe deux contrôles Grid Aspose.Cells : Aspose.Cells.GridDesktop & Aspose.Cells.GridWeb. Les deux contrôles prennent en charge un grand nombre de formats numériques, qui sont divisés en deux sections par rapport à chaque contrôle comme suit :

- Formats de nombre pris en charge dans Aspose.Cells.GridDesktop
- Formats de nombre pris en charge dans Aspose.Cells.GridWeb
### **Formats de nombre pris en charge dans Aspose.Cells.GridDesktop**
Aspose.Cells.GridWeb prend également en charge 59 types de formats de nombres répertoriés ci-dessous :

|**Indice**|**Formats de nombre**|
|:- |:- |
|0 | Général|
|1 |0 |
|2 |0.00 |
|3 |# ,##0
|
|4 |# ,##0.00
|
|5 |\"$\"#,##0_);(\"$\"#,##0) |
|6 |\"$\"#,##0_);(\"$\"#,##0) |
|7 |\"$\"#,##0.00_);(\"$\"#,##0.00) |
|8 |\"$\"#,##0.00;\"$\"-#,##0.00 |
|9 |0% |
|10 |0.00% |
|11 | 0.00E+00|
|12 |# ?/?
|
|13 |# */*
|
|14 | m/j/aa|
|15 | j-mmm-aa|
|16 | d-mmm|
|17 | mmm-aa|
|18 | h:mm AM/PM|
|19 | h:mm:ss AM/PM|
|20 | hmm|
|21 | h:mm:ss|
|22 | j/j/aa h:mm|
|23 |\"$\"#,##0;(\"$\"#,##0) |
|24 |\"$\"#,##0.00;(\"$\"#,##0.00) |
|25 |\"$\"#,##0.00;(\"$\"#,##0.00) |
|26 |\"$\"#,##0.00_);(\"$\"#,##0.00) |
|27 |M月D日|
|28 |M月D日|
|29 |M月D日|
|30 |M/J/AA|
|31 |AAAA年M月D日|
|32 | 时 mm 分|
|33 |h\ 时 \"mm\" 分 \"ss\" 秒 \"|
|34 | tt 时 mm 分|
|35 |tth 时 mm 分 ss 秒|
|36 |AAAA年M月|
|37 |_(#,##0 );(#,##0) |
|38 |_(#,##0 );(#,##0) |
|39 |_(#,##0.00 );(#,##0.00) |
|40 |_(#,##0.00 );(#,##0.00) |
|41 |*(\"$\"* ***#,##0** **);*** **(\"$\"** *_(#,##0);* (\"$\"*\"-\" *);* (@_) |
|42 |*(* ***#,##0** **);*** **(** *_(#,##0);* (*-\" *);* (@_) |
|43 |*(* ***#,##0.00** **);*** **(** *_(#,##0.00);* (*-\"?? *);* (@_) |
|44 |*(\"$\"* ***#,##0.00** **);*** **(\"$\"** *_(#,##0.00);* (\"$\"*\"-\"?? *);* (@_) |
|45 | mm:ss|
|46 | h:mm:ss|
|47 | mm:ss.0|
|48 |## 0.0E+0
|
|49 |@ |
|50 |AAAA年M月|
|51 |M月D日|
|52 |AAAA年M月|
|53 |M月D日|
|54 |M月D日|
|55 | tt 时 mm 分|
|56 |tth 时 mm 分 ss 秒|
|57 |AAAA年M月|
|58 |M月D日|

{{% alert color="primary" %}} 

Dans certains formats numériques, vous remarquerez peut-être des caractères comme 月. Ce sont en fait des caractères chinois et peuvent être utilisés dans les versions chinoise et japonaise de MS Excel.

{{% /alert %}} 
### **Formats de nombre pris en charge dans Aspose.Cells.GridWeb**
Aspose.Cells.GridWeb prend également en charge 59 types de formats de nombres répertoriés ci-dessous :

|**Types de format de nombre**|**Formats de nombre**|
|:- |:- |
| Général| Général|
| Décimal1|0 |
| Décimal2|0.00 |
| Décimal3|# ,##0
|
| Décimal4|# ,##0.00
|
| Devise1|$#,##0;$-#,##0 |
| Devise2|$#,##0;$-#,##0 |
| Devise3|$#,##0.00;$-#,##0.00 |
| Devise4|$#,##0.00;$-#,##0.00 |
| Devise5|# ,##0;-#,##0
|
| Devise6|# ,##0;-#,##0
|
| Devise7|# ,##0.00;-#,##0.00
|
| Devise8|# ,##0.00;-#,##0.00
|
| Devise9|$#,##0;($#,##0) |
| Monnaie10|$#,##0;($#,##0) |
| Monnaie11|$#,##0.00;($#,##0.00) |
| Monnaie12|$#,##0.00;($#,##0.00) |
| Pourcentage1|0% |
| Pourcentage2|0.00% |
| Scientifique1| 0.00E+00|
| Scientifique2|## 0.0E+00
|
| Fraction1|# ?/?
|
| Fraction2|# */*
|
| Date1| m/j/aa|
| Date2| j-mmm-aa|
| Date3| d-mmm|
| Date4| mmm-aa|
| Temps1| h:mm AM/PM|
| Temps2| h:mm:ss AM/PM|
| Temps3| hmm|
| Temps4| h:mm:ss|
| Temps5| j/j/aa h:mm|
| Temps6| mm:ss|
| Heure7| h:mm:ss|
| Temps8| mm:ss.0|
| Comptabilité1|_ * #,##0_ ;_ * "_ ;_ @_ |
| Comptabilité2|_ $* #,##0_ ;_ $* "_ ;_ @_ |
| Comptabilité3|_ * #,##0.00_ ;_ * "??_ ;_ @_ |
| Comptabilité4|_ $* #,##0.00_ ;_ $* "??_ ;_ @_ |
| Texte|@ |
| DateEst1| AAAA?M?|
| DateEst2| MARYLAND?|
| DateEst3| MARYLAND?|
| DateEst4|M/J/AA|
| DateEst5| AAAA?M?D?|
| DateEst6| AAAA?M?|
| DateEst7| AAAA?M?|
|DateEst8| MARYLAND?|
| DateEst9| AAAA?M?|
| DateEst10| MARYLAND?|
| DateEst11| MARYLAND?|
| DateEst12| AAAA?M?|
| DateEst13| MARYLAND?|
| Heure de l'Est1| hmm?|
| Heure de l'Est2| h?mm?ss?|
| Heure de l'Est3| tt?mm?|
| Heure de l'Est4| tt?mm?ss?|
| Heure de l'Est5| tt?mm?|
| Heure de l'Est6| tt?mm?ss?|

