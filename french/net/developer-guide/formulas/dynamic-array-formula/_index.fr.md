...
description: Comment utiliser la bibliothèque Aspose.Cells pour calculer des formules de tableau dynamiques dans Microsoft Excel. En chargeant un fichier Excel existant ou en créant un nouveau fichier Excel, nous pouvons utiliser la méthode fournie par Aspose.Cells pour calculer la formule de tableau dynamique et obtenir le résultat. Enfin, nous enregistrons le fichier Excel modifié sur le disque.
...
Veuillez consulter le code d'exemple suivant qui charge le [fichier Excel d'exemple](dynamicArrayFormula.xlsx) contenant des données fictives. Après avoir chargé le fichier, appelez la fonction [Cell.SetDynamicArrayFormula](https://reference.aspose.com/cells/net/aspose.cells/cell/setdynamicarrayformula/#setdynamicarrayformula) pour définir une formule de tableau dynamique et la fonction [Workbook.RefreshDynamicArrayFormulas](https://reference.aspose.com/cells/net/aspose.cells/workbook/refreshdynamicarrayformulas/#refreshdynamicarrayformulas) pour actualiser les formules de tableau dynamique avant d'appeler le calcul de formules, et enfin enregistrer le classeur sous le [fichier Excel de sortie](out.xlsx). 
