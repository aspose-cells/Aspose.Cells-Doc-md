...
type: docs
weight: 30
url: /fr/net/aspose-cells-gridweb/read-the-values-of-the-gridweb-cells-on-client-side/
keywords: GridWeb, cellule, valeur de la cellule
description: Cet article présente comment obtenir la valeur de la cellule dans GridWeb.
---

## **Scénarios d'utilisation possibles**
Vous pouvez lire les valeurs des cellules de GridWeb côté client en utilisant la méthode gridwebinstance.getCellsArray(). Une fois appelée, elle renverra le tableau de toutes les cellules de la feuille active. Vous pouvez ensuite utiliser les méthodes suivantes pour récupérer la valeur et d'autres informations des cellules.

- gridwebinstance.getCellName()
- gridwebinstance.getCellValueByCell()
- gridwebinstance.getCellRow()
- gridwebinstance.getCellColumn()
## **Lire les valeurs des cellules de GridWeb côté client**
Le code d'exemple suivant récupère toutes les cellules, puis imprime leur nom, leur valeur, leur ligne et leur colonne. Vous pouvez voir sa sortie dans la console en bas de cet article. La capture d'écran suivante montre la sortie de la console du code d'exemple sur Google Chrome.
## **Capture d'écran**
...
Veuillez appeler la fonction JavaScript ReadGridWebCells() comme le montre le code d'exemple ci-dessus.
