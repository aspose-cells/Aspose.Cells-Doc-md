---
title: comment personnaliser l apparence de l interface utilisateur pour le toast d alerte de message dans GridJs  
type: docs
weight: 250
url: /fr/net/aspose-cells-gridjs/how-to-customize-toast/
description: Cet article décrit comment personnaliser l apparence de l UI pour le toast de message dans GridJs.
...
# Guide du toast personnalisé pour GridJs

## Aperçu

GridJs propose un système de notifications toast personnalisables flexible qui vous permet de remplacer les messages toast par défaut par votre propre implémentation de notification. Ce guide démontre différentes approches pour personnaliser les notifications toast selon les besoins de votre application.

## Table des matières

- [Utilisation de base](#basic-usage)
- [Exemples d'intégration](#integration-examples)
  - [Superbe toast personnalisé](#1-beautiful-custom-toast)
...
- [Cas d'utilisation avancés](#advanced-use-cases)
  - [Journalisation et Analyse](#6-logging-and-analytics)
  - [Toast adaptatif mobile](#7-mobile-friendly-toast)
...
**Paramètres :**
- `title` (string) : Titre de la notification toast
- `content` (string) : Contenu du message toast
...
- `progressBar` : Affiche une barre de progression de compte à rebours
- `positionClass` : Contrôle la position du toast (coin supérieur droit, coin inférieur gauche, etc.)
...
- Positionnement en bas (convivial pour le pouce)
- Largeur basée en pourcentage (max 80%)
...
Choisissez des durées de dismissal automatique appropriées en fonction de la longueur du contenu :
...
Enveloppez votre fonction toast dans un try-catch pour empêcher de faire planter la feuille de calcul :
...
Fournissez une méthode pour rétablir le comportement par défaut des notifications :
...
**Problème :** La toast a un mauvais rendu sur les appareils mobiles.
