---
title: Charger un classeur avec une taille de papier d imprimante spécifiée via JavaScript et C++
linktitle: Charger le classeur avec la taille de papier de l imprimante spécifiée
type: docs
weight: 430
url: /fr/javascript-cpp/load-workbook-with-specified-printer-paper-size/
description: Apprenez comment définir la taille du papier de l imprimante lors du chargement d’un classeur en utilisant Aspose.Cells for JavaScript via C++.
---

{{% alert color="primary" %}}
Vous pouvez spécifier la taille de papier de votre choix lors du chargement de votre classeur en utilisant la propriété [**LoadOptions.paperSize(PaperSizeType)**](https://reference.aspose.com/cells/javascript-cpp/loadoptions/#paperSize-papersizetype-). Notez que si vous créez un nouveau fichier dans MS Excel, vous constaterez que la taille du papier est la même que le paramètre de l'imprimante par défaut de votre machine.
{{% /alert %}}

Le code d'exemple suivant illustre l'utilisation de la propriété [**LoadOptions.paperSize(PaperSizeType)**](https://reference.aspose.com/cells/javascript-cpp/loadoptions/#paperSize-papersizetype-). Il crée d'abord un classeur, puis le sauvegarde dans un flux mémoire au format XLSX. Ensuite, il le charge avec une taille de papier A5 et le sauvegarde au format PDF. Ensuite, il le recharge avec une taille de papier A3 et le sauvegarde à nouveau en PDF. Si vous ouvrez les PDF générés et vérifiez leur taille de papier, vous verrez qu'ils sont différents. L'un est A5 et l'autre A3. Veuillez télécharger le [PDF de sortie A5](5115234.pdf) et le [PDF de sortie A3](5115233.pdf) générés par le code pour référence.
