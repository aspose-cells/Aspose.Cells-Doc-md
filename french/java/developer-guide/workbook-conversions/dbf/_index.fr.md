...
- Importer des données à partir de fichiers DBF existants dans des objets Workbook d'Aspose.Cells pour un traitement ultérieur ou une conversion vers d'autres formats.
- Créer de nouveaux fichiers DBF à partir de zéro ou en convertissant des données à partir d'autres formats de feuilles de calcul.
...
- Lecture des données DBF dans un objet Workbook, avec conservation de tous les enregistrements et définitions de champs.
...
- Conservation des définitions de champs telles que le nom, le type et la longueur lors des opérations de lecture/écriture.
...
Aspose.Cells simplifie le chargement de données à partir d'un fichier DBF dans un objet Workbook. La bibliothèque utilise la classe `LoadOptions` pour spécifier le format source, garantissant que les données sont interprétées correctement pendant le processus de chargement.

### Lecture d'un fichier DBF avec Aspose.Cells
Pour lire un fichier DBF, vous devez créer une instance de `LoadOptions`, définir sa propriété `LoadFormat` sur `LoadFormat.Dbf`, et la passer au constructeur `Workbook` avec le chemin du fichier. Une fois chargé, les données deviennent accessibles via la collection `getWorksheets()`, où vous pouvez parcourir les cellules, extraire des valeurs ou manipuler les données selon vos besoins.
...
2. Accédez à la première feuille de calcul à partir de la collection `getWorksheets()`.
...
L'exemple suivant montre comment créer un nouveau fichier DBF à partir de zéro. Il remplit une feuille de calcul avec des données d'exemple (chaînes, nombres et dates) pour illustrer la façon dont les types de champs sont gérés lors de l'exportation vers le format DBF.
...
- Les **chaînes** sont mappées vers des champs de type caractère (C).
- Les **valeurs numériques** (entiers et décimales) sont mappées vers des champs numériques (N).
...
### Règles pour les noms de champs
