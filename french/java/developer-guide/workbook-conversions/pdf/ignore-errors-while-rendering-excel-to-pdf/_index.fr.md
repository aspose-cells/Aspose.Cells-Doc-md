---
title: Ignorer les erreurs lors du rendu d'Excel au format PDF
type: docs
weight: 70
...
Parfois, lorsque vous convertissez votre fichier Excel en PDF, des erreurs ou des exceptions se produisent et le processus de conversion se termine. Vous pouvez ignorer toutes ces erreurs pendant le processus de conversion en utilisant le[**PdfSaveOptions.IgnoreError**](https://reference.aspose.com/cells/java/com.aspose.cells/pdfsaveoptions#IgnoreError)propriété. De cette façon, le processus de conversion se terminera en douceur sans générer d'erreur ou d'exception, mais la perte de données peut se produire. Par conséquent, veuillez n'utiliser cette propriété que si la perte de données n'est pas critique pour vous.
