---
title: Aspose.Cells for Android via Java 20.9 Notes de mise à jour
type: docs
weight: 9
url: /fr/java/aspose-cells-for-android-via-java-20-9-release-notes/
---
{{% alert color="primary" %}}

Cette page contient les notes de version pour Aspose.Cells for Android via Java 20.9.
...
Ajoutez un nouveau Slicer en utilisant ListObjet comme source de données.
...
### **Ajoute la méthode Cells.RemoveDuplicates().**
 ...
 Méthode surchargée de**Cells.Supprimer les doublons(...)** pour la commodité de l'utilisateur, supprimez les lignes en double dans toute la feuille.
...
Indique si la structure et la fenêtre sont protégées par mot de passe.
...
Pour des raisons de performances et de commodité pour l'utilisateur, nous ajoutons cette propriété et définissons sa valeur par défaut sur false afin que l'utilisateur puisse se concentrer sur les fonctions qui n'ont pas été prises en charge par le moteur intégré. Si l'implémentation existante de l'utilisateur de**RésuméCalculMoteur** modifié le calcul de certaines fonctions intégrées, l'utilisateur doit remplacer cette propriété pour la rendre aussi**vrai** de**20.9**.
...
Indique s'il faut conserver les macros dans le classeur cible. Cela n'a d'effet que lorsque le classeur d'origine ne contient pas de macros.
