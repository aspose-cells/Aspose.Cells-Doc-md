...
type: docs
weight: 250
url: /fr/python-net/aspose-cells-gridjs/client/
keywords: GridJs, personnalisé, logo, paramètre, API, gridjs, python, éditer, feuille de calcul, vue, visionneuse, éditeur, excel, API js, API côté client
...
| `isCollaboratif` | Supporte-t-il le mode collaboratif . | `false` | Oui |
...
| `token` | Définir le jeton d'authentification. Lorsque le jeton n'est pas nul, l'en-tête `Authorization: Bearer {token}` sera automatiquement ajouté aux en-têtes de la requête. Vous pouvez utiliser `xs.refreshToken(token)` pour définir un nouveau jeton. | Aucun | Oui | 
| `showBottombarStats` | Afficher la statistique de la barre inférieure.<br>La valeur par défaut est true. | `true` | Oui |   
| `showRowAppenderToolbar` | Afficher la barre d'outils d'ajout en lot de lignes.<br>La valeur par défaut est true. | `true` | Oui |   
...
```
- définir une toast personnalisé
```javascript
...
```
- Définir l'appel de fonction avant la sauvegarde 
```javascript
...
- Configurer les paramètres collaboratifs en mode collaboratif, assurez-vous de définirCollaborativeSetting avant de définir l'ID unique  
```javascript
...
- Définir la plage éditable / en lecture seule
```javascript
...
```
-  Supprimer le contenu de la cellule ou effacer le style dans la cellule sélectionnée  
```javascript
...
-  Supprimer le contenu de la cellule ou effacer le style dans la zone de cellule souhaitée
```javascript
...
```
## API pour l'objet forme
-  Changer la couleur de fond pour l'objet forme
```javascript
...
-  Appliquer les paramètres de police pour l'objet zone de texte
```javascript
