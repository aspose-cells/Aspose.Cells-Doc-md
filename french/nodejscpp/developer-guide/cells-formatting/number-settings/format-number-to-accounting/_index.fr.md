...
type: docs
weight: 10
url: /fr/nodejs-cpp/how-to-format-number-to-accounting/
description: Cet article présentera comment formater un nombre en comptabilité en utilisant l API Aspose.Cells for Node.js via C++.
...
## **Comment formater un nombre en comptabilité dans Aspose.Cells for Node.js via C++**
Pour formater des nombres en format comptable dans Aspose.Cells for Node.js via C++, vous pouvez utiliser l'objet `Style` associé à une cellule ou une plage de cellules. L'objet `Style` vous permet de définir diverses options de mise en forme, y compris les formats de nombres. Le format comptable possède généralement un code de format qui peut varier en fonction des exigences spécifiques (comme l'affichage de symboles monétaires, décimales, etc.).

Voici un exemple basique pour appliquer un format de nombre comptable à une cellule dans Aspose.Cells for Node.js via C++ :

1. **Référence Aspose.Cells** : Assurez-vous que Aspose.Cells for Node.js via C++ est référencé dans votre projet. Vous pouvez l'obtenir sur le site Web d'Aspose.
