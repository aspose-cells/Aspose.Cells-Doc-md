...
type: docs
weight: 10
url: /fr/nodejs-cpp/how-to-format-number-to-special/
description: Cet article expliquera comment formater un nombre en utilisant un format spécial dans Aspose.Cells for Node.js via C++.
...
## **Comment formater un nombre en spécial dans Aspose.Cells for Node.js via C++**
Dans Aspose.Cells for Node.js via C++, la mise en forme des nombres en un format spécial implique l'utilisation de l'objet `Style` associé à une cellule. L'objet `Style` vous permet de spécifier diverses options de mise en forme, y compris les formats numériques. Les formats numériques spéciaux peuvent inclure des formats comme les dates, les heures, les numéros de téléphone, les codes postaux ou tout autre format numérique personnalisé que vous souhaitez appliquer.

Voici un guide étape par étape sur la façon de formater un nombre en format spécial en utilisant Aspose.Cells for Node.js via C++ :
