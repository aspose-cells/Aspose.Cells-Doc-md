---
title : "AutoFit Rows for Merged Cells" 
description : "" 
weight : 16450 
toc : false
type: docs
url: /java/developerguide/technicalarticles/mngrowscolumnsandcells/autofit+rows+for+merged+cells/
---

# Aspose.Cells for Java : AutoFit Rows for Merged Cells


Microsoft Excel provides a feature that allows you to auto-size the height of a cell according to its content. The feature is called auto-fit rows. Microsoft Excel doesn't set auto-fit operation on merged cells natively. Sometimes the feature becomes vital for a user who really needs to implement auto-fit rows on merged cells too.

Aspose.Cells supports this feature through the [AutoFitterOptions](https://apireference.aspose.com/java/cells/com.aspose.cells/AutoFitterOptions) API. Using this API, it is possible to auto-fit rows in a worksheet including merged cells.

After executing the code, Aspose.Cells auto-fits rows for merged cells.

**Output Excel file**  
![](https://docs2.aspose.com/cells/java/attachments/5276683/5472885.png)


## Attachments:

![](https://docs2.aspose.com/cells/java/images/icons/bullet_blue.gif) [AutoFit Rows for Merged Cells-001.png](https://docs2.aspose.com/cells/java/attachments/5276683/5472885.png) (image/png)  

