---
title: احذف الصفوف والأعمدة الفارغة في ورقة عمل
type: docs
weight: 360
url: /ar/java/delete-blank-rows-and-columns-in-a-worksheet/
---
{{% alert color="primary" %}} 

من الممكن حذف جميع الصفوف والأعمدة الفارغة من ورقة العمل. هذا مفيد على سبيل المثال عند إنشاء ملف PDF من ملف Microsoft Excel وتريد تحويل الصفوف والأعمدة التي تحتوي على بيانات فقط.

استخدم الطرق Aspose.Cells التالية لحذف الصفوف والأعمدة الفارغة:

1.  لحذف الصفوف الفارغة ، استخدم ملف[Cells.deleteBlankRows ()](https://reference.aspose.com/cells/java/com.aspose.cells/cells#deleteBlankRows\(\)) طريقة.
1.  لحذف الأعمدة الفارغة ، استخدم ملف[Cells.deleteBlankColumns ()](https://reference.aspose.com/cells/java/com.aspose.cells/cells#deleteBlankColumns\(\)) طريقة.

{{% /alert %}} 
## **حذف الصفوف الفارغة**
{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-main-java-com-aspose-cells-examples-articles-DeletingBlankRows-DeletingBlankRows.java" >}}
## **حذف الأعمدة الفارغة**
{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-main-java-com-aspose-cells-examples-articles-DeletingBlankColumns-DeletingBlankColumns.java" >}}
