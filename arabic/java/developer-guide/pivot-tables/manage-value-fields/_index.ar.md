...
يوضح هذا السيناريو كيفية وضع حقل أساسي واحد (`Amount`) في منطقة البيانات في جدول محوري موجود. تضع بنية الجدول المحوري المشتركة `Category` و`Item` على محور الصفوف و`Year` على محور الأعمدة. بعد العملية، يظهر `Amount` في منطقة البيانات ويُحسب كـ `Sum` للقيمة `Amount` افتراضياً.
...
يبدأ هذا السيناريو من نفس بنية الجدول المحوري كالسيناريو 1 ولكنه يضيف حقل `Amount` إلى منطقة البيانات مرتين. يشير كلا حقلَي البيانات إلى نفس عمود المصدر، ومع ذلك يتم تجاوز الحقل الثاني باستخدام أداة الضبط `PivotField.setFunction(...)` بحيث يصبح `COUNT` بدلاً من `SUM` الافتراضي.
...
مع وجود حقلَي بيانات، يصبح `PivotTable.getValuesField()` قابلاً للاستخدام. يسحب هذا السيناريو حقل التجميع الافتراضي إلى منطقة الأعمدة بحيث يظهر كل مقياس في منطقة البيانات ككتلة عمود خاصة به بجانب `Year`.
