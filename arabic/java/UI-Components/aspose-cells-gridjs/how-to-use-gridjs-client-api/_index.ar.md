...
| `isCollaborative` | ما إذا كانت الحالة التعاونية مدعومة. | `false` | نعم |
...
| `token` | تعيين رمز المصادقة. عندما يكون الرمز غير null، سيتم إضافة رأس `Authorization: Bearer {token}` تلقائيًا إلى رؤوس الطلب. يمكنك استخدام `xs.refreshToken(token)` لتعيين رمز جديد. | لا شيء | نعم |    
| `showBottombarStats` | هل تريد عرض إحصائيات الشريط السفلي.<br>القيمة الافتراضية هي true. | `true` | نعم |   
| `showRowAppenderToolbar` | هل تريد عرض شريط أدوات إضافة الصفوف الدفعي.<br>القيمة الافتراضية هي true. | `true` | نعم |   
...
- تحديد مدى قابل للتحرير/للقراءة فقط
```javascript
