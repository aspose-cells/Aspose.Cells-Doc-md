...
description: تعرّف على كيفية إضافة وتكوين حقول الصفحات في الجداول المحورية باستخدام Aspose.Cells for C++، بما في ذلك إضافة حقول الصفحات، والتصفية بخيار واحد، والتصفية متعددة الخيارات.
keywords: Aspose.Cells, C++, pivot table, page field, PivotFieldType.Page, PageFields, IsMultipleItemSelectionAllowed, CurrentPageItem, PivotItem, IsHidden, filter
type: docs
weight: 250
...
حقل الصفحة هو حقل محوري يتحكم في *أي مجموعة فرعية* من البيانات المصدرية يعرضها جسم الجدول المحوري. يراه المستخدمون النهائيون كقائمة منسدلة في أعلى الجدول المحوري المعروض في Excel، وتحديد أحد عناصر الصفحة المتاحة يعيد بناء جسم الجدول المحوري بحيث يتم تلخيص السجلات التي تنتمي إلى عنصر الصفحة هذا فقط. يصبح حقل المحور حقل صفحة عند تسجيله كـ `PivotFieldType.Page` بدلاً من `PivotFieldType.Row` أو `PivotFieldType.Column` أو `PivotFieldType.Data`.

يمكن أن يعمل حقل الصفحة بسلوكين. في السلوك الافتراضي **تحديد واحد**، يكون عنصر صفحة واحد فقط مرئيًا في كل مرة، لذلك يلخص جسم الجدول المحوري مجموعة فرعية واحدة بالضبط. في سلوك **التحديد المتعدد**، يعرض الحقل قائمة مربعات اختيار، ويلخص جسم الجدول المحوري اتحاد كل عناصر الصفحة المحددة. يمكن نقل نفس حقل المصدر ذهابًا وإيابًا بين هذه السلوكيات عن طريق تبديل خاصية واحدة.

يوفر Aspose.Cells for C++ طريقتين متكافئتين لتسجيل حقل صفحة. واجهة برمجة التطبيقات عالية المستوى هي `PivotTable.AddFieldToArea(PivotFieldType.Page, "fieldName")`، التي تأخذ اسم عمود المصدر وتضيف الحقل في استدعاء واحد. واجهة برمجة التطبيقات منخفضة المستوى هي `PivotTable.PageFields.Add(PivotField)`، التي تُستخدم عندما يكون لديك بالفعل مرجع `PivotField` وترغب في إضافة نفس مثيل الحقل إلى منطقة الصفحة. تنتهي كلتا واجهتي البرمجة إلى ملء نفس مجموعة `PageFields`، وتوضح بقية هذه المقالة كيفية الاختيار بينهما وكيفية تشغيل كل وضع تصفية.

## **إضافة حقل صفحة**

توجد طريقتان لتسجيل حقل محور في منطقة الصفحة. يأخذ الاستدعاء عالي المستوى اسم عمود المصدر كسلسلة وهو المسار الأكثر شيوعًا. يقبل الاستدعاء منخفض المستوى مثيل `PivotField` موجودًا وهو مناسب عندما يجب إعادة استخدام نفس كائن الحقل عبر مناطق محورية متعددة. يضع كلا الاستدعاءين الحقل في `PivotTable.PageFields`، وبعد ذلك يظهر كقائمة منسدلة للصفحة في أعلى الجدول المحوري المعروض.

### إضافة حقل صفحة باستخدام AddFieldToArea

يبني المثال التالي مجموعة بيانات صغيرة من الفاكهة / السنة / المبلغ، ويضع جدولاً محوريًا في الخلية E3 مع `Fruit` في منطقة الصفوف، و`Amount` في منطقة البيانات، و`Year` في منطقة الصفحة، ويُحدّث الجدول المحوري، ويحفظ المصنف.
...
عندما تعمل بالفعل مع مثيل `PivotField`، يمكنك تمريره مباشرة إلى `PivotTable.PageFields.Add`. يتم إنشاء الجدول المحوري وحقل الصفحة تمامًا كما في السيناريو السابق؛ يتم استبدال تسجيل منطقة الصفحة النهائي فقط باستدعاء واجهة البرمجة منخفضة المستوى.
...
تعيين `CurrentPageItem` إلى فهرس حقيقي يختار عنصر الصفحة الواحد فقط. الفهرس هو موقع العنصر في قائمة العناصر المرتبة لحقل الصفحة، فعلى سبيل المثال يحدد `1` العنصر الثاني بعد الفرز.
