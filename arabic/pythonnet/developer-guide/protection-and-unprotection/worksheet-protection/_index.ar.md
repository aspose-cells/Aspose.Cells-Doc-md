---
title: حماية وإلغاء الحماية لورق العمل
type: docs
weight: 40
url: /ar/python-net/protect-and-unprotect-worksheets/
description: حماية وإلغاء حماية ورقة العمل لملفات إكسل باستخدام Aspose.Cells لبايثون via .NET.
---


{{% alert color="primary" %}}
لمنع مستخدمين آخرين من تغيير البيانات في ورقة العمل عن طريق الخطأ أو بشكل متعمد، يمكنك قفل الخلايا في ورقة العمل الخاصة بك ثم حماية الورقة بكلمة مرور. 
{{% /alert %}}


## **حماية وإلغاء حماية ورقة العمل في MS Excel**

**![حماية وإلغاء حماية ورقة العمل](protect-and-unprotect-worksheet.png)**

١. انقر فوق **مراجعة > حماية ورقة عمل**.
١. أدخل كلمة مرور في **مربع الكلمة السرية**.
١. حدد خيارات **السماح**.
١. حدد **موافق**, أدخل كلمة المرور مرة أخرى لتأكيدها، ثم حدد **موافق** مجددًا.


## **حماية ورقة العمل باستخدام Aspose.Cell for .Net**
متطلبات الترميز البسيطة التالية فقط لتنفيذ حماية هيكل دفتر العمل لملفات Excel.

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "Protection-and-unprotection-Protect-Worksheet.py" >}}

## **إلغاء حماية ورقة العمل باستخدام Aspose.Cell for .Net**
سهولة إلغاء حماية ورقة العمل باستخدام Aspose.Cells لبايثون via .NET. إذا كانت ورقة العمل محمية بكلمة مرور، يجب توفير كلمة المرور الصحيحة.

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "Protection-and-unprotection-Unprotect-Worksheet.py" >}}

## **مواضيع متقدمة**
- [إعدادات الحماية المتقدمة منذ Excel XP](/cells/ar/python-net/advanced-protection-settings-since-excel-xp/)
- [الكشف عما إذا كانت ورقة العمل محمية بكلمة مرور](/cells/ar/python-net/detect-if-worksheet-is-password-protected/)
- [حماية ورق العمل](/cells/ar/python-net/protecting-worksheets/)
- [إلغاء حماية ورقة العمل](/cells/ar/python-net/unprotect-a-worksheet/)
- [التحقق من الكلمة المستخدمة لحماية ورقة العمل](/cells/ar/python-net/verify-password-used-to-protect-the-worksheet/)
{{< app/cells/assistant language="csharp" >}}
