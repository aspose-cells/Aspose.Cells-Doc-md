...
قبل Aspose.Cells for Python via .NET v26.7، كانت الطريقة القياسية لتحديث جدول محوري هي استدعاء `PivotTable.refresh_data()` على كل جدول محوري على حدة. اعتبارًا من الإصدار 26.7، تم وضع علامة على هذه الطريقة كـ **مهجورة** ويجب استبدالها بواجهات برمجة التطبيقات الواعية بالتخزين المؤقت الموضحة أعلاه.
