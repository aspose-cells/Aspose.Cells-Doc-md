...
| `isCollaborative` | ما إذا كان يدعم الوضع التعاوني. | `false` | نعم |
...
| `token` | تعيين رمز المصادقة. عندما يكون الرمز غير null، سيتم تلقائيًا إضافة رأس `Authorization: Bearer {token}` إلى رؤوس الطلبات. يمكنك استخدام `xs.refreshToken(token)` لضبط رمز جديد. | لا شيء | نعم |    
| `showBottombarStats` | ما إذا كان يجب عرض إحصائيات الشريط السفلي.<br>القيمة الافتراضية هي true. | `true` | نعم |   
| `showRowAppenderToolbar` | ما إذا كان يجب عرض شريط أدوات ملقم الصف الجماعي.<br>القيمة الافتراضية هي true. | `true` | نعم |   
...
```
- تعيين استدعاء الوظيفة قبل الحفظ 
```javascript
...
- إعداد الإعدادات التعاونية في الوضع التعاوني، تأكد من إعداد إعدادات التعاون قبل تعيين المعرف الفريد  
```javascript
...
- تعيين النطاق القابل للتعديل/قراء-only
```javascript
...
- تعيين نمط للمنطقة المطلوبة من الخلايا
```javascript
...
- حذف محتوى الخلية أو مسح النمط في المنطقة المطلوبة من الخلايا
```javascript
...
- تطبيق إعدادات الخط لكائن مربع النص
```javascript
