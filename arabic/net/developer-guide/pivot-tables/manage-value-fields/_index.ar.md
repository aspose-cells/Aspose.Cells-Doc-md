...
يوضح هذا السيناريو كيفية وضع حقل أساسي واحد (`Amount`) في منطقة البيانات لجدول محوري موجود. تضع بنية الجدول المحوري المشتركة `Category` و`Item` على محور الصفوف و`Year` على محور الأعمدة. بعد العملية، يظهر `Amount` في منطقة البيانات ويتم حسابه كـ `Sum` لـ `Amount` بشكل افتراضي.
```csharp
using System;
using Aspose.Cells;
...
يبدأ هذا السيناريو من نفس بنية الجدول المحوري كالسيناريو 1 ولكنه يضيف حقل `Amount` إلى منطقة البيانات مرتين. كلا حقلي البيانات يشيران إلى نفس عمود المصدر، ومع ذلك يتم تجاوز الحقل الثاني باستخدام مُعيّن `PivotField.Function` بحيث يصبح `Count` بدلاً من `Sum` الافتراضي.
```csharp
using System;
using Aspose.Cells;
using Aspose.Cells.Pivot;
...
مع وجود حقلين من البيانات في مكانهما، يصبح `PivotTable.ValuesField` قابلاً للاستخدام. يسحب هذا السيناريو حقل التجميع الافتراضي هذا إلى منطقة الأعمدة بحيث يظهر كل مقياس في منطقة البيانات ككتلة عمود خاصة به بجانب `Year`.
```csharp
using System;
using Aspose.Cells;
using Aspose.Cells.Pivot;
