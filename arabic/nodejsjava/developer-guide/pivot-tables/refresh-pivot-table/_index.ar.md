...
يوفر Aspose.Cells واجهة برمجة تطبيقات للتحديث متعدد الطبقات تتيح لك إعادة تحميل بيانات الجدول المحوري على أربعة نطاقات مختلفة — من المصنف بالكامل وصولاً إلى جدول محوري واحد. بدءًا من الإصدار **Aspose.Cells for Aspose.Cells for Node.js via Java v26.7**، تم وضع علامة على الطريقة القديمة `PivotTable.RefreshData()` باعتبارها مهملة ويجب استبدالها بواجهات برمجة التطبيقات الأكثر كفاءة والمدركة لذاكرة التخزين المؤقت الموضحة في هذه المقالة.
...
قبل Aspose.Cells for Aspose.Cells for Node.js via Java v26.7، كانت الطريقة القياسية لتحديث جدول محوري هي استدعاء `PivotTable.RefreshData()` على كل جدول محوري على حدة. اعتبارًا من الإصدار v26.7، تم وضع علامة على هذه الطريقة باعتبارها **مهملة** ويجب استبدالها بواجهات برمجة التطبيقات المدركة لذاكرة التخزين المؤقت الموضحة أعلاه.
