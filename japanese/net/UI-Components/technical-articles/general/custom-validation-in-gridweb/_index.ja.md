---
title: GridWebでのカスタムバリデーション
type: docs
weight: 90
url: /ja/net/aspose-cells-gridweb/custom-validation-in-gridweb/
keywords: GridWeb,validation,custom,server
description: この記事では、GridWebでのカスタムバリデーションを紹介します。

---


## **可能な使用シナリオ**
Aspose.Cells.GridWebは、通常のセルバリデーションAPIで定義されているものよりも多くのバリデーションを提供します。
...
以下に、GridWebで使用できる拡張バリデーションを示します。
...
result:
無効な値を入力した後
![todo:バリデーション結果](gridvalidation_customexpression.png)  
...
result:
無効な値を入力した後 
![todo:バリデーション結果](gridvalidation_bool.png)  

trueの値を入力した後 
![todo:バリデーション結果](gridvalidation_bool2.png)  
