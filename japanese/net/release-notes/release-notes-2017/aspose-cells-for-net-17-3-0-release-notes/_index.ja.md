---
title: Aspose.Cells for .NET 17.3.0 リリースノート
type: docs
weight: 100
url: /ja/net/aspose-cells-for-net-17-3-0-release-notes/
---
{{% alert color="primary" %}} 

このページには、[Aspose.Cells for .NET 17.3.0](https://downloads.aspose.com/cells/net/new-releases/aspose.cells-for-.net-17.3.0/).
...
|CELLSNET-45143|Aspose.Cells XLSB に保存すると、レポート フィルターを使用して破損したピボット テーブルが生成される|バグ|
|CELLSNET-45135|HTML を XLSX としてインポート中に画像が重なる|バグ|
|CELLSNET-45134|HTML を XLSX としてインポート中に列幅が失われる|バグ|
|CELLSNET-45133|HTML を XLSX としてインポートすると、コンテンツのスタイルが失われる|バグ|
|CELLSNET-45132|XLSX 形式で保存すると、HTML で有効化された WrapText が失われる|バグ|
|CELLSNET-45127|HTML を XLSX としてインポートした後、フォント スタイルとサイズが変更される|バグ|
...
|CELLSNET-45076|Excel から HTML への変換で配置が機能せず、テキストがセルからはみ出す|バグ|
|CELLSNET-45057|HTML から XLSX への変換が正しくない|バグ|
|CELLSNET-45040|一部のピボット フィールド アイテムは、数値ではなく日付値としてレンダリングされます|バグ|
...
|CELLSNET-45151|スプレッドシートを PDF にレンダリングする際に、チャートの書式設定がすべて失われます|バグ|
|CELLSNET-45147|スプレッドシートを PDF にレンダリング中にグラフが見つからない|バグ|
|CELLSNET-44998|スプレッドシートを PDF に変換すると、グラフのデータ ラベルが変更される|バグ|
|CELLSNET-44984|番目の図の傾向線が、出力 PDF で適切にレンダリングされない|バグ|
|CELLSNET-41539|緑色の楕円形のワード アート/画像が、PDF のグラフの周りに正しく表示されない|バグ|
|CELLSNET-45197|Range.MoveTo はデータ検証と条件付き書式を移動しません|バグ|
|CELLSNET-45183|ActiveX コントロール (ComboBox) の "Locked" 属性は変更されません|バグ|
|CELLSNET-45179|XLSX ファイルの CodeName の設定が正常に機能しない|バグ|
|CELLSNET-45175|XLSX ファイルで画像の書式設定が機能しない|バグ|
|CELLSNET-45174|DeleteRange 後に Print_Area 範囲が壊れる|バグ|
|CELLSNET-45173|保存後に XLSX ファイルが破損する|バグ|
...
|CELLSNET-43722|ファイル エラー: データが失われた可能性があります。XLS を開くときに Excel によってスローされました。|バグ|
|CELLSNET-45190|WORKDAY 数式の計算中に例外が発生しました|例外|
|CELLSNET-45199|Excel から PDF - 配列例外からのインデックス|例外|
...
1. [ピボット テーブルのグローバリゼーション設定のカスタマイズ](/cells/ja/net/customize-globalization-settings-for-pivot-table/)
1. [GridWebのページ変更時にクライアントサイド関数を実行する](/cells/ja/net/execute-client-side-function-on-gridweb-page-change/)[](/cells/ja/net/execute-client-side-function-on-gridweb-page-change/)
1. [更新されたセルだけではなく、ワークシート全体を検証する](/cells/ja/net/validate-entire-worksheet-instead-of-only-the-updated-cells/)
