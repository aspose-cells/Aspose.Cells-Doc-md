...
type: docs
weight: 90
url: /ja/net/aspose-cells-for-net-17-4-0-release-notes/
---
{{% alert color="primary" %}} 

このページには、[Aspose.Cells for .NET 17.4.0](https://downloads.aspose.com/cells/net/new-releases/aspose.cells-for-.net-17.4.0/).
...
|CELLSNET-44438|CSV を PDF に保存中に OutOfMemoryException が発生する|パフォーマンス|
...
|CELLSNET-45275|ワークシートを削除すると破損した XLSM が保存される|バグ|
|CELLSNET-45262|ワークシートを HTML として保存し、セルにオーバーフローしたテキストを含む|バグ|
|CELLSNET-45248|Excel から HTML へのレンダリングで一部のテキストが重なっているか見えず、垂直方向の配置が正しくない|バグ|
|CELLSNET-45229|HTMLをXLSXで保存すると文字のレイアウトが違う|バグ|
|CELLSNET-45228|HTML を XLSX で保存するとテキスト形式が変わる|バグ|
|CELLSNET-45227|HTML を XLSX として保存すると、コンテンツの一部が欠落する|バグ|
...
|CELLSNET-45283|PDF最適化タイプを「MinimumSize」に設定すると、出力PDFファイルのフォントが太くなります|バグ|
|CELLSNET-45265|出力画像・PDFのフォント変更|バグ|
...
|CELLSNET-45188|出力 PDF のグラフのカテゴリ軸ラベルが表示されない|バグ|
...
|CELLSNET-45252|特定の XLS で再保存した後、表のスタイルが変更された (表の行が欠落している)|バグ|
...
|CELLSNET-45237|XLSX で Datatable が適切にシフトダウンされていない|バグ|
...
|CELLSNET-45220|XLS で 1 行のデータを複製するときにハイパーリンクの数が正しくない|バグ|
...
|CELLSNET-44975|Excel を PDF に変換するときの図形テキスト内のフォーマット エラー|例外|
...
- [列ヘッダーのヒントを設定する](/cells/ja/net/set-column-header-tip/)
