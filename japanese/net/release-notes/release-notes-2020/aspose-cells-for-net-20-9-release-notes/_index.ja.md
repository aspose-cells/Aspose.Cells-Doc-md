...
type: docs
weight: 8
url: /ja/net/aspose-cells-for-net-20-9-release-notes/
---
{{% alert color="primary" %}}
...
|CELLSNET-47555|グラフは、PDF として保存するときに #N/A を空のセルとして扱うことを許可しません|強化|
...
|CELLSNET-47562|エクスポートされたグリッド線設定を含む空のシートを HTML に保存する|バグ|
...
|CELLSNET-47545|カスタムの負の数が PDF に正しく表示されない|バグ|
|CELLSNET-47548|二重引用符を含むテキスト ファイルのインポートに関する問題|バグ|
|CELLSNET-47558|カスタムの負の数 (スイス地域を使用) が PDF に正しく表示されない|バグ|
...
|CELLSNET-47360|Excel ファイルのチャートの透明なマーカー ポイントが、出力 PDF で歪んでいます。|バグ|
|CELLSNET-47565|前景のフッター画像が背景になる|バグ|
|CELLSNET-46502|XLSX から TIFF への変換でブラック ボックスが発生する|バグ|
|CELLSNET-46821|ワークシートを TIFF に変換中 - 画像が黒く塗りつぶされています|バグ|
...
|CELLSNET-47574|XLS から HTML への変換で空のファイルが生成される|バグ|
