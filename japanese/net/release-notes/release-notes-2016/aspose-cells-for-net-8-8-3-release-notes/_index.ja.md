---
title: Aspose.Cells for .NET 8.8.3 リリースノート
type: docs
weight: 80
url: /ja/net/aspose-cells-for-net-8-8-3-release-notes/
---
### **1) Aspose.Cells**

|**鍵** |**概要** |**カテゴリー** |
|:- |:- |:- |
|CELLSNET-44175 |計算項目を含むピボット テーブルの更新と計算をサポート|新機能|
|CELLSNET-44535 |トグル ボタンを含む ActiveX コントロールを追加/操作するための関連 API を公開する|新機能|
...
|CELLSNET-44523 |スマート マーカーでの配列数式の使用|強化|
|CELLSNET-44509 |insertRows=true を指定した ImportDataTable は、最初の行を空白行に置き換えます|強化|
|CELLSNET-44480 |テーブル スタイルをリッチ テキスト設定に適用する|強化|
|CELLSNET-44512 |Cells.CopyRows() メソッドのパフォーマンスの問題|パフォーマンス|
...
|CELLSNET-44508 |Excel とは異なる出力 PDF のページ サイズ|バグ|
|CELLSNET-44507 |Shape.GetCharacters() メソッドの問題|バグ|
...
|**鍵** |**概要** |**カテゴリー** |
|:- |:- |:- |
|CELLSNET-44533 |GridWeb が一時セッション ファイルを保存するパスを指定する方法を提供する|新機能|
|CELLSNET-44497 |範囲を選択するため、または JavaScript を使用して選択された範囲を取得するためのクライアント側関数を提供する|新機能|
|CELLSNET-44483 |コントロール内およびコントロールと Excel 間で Aspose.Cells.GridDesktop の行のコピー/貼り付けをサポート|新機能|
|CELLSNET-44499 |GridWeb.ShowHeaderBar を false に設定したことが原因で、未定義または null 参照のプロパティ 'style' を取得できません|バグ|
|CELLSNET-44524 |Gridweb で大きな Excel ファイルを処理する|バグ|
|CELLSNET-44440 |GridWeb のドロップダウンには 4 つのエントリが表示されますが、ドイツ語ロケールには 2 つのエントリがあります|バグ|
|CELLSNET-44506 |GridWeb.ImportExcelFile での InvalidFormula 例外|例外|
...
#### **GridWeb.SessionStorePath プロパティを追加します**
セッション モードが ViewState の場合、セッション ストア パスを取得または設定します。これは、現在の Web アプリケーションのベース ディレクトリへの相対パスです。
