...
type: docs
weight: 50
url: /ja/net/aspose-cells-for-net-8-9-2-release-notes/
---
### **1) Aspose.Cells**
...
|CELLSNET-44630 |DataGrid からデータをインポートすると、数値の書式設定 (DateTime) が失われる (.NET)|強化|
|CELLSNET-44626 | CopyRows でコピーされたチャートは、チャート データ範囲のソース ワークシートを引き続き参照します。|強化|
|CELLSNET-44619 |日付文字列を日付形式に変換|強化|
|CELLSNET-44625 |Colspan as 2 を使用すると、HTML の読み込みと変換にかかる時間が大幅に増加します|パフォーマンス|
|CELLSNET-44338 |スプレッドシートを PDF にレンダリングする際のパフォーマンスの低下|パフォーマンス|
|CELLSNET-44610 |HTML を XLSX に変換する際にボールド属性が失われる|バグ|
|CELLSNET-44609 |HTML から XLSX への変換中にコンテンツの方向が変更される|バグ|
|CELLSNET-44608 |HTML を XLSX に変換中に枠線が表示されない|バグ|
...
|CELLSNET-44579 |MHTML から XLS/XLSX への変換が空白のドキュメントとして戻ってくる|バグ|
|CELLSNET-44572 |同じドキュメント内のワークシートを参照するハイパーリンクに関する問題|バグ|
|CELLSNET-44617 |SumProduct 式が正しく計算されない|バグ|
|CELLSNET-44604 |XLS から Pdf/A への変換後に失われたテキスト|バグ|
|CELLSNET-44603 |スプレッドシートを PDF にレンダリングする際に、セルのコンテンツが上部の境界線に接触することによる垂直方向の配置の問題|バグ|
...
|CELLSNET-44635 | sample2.xlsx のグラフが正しくない (Excel から PDF への変換)|バグ|
...
|CELLSNET-44659 |XLS ファイルの読み込みと保存後に色が変更される|バグ|
