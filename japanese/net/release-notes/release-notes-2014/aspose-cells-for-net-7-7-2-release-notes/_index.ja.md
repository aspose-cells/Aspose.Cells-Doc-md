---
title: Aspose.Cells for .NET 7.7.2 リリースノート
type: docs
weight: 100
url: /ja/net/aspose-cells-for-net-7-7-2-release-notes/
---
{{% alert color="primary" %}} 

このページには、[Aspose.Cells for .NET 7.7.2](https://downloads.aspose.com/cells/net/new-releases/aspose.cells-for-.net-7.7.2/)
...
パスワードで保護された MS Excel 2013 ファイルのオープンをサポート
...
(CELLSNET-42299) - B3 (13.9 x 19.7 インチ) PaperSizeType をサポート

(CELLSNET-42370) - Justify Distributed Text Alignment 設定を適用する

(CELLSNET-42312) - RemovePersonalInformation 設定の取得
...
(CELLSNET-40847) - MS Excel シートを HTML として保存する際の問題

(CELLSNET-40161) - Excel から HTML - 改行なし
...
(CELLSNET-42319) - 数式計算の不一致

(CELLSNET-42318) - 行の削除時に条件付き書式が失われる

(CELLSNET-42295) - XLSM から PDF: 数式フィールドが #DIV/0 として表示される!

 (CELLSNET-42294) - SheetRender は ComboBoxes で選択された値を考慮しません

(CELLSNET-42284) - VLOOKUP 数式が Aspose.Cells で解決されない

(CELLSNET-42283) - MS Excel XLT 形式のサポート

(CELLSNET-42282) - Excel ファイルの読み込み - ブラウザの表示オプションがリセットされる

(CELLSNET-42276) - Workbook.Copy() メソッドの使用時にスタイル (親スタイル) がコピーされない


## **例外**


(CELLSNET-42345) - Excel 2013 ファイルの読み込み中に NullReferenceException が発生する

(CELLSNET-42330) - Workbook コンストラクターでの ArgumentOutOfRangeException

 (CELLSNET-42362) - 保存時の Null 参照例外

(CELLSNET-42350) - ArgumentException: パラメータが無効です

(CELLSNET-42348) - ワークブックを SaveFormat.Xlsx で MemoryStream に保存中に NullReferenceException

 (CELLSNET-42333) - Cells.ExportDataTableAsString での System.ArgumentException

 (CELLSNET-42290) - Excel ファイルに対して画像が生成されず、null 参照例外がスローされる

(CELLSNET-42268) - インデックスが配列エラーの範囲外でした

(CELLSNET-42311) - ArgumentNullException: バッファを null にすることはできません。パラメータ名: Workbook.Save のバッファ
...
WorkbookSettings.RemovePersonalInformation プロパティを追加します

指定したブックから個人情報を削除できるかどうかを示します。
 ...
 Style.IsJustifyDistributed プロパティを追加します。

テキストの最終行でセルを両端揃えにするか分散配置にするかを示します。
