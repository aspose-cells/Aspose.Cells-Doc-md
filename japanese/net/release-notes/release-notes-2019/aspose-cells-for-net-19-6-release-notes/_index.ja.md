...
type: docs
weight: 70
url: /ja/net/aspose-cells-for-net-19-6-release-notes/
---
{{% alert color="primary" %}} 
...
|CELLSNET-46755|オブジェクト ファイルがチャートまたは ODS の oleobject であるかどうかを検出します。|強化|
...
|CELLSNET-46747|PDF へのレンダリング中に埋め込みオブジェクトの上にグリッド線が印刷される|バグ|
|CELLSNET-41479|生成された PDF のスライサー設定|バグ|
|CELLSNET-41783|スライサーを含むテンプレート ファイルから生成されたファイルを修正する必要がある|バグ|
|CELLSNET-46733|ピボット テーブルを HTML として保存中にスタイル/フォーマットが失われる|バグ|
...
|CELLSNET-46592|XLSX から PDF への変換中のテキスト レンダリングの問題|バグ|
|CELLSNET-46735|出力 PDF の各シートでページ番号が 1 にならない|バグ|
...
|CELLSNET-46752|InsertCutCells() を呼び出した後、出力 XLSX ファイルが破損する|バグ|
...
|CELLSNET-46510|形状から画像へのエラー! XLSX を PDF に変換する場合|例外|
