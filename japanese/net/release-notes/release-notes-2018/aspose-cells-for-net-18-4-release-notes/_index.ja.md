---
title: Aspose.Cells for .NET 18.4 リリースノート
type: docs
weight: 90
url: /ja/net/aspose-cells-for-net-18-4-release-notes/
---
{{% alert color="primary" %}} 

このページには、[Aspose.Cells for .NET 18.4](https://www.nuget.org/packages/Aspose.Cells/18.4.0).

{{% /alert %}} 

|**鍵**|**概要**|**カテゴリー**|
|:- |:- |:- |
|CELLSNET-46045|Chart.ToPdf メソッドの使用中に Pdf ページ サイズを設定する|新機能|
|CELLSNET-45590|ヒストグラム MS Excel 2016 チャートのレンダリングをサポート|新機能|
|CELLSNET-46007|MS Excel ワークシート オブジェクト (VBA) の「FilterMode」プロパティに同等のプロパティを追加します。|新機能|
|CELLSNET-46026|cellModifiedOnAjax で追加のセル変更をサポート - Aspose.Cells.GridWeb|新機能|
|CELLSNET-46013|HTML として保存するときにオーバーレイされたコンテンツを非表示にする新しいクロス タイプ|強化|
|CELLSNET-45965|後方変換中に標準の LINK 要素を処理する機能|強化|
|CELLSNET-46032|Excel ファイルが空の場合、単一ページの空白の PDF を生成しない|強化|
|CELLSNET-46027|Excel から PDF へのレンダリング - ヘッダー/フッターの問題|強化|
|CELLSNET-45970|Aspose.Cells 列を自動調整する場合、セルがテキストで折り返されている場合に行の高さを考慮しない|強化|
|CELLSNET-44985|テキストの折り返しをオンにした場合の列の自動調整操作に関する問題|強化|
|CELLSNET-42701|AutoFitColumns の折り返しテキストの問題|強化|
|CELLSNET-46005|出力 PDF ファイル形式でシートごとに反転した書き込み|バグ|
|CELLSNET-45958|XLSX を HTML として保存するときのフォーマットが正しくない|バグ|
|CELLSNET-45907|チャートのレンダリングにおける欠損値|バグ|
|CELLSNET-46034|XLS ファイル形式から (データ ソースが外部の) ピボットテーブルを削除できません|バグ|
|CELLSNET-46016|ピボットテーブルの更新後に Excel ファイルが破損する|バグ|
|CELLSNET-45988|「Sample2.xlsx」のピボット テーブルを更新すると、破損した Excel ファイルが生成される|バグ|
|CELLSNET-46011|Workbook.Calculation がセル F155 に間違った値を与える|バグ|
|CELLSNET-46001|DateTime 関数を計算するときの DateTime 値の間違った評価|バグ|
|CELLSNET-46000|セルに合わせて縮小すると、レンダリングされたイメージでテキストが通常よりわずかに小さくなります|バグ|
|CELLSNET-45998|すべてのマージンがゼロに設定され、OnePagePerSheet が true に設定されている場合でも、マージンは存在します。|バグ|
|CELLSNET-45990|PDF 出力は最適化タイプによって異なります|バグ|
|CELLSNET-46053|テンプレート ファイルでグラフを計算するときに「入力文字列が正しい形式ではありませんでした」|バグ|
|CELLSNET-46029|カスタム データ フィルタリングに関する問題|バグ|
|CELLSNET-46024|保存中にスラッシュをバックスラッシュに変更した OriginalDataSource|バグ|
|CELLSNET-46018|OTS ファイルの保存時に画像と図が表示されない|バグ|
|CELLSNET-46003|ActiveX ComboBox の ListFillRange が更新されない|バグ|
|CELLSNET-46002|ページ ヘッダー行は、出力 PDF の最初のページにのみ表示されます。|バグ|
