...
type: docs
weight: 10
url: /ja/net/how-to-format-number-to-accounting/
description: この記事では、Aspose.Cells for .NETAPIを使用して数値を会計に書式設定する方法を紹介します。
...
## ** 数値を会計フォーマットに設定する方法（Aspose.Cells for .NET）**
 Aspose.Cells for .NETで数値を会計フォーマットに設定するには、セルまたはセル範囲に関連付けられた`Style`オブジェクトを使用できます。`Style`オブジェクトでは、さまざまな書式設定オプション（数値フォーマットを含む）を設定可能です。会計フォーマットは、通貨記号や小数点以下の桁数など、具体的な要件に応じて異なるフォーマットコードを持つことがあります。

Aspose.Cells for .NETで会計番号フォーマットを適用する基本的な例は次のとおりです:
