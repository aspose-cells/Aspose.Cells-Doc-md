---
title: 動的配列式の設定
description: Aspose.Cellsライブラリを使用してMicrosoft Excelで動的配列式を計算する方法。既存のExcelファイルを読み込むか新しいExcelファイルを作成し、Aspose.Cellsで提供されるメソッドを使用して動的配列式を計算し、結果を得ます。最後に、変更されたExcelファイルをディスクに保存します。
...
ダミーデータを含む[サンプルExcelファイル](dynamicArrayFormula.xlsx)をロードする以下のサンプルコードをご覧ください。ファイルをロードした後、[Cell.SetDynamicArrayFormula](https://reference.aspose.com/cells/net/aspose.cells/cell/setdynamicarrayformula/#setdynamicarrayformula)関数を呼び出して動的配列式を設定し、[Workbook.RefreshDynamicArrayFormulas](https://reference.aspose.com/cells/net/aspose.cells/workbook/refreshdynamicarrayformulas/#refreshdynamicarrayformulas)関数を呼び出して動的配列式を更新し、最後にワークブックを [出力Excelファイル](out.xlsx) として保存します。 
