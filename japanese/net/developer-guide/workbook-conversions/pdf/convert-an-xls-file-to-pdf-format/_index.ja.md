...
スプレッドシートに数式が含まれている場合は、スプレッドシートを PDF にレンダリングする直前に Workbook.CalculateFormula() を呼び出すことをお勧めします。これにより、数式に依存する値が再計算され、正しい値が PDF にレンダリングされます。
