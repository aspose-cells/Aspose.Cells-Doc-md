...
**Aspose.Cells** Excel ファイルから PDF への変換をサポートし、変換時に高い視覚的忠実度を維持します。

Aspose.Cells for .NET は、他のソフトウェアとは独立してスプレッドシートから PDF への変換をサポートします。 Workbook クラスの Save メソッドを使用して、Excel ファイルを PDF に保存します。 Save メソッドは、ネイティブ Excel ファイルを PDF 形式に変換する SaveFormat.Pdf 列挙メンバーを提供します。

**変換中**サードパーティのツールまたは外部の API を使用する代わりに、スプレッドシートから直接 PDF に変換するには、いくつかの**利点**:
...
**MHTML**通常の HTML と外部リソース (つまり、画像、アニメーション、音声など、通常はリンクされるコンテンツ) を 1 つのファイルに結合します。これらは、ファイル拡張子が .mht の電子メールに使用されます。
