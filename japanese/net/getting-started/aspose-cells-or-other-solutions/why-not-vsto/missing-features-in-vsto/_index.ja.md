---
title: VSTOでの欠落している機能
type: docs
weight: 20
url: /ja/net/missing-features-in-vsto/
---

このセクションには以下のトピックがあります。

- [Aspose.Cellsでデータをリンクするためのハイパーリンクの追加](/cells/ja/net/adding-hyperlinks-to-link-data-in-aspose-cells/)
- [Excel XP以降の高度な保護設定](/cells/ja/net/advanced-protection-settings-since-excel-xp-in-aspose-cells/)
- [変換](/cells/ja/net/conversion/)
- [ワークシートの変換](/cells/ja/net/converting-worksheet/)
  - [ページごとの画像ファイルにワークシートを変換](/cells/ja/net/converting-worksheet-to-image-file-by-page/)
  - [Aspose.Cellsでワークシートを画像に変換](/cells/ja/net/converting-worksheet-to-image-in-aspose-cells/)
- [Aspose.Cellsでグリッド線を表示または非表示にする](/cells/ja/net/display-or-hide-gridlines-in-aspose-cells/)
- [Aspose.Cells で行列ヘッダーを表示または非表示にする](/cells/ja/net/display-or-hide-row-column-headers-in-aspose-cells/)
- [Aspose.Cells でスクロールバーを表示または非表示にする](/cells/ja/net/display-or-hide-scroll-bars-in-aspose-cells/)
- [Aspose.Cells でタブを表示または非表示にする](/cells/ja/net/display-or-hide-tabs-in-aspose-cells/)
- [Aspose.CellsでのExcelファイルの暗号化](/cells/ja/net/encrypting-excel-files-in-aspose-cells/)
- [ワークシートのインポートおよびエクスポート](/cells/ja/net/import-and-export-worksheet/)
  - [ドキュメントからのデータのエクスポート](/cells/ja/net/export-data-from-document/)
    - [非型指定データを含む列](/cells/ja/net/columns-containing-non-strongly-typed-data/)
    - [型指定データを含む列](/cells/ja/net/columns-containing-strongly-typed-data/)
  - [ドキュメントからのデータのインポート](/cells/ja/net/import-data-from-document/)
    - [配列からのインポート](/cells/ja/net/importing-from-array/)
    - [ArrayListからのインポート](/cells/ja/net/importing-from-arraylist/)
    - [カスタムオブジェクトからのインポート](/cells/ja/net/importing-from-custom-objects/)
    - [DataTableからのインポート](/cells/ja/net/importing-from-datatable/)
- [Aspose.Cells でワークブックをテキストまたは CSV 形式で保存する](/cells/ja/net/save-workbook-to-text-or-csv-format-in-aspose-cells/)
- [Aspose.Cellsのスマートマーカー](/cells/ja/net/smart-markers-in-aspose-cells/)
  - [データのグループ化](/cells/ja/net/grouping-data/)
  - [イメージマーカー](/cells/ja/net/image-markers/)
  - [Smart Markers 機能](/cells/ja/net/smart-markers-feature/)
  - [匿名型またはカスタムオブジェクトの使用](/cells/ja/net/using-anonymous-types-or-custom-objects/)
  - [ネストされたオブジェクトの使用](/cells/ja/net/using-nested-objects/)
- [Aspose.Cells で先行および従属をトレースする](/cells/ja/net/tracing-precedents-and-dependents-in-aspose-cells/)
