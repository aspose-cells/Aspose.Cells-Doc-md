...
type: docs
weight: 600
url: /ja/javascript-cpp/how-to-manage-dates-and-times/
description: 日付と時刻をC++ APIを使用して管理する方法を学習します。
keywords: 日付と時間の管理方法、1900年の日付システム、1904年の日付システム、日付と時間の設定、日付と時間の取得、日付と時間の管理、Excelでの日付と時間の格納、セルでの日付と時間の表示。
---

## **Excelでの日付と時間の格納方法**  
日付と時刻はセル内に数値として格納されます。したがって、日付や時刻を含むセルの値は数値型です。日付と時刻を指定する数値は、日付（整数部分）と時刻（小数部分）から構成されています。Cell.doubleValueプロパティは、この数値を返します。  

## **Aspose.Cellsでの日付と時間の表示方法**  
数値を日付と時刻として表示するには、Style.numberまたはStyle.Customプロパティを使用してセルに必要な日付と時刻の形式を適用します。CellValue.dateTimeValueプロパティは、この数値で表される日付と時刻を示すDateTimeオブジェクトを返します。  
<br>  
...
1. 1904年日付システム。最初の日付は1904年1月1日で、そのシリアル値は0です。最後の日付は9999年12月31日で、そのシリアル値は2,957,003です。この日付システムをワークブックで使用するには、WorkbookSettings.date1904をtrueに設定します。  
