...
type: docs
weight: 140
url: /ja/javascript-cpp/add-image-hyperlinks/
description: Aspose.Cells for JavaScript経由でC++ APIを使った画像ハイパーリンクの追加方法を学びます。
keywords: JavaScriptを使った画像ハイパーリンクの追加、C++を使った画像ハイパーリンクの挿入。
---

{{% alert color="primary" %}} 

ハイパーリンクは他のワークシートやウェブサイトの情報にアクセスするのに便利です。Microsoft Excelではセルのテキストや画像にハイパーリンクを追加できます。画像のハイパーリンクは、ナビゲーションを容易にし、次・前ボタンや特定のサイトにリンクするロゴなどに活用できます。本書では、Aspose.Cells for JavaScriptを使ってワークシートに画像ハイパーリンクを挿入する方法を説明します。

{{% /alert %}} 

Aspose.Cells for JavaScript経由でC++によりスプレッドシートの画像にハイパーリンクを追加できます。リンクのスクリーンチップやアドレスの設定・変更も可能です。以下のサンプルコードは、ワークシートに画像ハイパーリンクを追加する方法を示しています。
