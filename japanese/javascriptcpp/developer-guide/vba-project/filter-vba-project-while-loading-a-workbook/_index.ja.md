---
title: JavaScriptを通じてC++でブックの読み込み時にVBAプロジェクトをフィルタリングする
linktitle: ワークブックを読み込む際にVBAプロジェクトをフィルタリングする
type: docs
weight: 140
url: /ja/javascript-cpp/filter-vba-project-while-loading-a-workbook/
description: Excelワークブックを読み込む際にAspose.Cells for JavaScriptを使用してVBAプロジェクトをフィルタリングする方法を学びます。
---

## **JavaScriptを介してC++でExcelワークブックの読み込み時にVBAプロジェクトをフィルタリングする**

一部の.xlsm/.xslbファイルには非常に多くのマクロ（または非常に長いマクロ）が含まれています。Aspose.Cells for JavaScriptをC++で利用すると、これらのワークブックを開くときにこの（メタ）データを無条件に読み込みます。必要に応じてコントロールする必要があるかもしれません [**LoadDataFilterOptions**](https://reference.aspose.com/cells/javascript-cpp/loaddatafilteroptions)。たとえば、大量のワークブックからシート名だけを抽出したい場合、不要な内容をスキップできます。このフィルタは新しいオプション [**LoadDataFilterOptions.VBA**](https://reference.aspose.com/cells/javascript-cpp/loaddatafilteroptions) を導入することで提供されます。
