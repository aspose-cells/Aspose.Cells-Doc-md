---
title: JavaScriptを使って日本地域用のチャートを画像に変換（C++経由）
description: Aspose.Cells for JavaScriptを使って、チャートの日本語設定を行う方法を学びます。このガイドでは、フォント、サイズ、テキスト方向など、日本語に対応したチャートの設定方法を詳しく解説します。  
keywords: Aspose.Cells for JavaScriptを使ったC++、チャート、日本語設定、フォント、フォントサイズ、テキスト方向、サポート。  
linktitle: 日本語地域の設定  
type: docs  
weight: 10  
url: /ja/javascript-cpp/convert-chart-to-image-for-japanese-region/  
...
最初のステップでは、[**ChartGlobalizationSettings**](https://reference.aspose.com/cells/javascript-cpp/chartglobalizationsettings/)から継承する"ChartJapaneseSettings"クラスを定義する必要があります。  
