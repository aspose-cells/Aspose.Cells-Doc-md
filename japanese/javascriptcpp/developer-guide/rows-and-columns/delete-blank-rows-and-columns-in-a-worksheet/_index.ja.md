---
title: JavaScriptをC++経由で使用してワークシートの空白の行と列を削除する方法
linktitle: ワークシート内の空白の行と列を削除する
type: docs
weight: 300
url: /ja/javascript-cpp/delete-blank-rows-and-columns-in-a-worksheet/
description: Aspose.Cells for JavaScriptをC++経由で使用して、ワークシートからすべての空白の行と列を削除する方法を学びます。 
---
...
1. 空白の行を削除するには、[**Cells.deleteBlankRows()**](https://reference.aspose.com/cells/javascript-cpp/cells/#deleteBlankRows--)メソッドを使用します。削除される空白の行については、[**Row.isBlank()**](https://reference.aspose.com/cells/javascript-cpp/row/#isBlank--)がtrueであるだけでなく、それらの行のいかなるセルにも見えるコメントが定義されていないこと、そしてそれらと交差するピボットテーブルがないことも必要です。
2. 空白の列を削除するには、[**Cells.deleteBlankColumns()**](https://reference.aspose.com/cells/javascript-cpp/cells/#deleteBlankColumns--) メソッドを使用します。

{{% /alert %}}

## 空白の行を削除するJavaScriptコード
