---
title: JavaScript を C++ 経由で使用して動的配列数式を設定
linktitle: 動的配列式の設定
description: Aspose.Cells ライブラリを使用して JavaScript 経由でExcelの動的配列数式を計算する方法。読み込み、計算、保存も簡単に行えます。
keywords: 動的配列数式、Excelの動的配列数式、JavaScriptを C++ で使用して動的配列数式を設定、JavaScriptをC++で使用して動的配列数式の計算、Excelの動的配列数式の操作。
type: docs
weight: 70
url: /ja/javascript-cpp/calculation-of-dynamic-array-formulas/
---
...
次のサンプルコードでは、一部のダミーデータを含む [サンプルExcelファイル](dynamicArrayFormula.xlsx) を読み込みます。ファイルを読み込んだ後、[Cell.dynamicArrayFormula(string, FormulaParseOptions, boolean)] 関数を呼び出して動的配列数式を設定し、[Workbook.refreshDynamicArrayFormulas(boolean)] 関数を呼び出して数式の計算前に動的配列数式を更新し、最後にワークブックを [出力Excelファイル](out.xlsx) として保存します。  
