...
type: docs
weight: 10
url: /ja/java/aspose-cells-for-java-7-0-3-release-notes/
---
{{% alert color="primary" %}} 

このページには、[Aspose.Cells for Java 7.0.3](https://downloads.aspose.com/cells/java/new-releases/aspose.cells-for-java-7.0.3/)
...
40022 - オプション セットで XLS テンプレート ファイルを読み取る: LoadDataOnly=true で例外が発生する

40017 - WorksheetCollection.getNamedRanges() メソッドで例外が発生する

40033 - XLSX テンプレート ファイルの読み取り時に「無効なパスワード」という例外が発生する

バグ

31303 - Cell 文字 '"' および ',' を含む値が CSV ファイルに正しく保存されませんでした

31376 - PDF の保存に時間がかかりすぎて、非常に大きな PDF ファイルが生成されました

40001 - セル値の書式設定時に通貨記号が失われる

40003 - 生成された PDF ファイルで、結合されたセルのテキストの向きが変更される
...
40014 - シート用に生成された画像にグリッド線が表示されませんでした。
 ...
 40016 - 保存された XLS ファイルにより、MS Excel に警告メッセージ -1 が表示される

40020 - 保存された XLS ファイルにより、MS Excel に警告メッセージが表示される -2

40021 - 保存された XLS ファイルにより、MS Excel に警告メッセージが表示される -3
...
40085 - 行/列が XLS ファイルの制限を超えると、保存された XLS が次の原因になります: MS Excel で開くと、「Excel で読み取り不能なコンテンツが見つかりました」というエラーが発生する
