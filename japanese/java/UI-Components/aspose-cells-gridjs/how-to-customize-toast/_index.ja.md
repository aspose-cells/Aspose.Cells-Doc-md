---
title: GridJs におけるメッセージツイトーストのUI外観のカスタマイズ方法  
type: docs
weight: 250
url: /ja/java/aspose-cells-gridjs/how-to-customize-toast/
description: この記事は、GridJsにおけるメッセージトーストのUI外観をカスタマイズする方法について説明します。
keywords: GridJs, カスタマイズ, トースト, UI, 外観, ビジュアル, メッセージ, 警告
aliases:
aliases:
...
# GridJsのためのカスタムトーストガイド

## 概要

GridJsは柔軟なカスタムトースト通知システムを提供しており、デフォルトのトーストメッセージを独自の通知実装に置き換えることが可能です。このガイドでは、アプリケーションのニーズに応じたトースト通知のカスタマイズ方法をさまざまなアプローチで示します。
...
- [高度なユースケース](#advanced-use-cases)
  ...
  - [モバイルフレンドリートースト](#7-mobile-friendly-toast)
...
- `callback`（関数、省略可能）：トーストが閉じられた後に実行されるコールバック関数
...
- `progressBar`: カウントダウンのプログレスバーを表示
- `positionClass`: トーストの位置（右上、左下など）を制御
- `timeOut`: 自動閉鎖の持続時間（ミリ秒）
...
より良いユーザー体験のために、モーダルアラートの代わりに非ブロッキングのトースト通知を使用してください：
...
コンテンツの長さに基づいて適切な自動消去時間を選択してください：
...
スプレッドシートが壊れないように、トースト機能をtry-catchでラップしてください：
...
**問題：** 多くのトースト通知の後、ページが遅くなる。

**解決策:** DOM要素とイベントリスナーのクリーンアップ:
```javascript
const remove = () => {
  document.body.removeChild(toast);
...
**問題:** トーストの表示がモバイルデバイスで良くない。

**解決策:**
1. パーセンテージベースの幅を使用: `maxWidth: '80%'`
