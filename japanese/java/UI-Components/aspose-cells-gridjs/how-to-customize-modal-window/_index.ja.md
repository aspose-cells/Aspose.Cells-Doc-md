---
title: GridJsにおけるモーダルウィンドウのUI外観をカスタマイズする方法  
type: docs
weight: 250
url: /ja/java/aspose-cells-gridjs/how-to-customize-ui-window/
description: この記事では、GridJsにおけるモーダルウィンドウのUI外観をカスタマイズする方法について説明します。
keywords: グリッドJs、カスタマイズ、モーダル、UI、外観、ビジュアル、ウィンドウ
aliases:
aliases:
...
# GridJsのモーダルスタイリングガイド

## 目次

- [概要](#overview)
- [CSSクラス命名規則](#css-class-naming)
- [クイックスタート](#quick-start)
- [美しいスタイル例](#beautiful-style-examples)
- [テーマギャラリー](#theme-gallery)
- [高度な技術](#advanced-techniques)
...
| モーダルタイプ | 名前 | CSSクラス |
|------------|------|-----------|
| 並べ替え | `sort` | `.x-spreadsheet-modal-sort` |
| バリデーション | `validation` | `.x-spreadsheet-modal-validation` |
| 検索 | `search` | `.x-spreadsheet-modal-search` |
| コントロール | `control` | `.x-spreadsheet-modal-control` |
| ダウンロード | `download` | `.x-spreadsheet-modal-download` |
| メール | `email` | `.x-spreadsheet-modal-email` |
| 入力候補 | `input-suggest` | `.x-spreadsheet-modal-input-suggest` |
| URL挿入 | `insert-url` | `.x-spreadsheet-modal-insert-url` |
| リンク | `link` | `.x-spreadsheet-modal-link` |
| 印刷 | `print` | `.x-spreadsheet-modal-print` |
| テキスト回転 | `text-rotate` | `.x-spreadsheet-modal-text-rotate` |
| データ検証 | `data-validation` | `.x-spreadsheet-modal-component-data-validation` |
| 数値書式の詳細 | `more-number-formats` | `.x-spreadsheet-modal-more-number-formats` |
| 検証アラート | `validation-alert` | `.x-spreadsheet-modal-validation-alert` |
| 挿入削除 | `insert-delete` | `.x-spreadsheet-modal-insert-delete` |
...
プロフェッショナルおよびエンタープライズアプリケーションに最適です。
...
### 例5: コーポレートプロフェッショナルテーマ 💼

ビジネスやコーポレートアプリケーションに適しています。
