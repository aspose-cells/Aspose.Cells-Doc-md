...
## **可能な使用シナリオ**
client side scriptを使用してGridWebセルの値を読み取るには、gridwebinstance.getCellsArray()メソッドを使用します。一度呼び出すと、アクティブなワークシートのすべてのセルの配列が返されます。その後、以下のメソッドを使用してセルの値やその他の情報を取得できます。
