...
プロジェクトのビルド プロセスは Maven を使用して管理されます。したがって、IDE を使用せずにコマンド ラインから WAR ファイルを準備できます。次のコマンドを使用して、デプロイ用の WAR を生成します。対応するアプリケーション サーバーのドキュメントは、生成された WAR とその依存関係をデプロイする方法に役立ちます。
