...
Aspose.Cells は、Excel ファイルを HTML にエクスポートする際に、右から左へのテキストの展開をサポートするようになりました。この機能は v8.9.0.0 から実装されています。ソース Excel ファイルに右から左に展開されるテキストが含まれている場合、Aspose.Cells はそれを HTML に正しくエクスポートします。

{{% /alert %}} 
## **ExcelファイルをHTMLにエクスポートする際にテキストを右から左に展開する**
次のサンプル コードは、[サンプルエクセルファイル](5472562.xlsx) HTMLに。このスクリーンショットは、サンプル Excel が Microsoft Excel 2013 でどのように見えるかを示しています。
