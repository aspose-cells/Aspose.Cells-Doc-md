...
Aspose.Cells for Java 8.3.2 は、XpsSaveOptions クラスの OnePagePerSheet、PageIndex、および PageCount プロパティを公開しました。ユーザーは、OnePagePerSheet プロパティを使用して XPS の 1 ページにスプレッドシートのすべてのコンテンツを収めたり、PageCount プロパティを使用して印刷するページ数を取得したりできます。 PageIndex プロパティは、保存する最初のページの 0 から始まるインデックスを取得または設定します。
