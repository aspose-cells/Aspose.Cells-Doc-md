...
type: docs
weight: 600
url: /ja/nodejs-cpp/get-cells-range/
description: Aspose.Cells for Node.js via C++ APIを使用してセル範囲を取得する方法を学びます。
keywords: セルの最大表示範囲を取得、セルの最大行を取得、セルの最大データ行を取得、セルの最大列を取得、セルの最大データ列を取得します。 
---

## **可能な使用シナリオ**
ワークシートの一部のデータだけを操作する必要がある場合、ワークシート全体のデータ範囲を知る必要があります。Aspose.Cells for Node.js via C++はこの機能を提供します。以下のプロパティとメソッドを使用して目的を達成できます。
...
## **Aspose.Cells for Node.js via C++を使用したセル範囲の取得**
この例では、次のことができます:

1. ワークブックを作成する。
1. 最初のワークシートのセルにデータを追加する。
1. セル[**Range**](https://reference.aspose.com/cells/nodejs-cpp/range)を取得。
