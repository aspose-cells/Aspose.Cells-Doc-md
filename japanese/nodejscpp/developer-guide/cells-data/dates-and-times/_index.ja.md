...
type: docs  
weight: 600  
url: /ja/nodejs-cpp/how-to-manage-dates-and-times/  
description: Aspose.Cells for Node.js via C++ APIを通じて日時や時間の管理方法を学びます。  
...
数値を日時と時間として表示するには、必要な日時フォーマットをセルに適用します。これは[Style.getNumber()](https://reference.aspose.com/cells/nodejs-cpp/style/#getNumber--)または[Style.Custom]()プロパティ経由で設定します。セル値の.DateTimeValueプロパティは、セル内の数値が表す日時と時間を指定するDateTimeオブジェクトを返します。  
<br>  
...
1. 1904日付システム。最初の日付は1904年1月1日で、そのシリアル値は0です。最後の日付は9999年12月31日で、そのシリアル値は2,957,003です。この日付システムをワークブックで使用するには、[WorkbookSettings.getDate1904()](https://reference.aspose.com/cells/nodejs-cpp/workbooksettings/#getDate1904--)プロパティをtrueに設定します。  
