...
type: docs
weight: 10
url: /ja/nodejs-cpp/how-to-format-number-to-accounting/
description: この記事では、Aspose.Cells for Node.js via C++ API を使用して数値を会計フォーマットにする方法を紹介します。
...
## **Aspose.Cells for Node.js via C++の数字を会計形式に整える方法**
Aspose.Cells for Node.js via C++の数字を会計形式に整えるには、セルまたはセル範囲に関連付けられた`スタイル`オブジェクトを使用します。`スタイル`オブジェクトでは、さまざまな書式設定オプションを設定でき、数値フォーマットも含まれます。会計フォーマットのコードは、必要に応じて通貨記号や小数点の表示の有無など、特定の要件に合わせて変わることがあります。

以下は、Aspose.Cells for Node.js via C++のセルに会計数値フォーマットを適用する基本例です：

1. **Aspose.Cellsを参照**：Aspose.Cells for Node.js via C++をプロジェクトに参照させてください。Aspose公式サイトから入手できます。
...
この例では、1つのセルに対して米ドルの会計フォーマットを適用します。フォーマット文字列は、必要に応じて通貨記号や会計形式に調整可能です。重要な部分は`style.setCustom`メソッドで、会計のためのカスタム数値フォーマットコードを指定します。
