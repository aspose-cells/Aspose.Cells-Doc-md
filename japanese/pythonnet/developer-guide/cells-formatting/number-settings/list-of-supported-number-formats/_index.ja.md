...
description: Aspose.Cellsは、スプレッドシートファイルを処理するためのPythonライブラリで、多くの数値書式をサポートしています。この記事では、ユーザーが必要に応じて適切な書式を選択できるように、サポートされている数値書式のリストを提供します。
keywords: Aspose.Cells、Python Excelライブラリ、スプレッドシート、数値書式、リスト、サポート
type: docs
weight: 30
url: /ja/python-net/list-of-supported-number-formats/
---

## **Aspose.Cells for Python via .NET**
Aspose.Cells for Python via .NETコンポーネントは、数値と日付の表示形式を設定できるいくつかのビルトイン数値書式を提供します。これらのビルトイン書式は、**Style**オブジェクトの**Number**プロパティを使用して適用できます。すべてのビルトイン書式には一意の数値値が割り当てられています。開発者は任意の数値を**Style**の**Number**プロパティに割り当て、その表示フォーマットを適用できます。この方法はより高速です。Aspose.Cells for Python via .NETでサポートされているビルトイン数値書式は以下の通りです:
...
一部の番号形式では、月などの文字が含まれていることがあります。これらは実際には中国語や日本語バージョンのMS Excelで使用できる中国語の文字です。
