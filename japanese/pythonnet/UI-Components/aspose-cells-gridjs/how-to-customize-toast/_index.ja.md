---
title: GridJsのメッセージトーストのUI外観をカスタマイズする方法  
type: docs
weight: 250
url: /ja/python-net/aspose-cells-gridjs/how-to-customize-toast/
description: このこの記事は、GridJsのメッセージトーストのUI外観をカスタマイズする方法について説明します。
keywords: GridJs、カスタマイズ、トースト、UI、外観、ビジュアル、メッセージ、アラート
aliases:
aliases:
...
GridJsは、柔軟なカスタムトースト通知システムを提供しており、デフォルトのトーストメッセージを独自の通知実装に置き換えることができます。このガイドでは、アプリケーションのニーズに応じたトースト通知のさまざまなカスタマイズ方法を紹介します。
...
- `callback`（関数、省略可）：トーストが閉じられた後に実行されるコールバック関数
...
- `progressBar`: カウントダウン進行状況バーを表示
- `positionClass`: トーストの位置を制御します（上右、下左など）
- `timeOut`: 自動的に閉じるまでの時間（ミリ秒）
...
より良いユーザー体験のために、モーダルアラートではなくノンブロッキングのトースト通知を使用してください：
...
コンテンツの長さに基づいて適切な自動終了時間を選択してください：
...
常にDOM要素とイベントリスナーをクリーンアップしてください：
