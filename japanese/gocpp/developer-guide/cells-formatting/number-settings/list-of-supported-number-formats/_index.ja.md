---
title: Golang経由でサポートされている数字形式の一覧
description: Aspose.Cellsは、スプレッドシートファイルを処理するC++ライブラリで、さまざまな数値書式に対応しています。この記事ではサポートされる書式の一覧を提供し、必要に応じて適切なフォーマットを選択できるようにしています。
keywords: Aspose.Cells, C++,ライブラリ, スプレッドシート, 数値書式, 一覧, サポート
type: docs
weight: 30
url: /ja/go-cpp/list-of-supported-number-formats/
---

## **Aspose.Cells**
Aspose.Cellsコンポーネントは、数値や日付の表示形式を構成するためのいくつかの組み込み番号形式を提供します。これらの組み込み番号形式は、**Style**オブジェクトの**Number**プロパティを使用して適用できます。すべての組み込み番号形式には固有の数値が割り当てられています。開発者は、*Number*プロパティに任意の数値を割り当てることができ、表示形式が適用されます。
