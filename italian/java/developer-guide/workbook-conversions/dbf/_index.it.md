...
Per leggere un file DBF, è necessario creare un'istanza di `LoadOptions`, impostare la sua proprietà `LoadFormat` su `LoadFormat.Dbf` e passarla al costruttore di `Workbook` insieme al percorso del file. Una volta caricati, i dati diventano accessibili tramite la collezione `getWorksheets()`, dove è possibile scorrere le celle, estrarre valori o manipolare i dati secondo necessità.
...
2. Accedere al primo foglio di lavoro dalla collezione `getWorksheets()`.
