...
description: Aspose.Cells è una libreria Java per lavorare con file di fogli di calcolo, che supporta la lettura e la scrittura di file dBASE III e IV (DBF). Questo articolo spiega come importare dati da ed esportare dati verso file DBF utilizzando Aspose.Cells, inclusi i dettagli sul formato dei file, le funzionalità supportate e gli esempi passo-passo.
linktitle: Lettura e scrittura di DBF
url: /it/java/reading-and-writing-dbf-files/
keywords: Aspose.Cells, libreria Java, DBF, dBASE, leggere DBF, scrivere DBF, importare DBF, esportare DBF, formato file, .dbf
type: docs
weight: 200
...
- **dBASE IV** — Una versione estesa che supporta ulteriori tipi di dati e dimensioni di campo maggiori.
...
- Gestione dei tipi di dati comuni utilizzati nei file DBF, inclusi campi di carattere, numerici, di data e logici.
- Conservazione delle definizioni dei campi come nome, tipo e lunghezza del campo durante le operazioni di lettura/scrittura.
...
Per leggere un file DBF, è necessario creare un'istanza di `LoadOptions`, impostare la sua proprietà `LoadFormat` su `LoadFormat.Dbf` e passarla al costruttore `Workbook` insieme al percorso del file. Una volta caricati, i dati diventano accessibili attraverso la raccolta `getWorksheets()`, dove è possibile scorrere le celle, estrarre valori o manipolare i dati secondo necessità.
L'esempio seguente mostra come caricare un file DBF esistente in Aspose.Cells, accedere al suo primo foglio di lavoro e leggere i valori delle celle.
...
2. Accedere al primo foglio di lavoro dalla raccolta `getWorksheets()`.
...
L'esempio seguente mostra come creare un nuovo file DBF da zero. Popola un foglio di lavoro con dati di esempio contenenti diversi tipi di dati (stringhe, numeri e date) per illustrare come i tipi di campo vengono gestiti durante l'esportazione nel formato DBF.
...
{{% alert color="primary" %}}
Quando si scrivono dati in un file DBF, assicurarsi che i dati siano conformi alle limitazioni del formato. I nomi dei campi non devono essere più lunghi di 10 caratteri e non devono contenere spazi. I record che superano 4000 byte in totale non verranno salvati correttamente. Le date devono essere valori di data validi che possono essere rappresentati nel formato YYYYMMDD.

## **Considerazioni sui tipi di dati e sulla formattazione**
Quando si trasferiscono dati tra Aspose.Cells e il formato DBF, comprendere come i tipi di dati si mappano tra i due sistemi è importante per garantire l'integrità dei dati.
...
- Le **stringhe** vengono mappate in campi di carattere (C).
