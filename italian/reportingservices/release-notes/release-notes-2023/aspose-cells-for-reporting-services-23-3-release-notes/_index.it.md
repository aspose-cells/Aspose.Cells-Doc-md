---
title: Aspose.Cells Reporting Services 23.3 Note sulla versione
second_title: Aspose.Cells Reporting Services Documento
type: docs
url: /it/reportingservices/aspose-cells-for-reporting-services-23-3-release-notes/
description: Aspose.Cells Reporting Services supporta il rendering in tipi di report in formato. ad esempio Xlsx, Pdf, Json, Docx, Pptx, Html, Svg, Ods, Png e così via
weight: 19
---
{{% alert color="primary" %}} 
Questa pagina contiene le note sulla versione per Aspose.Cells Reporting Services 23.3
'{{% /alert %}} '
##  **Elenco completo dei problemi e dei miglioramenti trattati in questa versione**

|**ID**|**Riepilogo**|**Categoria**|
| :- | :- | :- |
| 39191 | Ottimizza Aspose.Cells Reporting Services Core.|
