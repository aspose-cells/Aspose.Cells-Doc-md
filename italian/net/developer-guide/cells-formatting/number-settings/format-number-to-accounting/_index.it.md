...
type: docs
weight: 10
url: /it/net/how-to-format-number-to-accounting/
description: Questo articolo introdurrà come formattare il Numero in contabilità usando l API Aspose.Cells for .NET.
...
## **Come formattare il Numero in contabilità in Aspose.Cells for .NET**
Per formattare i numeri come formato contabile in Aspose.Cells for .NET, puoi usare l'oggetto `Style` associato a una cella o a un intervallo di celle. L'oggetto `Style` ti permette di impostare varie opzioni di formattazione, inclusi i formati numerici. Il formato contabile di solito ha un codice di formato che può variare a seconda delle esigenze specifiche (come mostrare simboli di valuta, decimali, ecc.).

Ecco un esempio base di come applicare un formato numerico contabile a una cella in Aspose.Cells for .NET:
