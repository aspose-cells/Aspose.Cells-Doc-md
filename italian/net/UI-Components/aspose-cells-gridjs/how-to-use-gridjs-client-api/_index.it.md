...
| `isCollaborative` | Supporta la modalità collaborativa . | `false` | Sì |
...
| `showBottombarStats` | Mostrare le statistiche della barra inferiore.<br>Il valore predefinito è true. | `true` | Sì |   
| `showRowAppenderToolbar` | Mostrare la barra degli strumenti di aggiunta batch di righe.<br>Il valore predefinito è true. | `true` | Sì |   
...
```
- impostare toast personalizzati
```javascript
...
```
- Imposta la chiamata della funzione prima di salvare 
```javascript
...
- Configura le impostazioni collaborative in modalità collaborativa, assicurati di impostare setCollaborativeSetting prima di setUniqueId  
```javascript
...
- Imposta intervallo modificabile/sola lettura
```javascript
...
- Imposta il commento sulla cella
```javascript
...
- Rimuovi il commento sulla cella
```javascript
...
- Imposta lo stile per l’area desiderata della cella
```javascript
...
```
- Elimina il contenuto della cella o azzera lo stile sulla cella selezionata  
```javascript
...
- Elimina il contenuto della cella o azzera lo stile sull’area desiderata della cella
```javascript
...
- Cambia il colore di sfondo dell’oggetto forma
```javascript
...
- Applicare le impostazioni del carattere per l'oggetto casella di testo
```javascript
