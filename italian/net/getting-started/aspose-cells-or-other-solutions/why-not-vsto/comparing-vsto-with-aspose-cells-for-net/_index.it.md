---
title: Confrontando VSTO con Aspose.Cells for .NET
type: docs
weight: 20
url: /it/net/comparing-vsto-with-aspose-cells-for-net/
---
{{% alert color="primary" %}}

Questo articolo confronta l'utilizzo di VSTO (Visual Studio Tools per Office) con altri approcci per lo sviluppo di soluzioni basate su Microsoft Office. In particolare guarda allo Aspose.Cells e mette a confronto il funzionamento delle due soluzioni. Gli articoli forniscono informazioni che gli sviluppatori possono utilizzare per analizzare, confrontare e valutare diverse opzioni prima di adottare una soluzione.
...
### **Requisito di .NET Framework**

VSTO richiede .NET Framework (inclusi Visual Studio Tools per Office SE Runtime) sul lato client per eseguire l'applicazione finale. Nella maggior parte degli ambienti aziendali, in particolare negli scenari Web, gli utenti finali non possono installare il software applicativo ei relativi framework di runtime. Questo requisito da solo rende problematiche le applicazioni basate su VSTO. Praticamente esclude le applicazioni standard basate su VSTO.

Al contrario, Aspose.Cells for .NET non richiede necessariamente .NET Framework sul lato client per lo scenario sottostante. Le applicazioni Office create con il componente sono leggere e garantite per funzionare su sistemi Microsoft Windows sotto carico significativo.
...
- Il costo iniziale di .NET è intrinsecamente costoso. Le applicazioni scritte con .NET devono sostenere il sovraccarico della compilazione Just-In-Time (JIT), quindi la compilazione JIT non può essere evitata.
- Un altro fattore di prestazioni che influenza le applicazioni basate su VSTO ha a che fare con la spesa per chiamare attraverso gli spessi strati dell'interfaccia di automazione che racchiudono gli oggetti COM di Microsoft Office. VBA, creato e ottimizzato per interagire con Microsoft Office, ha una distanza da percorrere più breve rispetto a .NET.
...
Se stai pensando di adottare VSTO come piattaforma di sviluppo per la tecnologia Microsoft Office, dedica un po' di tempo a esaminare le risorse per acquisire familiarità con questi attributi.
...
Al contrario, Aspose.Cells for .NET è altamente scalabile, flessibile e veloce. In genere, le applicazioni di Office non sono progettate per essere utilizzate contemporaneamente da centinaia e migliaia di utenti; tuttavia, Aspose.Cells lo è. L'API è stabile e può eseguire le attività del foglio di calcolo in modo impeccabile sia su un singolo server, alimentando una singola applicazione o su una web farm con bilanciamento del carico che alimenta un'applicazione a livello aziendale.
...
- **Sistemi operativi supportati**Windows 2000; Server Windows 2003; Windows Vista; Windows XP
- **Versioni di .NET Framework supportate**: solo .NET framework 2.0 o superiore.
- Una o più delle seguenti edizioni di Visual Studio Tools per Office:
 - Strumenti di Microsoft Visual Studio 2005 per Microsoft Office System
 - Strumenti di Microsoft Visual Studio 2005 per Microsoft Office System 2007
...
Aspose.Cells non richiede l'installazione di Microsoft Excel né sul client né sul server, in quanto è un motore di creazione di fogli di calcolo. Per visualizzare i documenti di Microsoft Excel, tuttavia, è necessario almeno Microsoft Excel Viewer installato sul sistema.

- **Sistemi operativi supportati**Windows 2000; Server Windows 2003; Windows Vista; Windows XP
- **Versioni di .NET Framework supportate**: sono supportati tutti i framework .NET, 1.0, 1.1, 2.0, 3.x ecc.
...
1. Come modello viene utilizzato un file Microsoft Excel (TempBook.xls).
 La cartella di lavoro contiene alcuni fogli di lavoro con alcune celle piene di dati.
1. Il codice di esempio inserisce 1000*20 record nel primo foglio di lavoro nel file Excel modello.
 Il foglio di lavoro è riempito con dati costanti (fittizi) nelle celle.
...
I segmenti di codice seguenti illustrano come eseguire queste attività con ciascuna API.
...
I risultati hanno mostrato che l'utilizzo dell'API VSTO ha richiesto circa 2,5 minuti (circa oltre 150 secondi) per completare l'attività, mentre Aspose.Cells ha utilizzato meno di 1 secondo su un hardware comune con normali configurazioni di sistema.
...
Se stai pensando di utilizzare una tecnologia Microsoft Office in una soluzione aziendale, prima acquisisci familiarità con le alternative disponibili. Esegui alcuni test basati su diversi prodotti ed esponili a una varietà di condizioni del mondo reale come carico e stress per vedere quanto bene si comportano.
