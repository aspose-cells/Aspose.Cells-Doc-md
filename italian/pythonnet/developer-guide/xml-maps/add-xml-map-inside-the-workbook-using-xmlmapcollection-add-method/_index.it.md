---
title: Aggiungi mappa XML all interno del foglio di lavoro utilizzando il metodo XmlMapCollection.Add
type: docs
weight: 10
url: /it/python-net/add-xml-map-inside-the-workbook-using-xmlmapcollection-add-method/
---

## **Possibili Scenari di Utilizzo**

Aspose.Cells for Python via .NET fornisce il metodo [**XmlMapCollection.add()**](https://reference.aspose.com/cells/python-net/aspose.cells/xmlmapcollection/add/#str) che puoi usare per importare la mappa XML nel file di lavoro.

## **Aggiungi mappa XML all'interno del foglio di lavoro utilizzando il metodo XmlMapCollection.Add**

Il seguente codice di esempio aggiunge una mappa XML all'interno del workbook utilizzando il metodo [**XmlMapCollection.add()**](https://reference.aspose.com/cells/python-net/aspose.cells/xmlmapcollection/add/#str) e lo salva come [file Excel di output](5115434.xlsx). La schermata mostra la mappa XML che è stata importata dal file [sample.xml](5115433.xml) all'interno del file Excel di output.

![add-xml-map](add-xml-map.png)

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "XmlMaps-AddXmlMapInsideWorkbook.py" >}}

