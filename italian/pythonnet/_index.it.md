---
title: Aspose.Cells for Python via .NET
type: docs
weight: 110
url: /it/python-net/
is_root: true
description: Aspose.Cells for Python supporta i popolari fogli di calcolo (XLS, XLSX, XLSM, XLSB, XLTX, XLTM, CSV, SpreadsheetML, ODS) formati di file che la tua azienda utilizza ogni giorno. Consente inoltre di esportare file Excel® in formato PDF, DOCX, PPTX, JSON, XPS, HTML, MHTML, testo normale e formati di immagini popolari tra cui TIFF, JPG, PNG, BMP e SVG.
---
{{% alert color="primary" %}}

**![Aspose.Cells for Python via .NET Logo del prodotto](home_1.png)**

**Benvenuti a Aspose.Cells for Python via .NET**

Aspose.Cells for Python via .NET consente alle applicazioni di eseguire una vasta gamma di attività di elaborazione di fogli di calcolo. Aspose.Cells for Python via .NET supporta i popolari fogli di calcolo (XLS, XLSX, XLSM, XLSB, XLTX, XLTM, CSV, SpreadsheetML, ODS) formati di file che la tua azienda utilizza ogni giorno. Consente inoltre di esportare file Excel® in formato PDF, DOCX, PPTX, JSON, XPS, HTML, MHTML, testo normale e formati di immagini popolari tra cui TIFF, JPG, PNG, BMP e SVG.

{{% /alert %}}

## **Aspose.Cells for Python via .NET Risorse**

Di seguito sono riportati i collegamenti ad alcune risorse utili di cui potresti aver bisogno per svolgere le tue attività.

- [Aspose.Cells for .NET Documentazione in linea](/cells/it/net/)(Aspose.Cells for Python via .NET è molto simile a Aspose.Cells for .NET. Quindi, puoi usare la stessa documentazione)
- [Aspose.Cells for Python via .NET Caratteristiche](/cells/it/python-net/features/)
- [Aspose.Cells for Python via .NET Note di rilascio](https://releases.aspose.com/cells/python-net/release-notes/)
- [Aspose.Cells for Python via .NET Pagina prodotto](https://products.aspose.com/cells/python-net/)
- [Scarica Aspose.Cells for Python via .NET](https://downloads.aspose.com/cells/python-net)
- [Aspose.Cells for Python via .NET Forum di supporto gratuito](https://forum.aspose.com/c/cells/9)
- [Aspose.Cells for Python via .NET Helpdesk di supporto a pagamento](https://helpdesk.aspose.com/)
