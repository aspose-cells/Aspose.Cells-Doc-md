...
I campi valore sono il cuore di ogni tabella pivot, le aggregazioni numeriche che sintetizzano i dati di origine. In Aspose.Cells for Node.js via C++, l'area dati di una tabella pivot viene popolata aggiungendovi campi di base tramite `PivotTable.addFieldToArea`, e ogni campo inserito in quell'area può avere una propria funzione di riepilogo. Quando esistono due o più campi dati, Aspose.Cells espone un campo aggregato speciale, `PivotTable.getValuesField`, che può essere posizionato sull'asse Riga o Colonna come campo di base, offrendoti un controllo più fine su come i campi valore vengono visualizzati nel layout.
...
## **Trascinare un campo di base nell'area Valore**

Questo scenario mostra come inserire un singolo campo di base (`Amount`) nell'area dati di una tabella pivot esistente. La struttura pivot condivisa posiziona `Category` e `Item` sull'asse Riga e `Year` sull'asse Colonna. Dopo l'operazione, `Amount` appare nell'area dati e viene calcolato come `Sum` di `Amount` per impostazione predefinita.
...
Questo scenario parte dalla stessa struttura pivot delloma aggiunge il campo `Amount` all'area dati due volte. Entrambi i campi dati fanno riferimento alla stessa colonna di origine, tuttavia il secondo campo viene sovrascritto utilizzando il setter `setFunction()` in modo che diventi `Count` invece del valore predefinito `Sum`.
...
Con due campi dati presenti, `PivotTable.getValuesField()` diventa utilizzabile. Questo scenario trascina quel campo virtuale aggregato nell'area Colonna in modo che ogni misura nell'area dati appaia come un proprio blocco di colonne accanto a `Year`.
