...
|CELLSCPP-69|Risolvi i problemi nei casi di test C++|Nuova caratteristica|
...
|CELLSCPP-75|Implementa la funzione C++: Disegna immagine da Stream|Nuova caratteristica|
...
|CELLSCPP-78|Risolvi i problemi nei casi di test C++: Firma digitale, EnumTypes, Trova, Formule, Moduli collegamenti ipertestuali|Nuova caratteristica|
|CELLSCPP-79|Risolvi il problema di collegamento della versione di rilascio C++|Nuova caratteristica|
...
|CELLSCPP-102|I grafici non vengono analizzati (conservati) quando si apre/salva un formato di file XLSX|Nuova caratteristica|
...
Rinomina i metodi, ad esempio SetIsOutlineShown in SetIsOutlineShown, SetIsSelected in SetSelected in IWorksheet e così via. Per maggiori dettagli, vedere la guida di riferimento API.
...
Rappresenta il formato di visualizzazione personalizzato di numeri e date. Poiché il nome del metodo GetNumberFormat è in conflitto con la funzione di sistema Windows, dobbiamo rinominarlo.
#### **Rinomina IStyleFlag::GetNumberFormat in Get_NumberFormat**
Poiché il nome del metodo GetNumberFormat è in conflitto con la funzione di sistema Windows, dobbiamo rinominarlo che rappresenta per ottenere l'impostazione del formato numerico.
