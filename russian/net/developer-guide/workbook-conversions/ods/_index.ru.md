---
title: Преобразование книги Excel в ODS, SXC и FODS (OpenOffice / LibreOffice Calc).
linktitle: Ods
type: docs
weight: 20
url: /ru/net/convert-excel-to-ods/
description: Как конвертировать Excel в Ods (OpenOffice / LibreOffice Calc) или конвертировать Ods (OpenOffice / LibreOffice Calc) в Excel с помощью Aspose.Cells.
---

## **Об OpenDocument**
[Формат OpenDocument (ODF)](https://en.wikipedia.org/wiki/OpenDocument) - бесплатный и открытый формат файла для электронных офисных документов, изначально разработанный Sun для пакета Open Office. OpenDocument Spreadsheet (ODS) - это формат файла для документов Excel. OpenDocument в настоящее время является стандартом OASIS и ISO.

## **Преобразовать Ods (OpenOffice / LibreOffice Calc) в Excel**
Aspose.Cells поддерживает загрузку Ods, Sxc и Fods, которые поддерживаются OpenOffice / LibreOffice Calc, и конвертировать [Ods](book1.ods), [Sxc](book1.sxc) и [Fods](book1.fods) в файлы Excel.

{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Convert-ODS-to-Excel.cs" >}}

## **Конвертировать Excel в Ods (OpenOffice / LibreOffice Calc)**
Aspose.Cells поддерживает преобразование файлов Excel в файлы Ods, Sxc и Fods. Приведенный ниже пример кода показывает, как конвертировать [шаблон](book1.xlsx) в файлы Ods, Sxc и Fods.

{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Convert-Excel-to-ODS.cs" >}}

## **Продвинутые темы**
- [Сохранить файл ODS в спецификациях ODF 1.1 и 1.2](/cells/ru/net/save-ods-file-in-odf-1-1-and-1-2-specifications/)
- [Работа с фоном в файлах ODS](/cells/ru/net/working-with-background-in-ods-files/)
