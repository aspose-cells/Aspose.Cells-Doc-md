...
На практике отдавайте предпочтение API на основе кэша вместо устаревшего подхода с вызовом `RefreshData()` для каждой таблицы. Они учитывают общие кэши, избегают избыточных извлечений из источника и позволяют выбрать наименьший объём, удовлетворяющий вашим требованиям к обновлению.{{< app/cells/assistant language="csharp" >}}
