---
title: Использование встроенных стилей
type: docs
weight: 80
url: /ru/net/using-built-in-styles/
description: C# код для использования встроенных стилей Excel с Aspose.Cells for .NET API
keywords: c# use excel built in styles, c# programmatically apply styles in workbook, programmatically apply styles in workbook, c# apply built in styles in excel, c# apply built in styles in workbook, c# code apply built in styles in workbook, c# code apply built in styles in excel workbook
---
{{% alert color="primary" %}}

Aspose.Cells предоставляет обширную коллекцию повторно используемых стилей для форматирования ячейки в табличном документе. Мы можем использовать встроенные стили в нашей книге, а также создавать собственные стили.

{{% /alert %}}

## **Как использовать встроенные стили**

 Метод[**Workbook.CreateBuiltinStyle**](https://reference.aspose.com/cells/net/aspose.cells/workbook/methods/createbuiltinstyle) и перечисление[**Тип встроенного стиля**](https://reference.aspose.com/cells/net/aspose.cells/builtinstyletype) сделать удобным использование встроенных стилей. Вот список всех возможных встроенных стилей:

- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_1
- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_2
- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_3
- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_4
- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_5
- ДВАДЦАТЬ_ПРОЦЕНТ_АКЦЕНТ_6
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_1
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_2
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_3
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_4
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_5
- СОРОК_ПРОЦЕНТ_АКЦЕНТ_6
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_1
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_2
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_3
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_4
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_5
- ШЕСТЬДЕСЯТ_ПРОЦЕНТ_АКЦЕНТ_6
- АКЦЕНТ_1
- АКЦЕНТ_2
- АКЦЕНТ_3
- АКЦЕНТ_4
- АКЦЕНТ_5
- АКЦЕНТ_6
- ПЛОХО
- РАСЧЕТ
- CHECK_CELL
- ЗАПЯТАЯ
- COMMA_1
- ВАЛЮТА
- ВАЛЮТА_1
- EXPLANATORY_TEXT
- ХОРОШИЙ
- HEADER_1
- HEADER_2
- HEADER_3
- ЗАГОЛОВОК_4
- ГИПЕРССЫЛКА
- FOLLOWED_HYPERLINK
- ВХОД
- LINKED_CELL
- НЕЙТРАЛЬНО
- НОРМАЛЬНЫЙ
- ПРИМЕЧАНИЕ
- ВЫВОД
- ПРОЦЕНТ
- ЗАГЛАВИЕ
- ОБЩЕЕ КОЛИЧЕСТВО
- WARNING_TEXT
- ROW_LEVEL
- COLUMN_LEVEL

## C# код для использования встроенных стилей

{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Examples-CSharp-Articles-UsingBuiltinStyles-1.cs" >}}
