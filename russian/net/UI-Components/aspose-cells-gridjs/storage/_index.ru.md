...
После импорта файла электронной таблицы

 GridJs создаст файл кеша с указанным uid в**`Config.FileCacheDirectory`** папка ,
 ...
 GridJs также сохраняет все формы/изображения в zip-архив в**`Config.PictureCacheDirectory`** папку для последующего отображения фигур/изображений в пользовательском интерфейсе клиента.

и после каждой операции обновления в пользовательском интерфейсе клиента,
...
В этом действии сохранение обратно в файл кеша произойдет во время метода UpdateCell.
```C#   
...
#### 1. указанный uid для файла
 Убедитесь, что указанное соответствие сопоставления между файлом и uid,
...
####  2. синхронизация с работой пользовательского интерфейса клиента
На самом деле для некоторых операций пользовательского интерфейса клиента
...
Таким образом, если мы хотим получить обновленный файл точно так же, как показывает пользовательский интерфейс клиента,

нам нужно выполнить операцию слияния перед сохранением, чтобы синхронизировать эту операцию пользовательского интерфейса клиента.
```javascript
//in the js
  function save() {
