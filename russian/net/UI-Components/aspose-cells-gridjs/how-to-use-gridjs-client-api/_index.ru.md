...
| `isCollaborative` | Поддержка совместного режима . | `false` | Да |
...
| `token` | Установка токена аутентификации. Когда токен не равен null, заголовок `Authorization: Bearer {token}` автоматически добавляется к заголовкам запроса. Можно использовать `xs.refreshToken(token)` для установки нового токена. | Нет | Да |    
| `showBottombarStats` | Показывать статистику в нижней панели.<br>Значение по умолчанию — true. | `true` | Да |   
...
```
- установить пользовательское уведомление
```javascript
...
```
- Установить вызов функции до сохранения 
```javascript
...
- Настроить совместные параметры в режиме совместной работы, убедитесь, что setCollaborativeSetting выполнено перед setUniqueId  
```javascript
...
- Установить разрешенную/запрещенную область редактирования
```javascript
...
- Установить стиль для выбранной области ячеек
```javascript
...
- Удалить содержимое ячейки или очистить стиль в выбранной области ячеек
```javascript
...
```
## API для объекта формы
- Изменить цвет фона объекта формы
```javascript
...
-  Применить настройки шрифта для объекта текстового поля
```javascript
