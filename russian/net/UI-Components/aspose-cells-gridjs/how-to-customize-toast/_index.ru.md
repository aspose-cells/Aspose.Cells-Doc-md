---
title: как настроить внешний вид UI для подсказки сообщения в GridJs  
type: docs
weight: 250
url: /ru/net/aspose-cells-gridjs/how-to-customize-toast/
description: Эта статья описывает, как настроить внешний вид UI для toast сообщений в GridJs.
keywords: GridJs,настройка,toast,UI,внешний вид,визуальный,сообщение,предупреждение
aliases:
aliases:
...
Руководство по созданию пользовательских toast сообщений для GridJs

## Обзор

GridJs предлагает гибкую систему пользовательских уведомлений toast, которая позволяет вам заменить стандартные сообщения toast на свои собственные. Это руководство демонстрирует различные подходы к настройке уведомлений toast в соответствии с потребностями вашего приложения.
...
- [Продвинутые сценарии использования](#advanced-use-cases)
...
- `callback` (функция, необязательно): Функция обратного вызова после закрытия тоста
...
- `progressBar`: Отображать счетчик обратного отсчета
- `positionClass`: Регулирует положение тоста (верхний правый, нижний левый и т.д.)
...
- Журнал ошибок (Debug логирование)
...
**Функции мобильной оптимизации:**
- Расположение внизу (удобно для большого пальца)
- Ширина на основе процентов (максимум 80%)
- Более короткая продолжительность (2 секунды)
- Большие цели для нажатия
- Без наложения (невмешивающийся)
...
Для лучшего пользовательского опыта используйте неблокирующие тост-уведомления вместо модальных оповещений:
...
### 3. Соответствующая длительность

Выбирайте подходящую длительность автоматического исчезновения в зависимости от длины контента:
...
Убедитесь, что ваш пользовательский тост доступен:
...
Оборачивайте функцию тоста в try-catch, чтобы избежать сбоев в таблице:
...
### 7. Восстановление стандартных настроек при необходимости

Обеспечьте способ восстановления стандартного поведения тостов:
...
**Проблема:** Страница становится медленной после многочисленных уведомлений тост.

**Решение:** Очистите DOM-элементы и обработчики событий:
```javascript
const remove = () => {
  document.body.removeChild(toast);
