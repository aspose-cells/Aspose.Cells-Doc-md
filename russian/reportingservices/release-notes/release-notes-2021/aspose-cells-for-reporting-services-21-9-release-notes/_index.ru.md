---
title: Aspose.Cells для примечаний к выпуску служб Reporting Services 21.9
type: docs
url: /ru/reportingservices/aspose-cells-for-reporting-services-21-9-release-notes/
weight: 13
---
{{% alert color="primary" %}} 
На этой странице содержатся примечания к выпуску Aspose.Cells Reporting Services 21.9.
{{% /alert %}} 
## **Полный список проблем и улучшений, затронутых в этом выпуске**
|**Я БЫ**|**Резюме**|**Категория**|
|:- |:- |:- |
|21848 |Aspose.Cells Службы Reporting Services поддерживают экспорт файла формата Dif.|
