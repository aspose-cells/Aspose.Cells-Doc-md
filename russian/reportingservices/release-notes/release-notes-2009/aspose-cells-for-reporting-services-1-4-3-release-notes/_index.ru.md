---
title: Aspose.Cells для Reporting Services 1.4.3 Примечания к выпуску
type: docs
weight: 10
url: /ru/reportingservices/aspose-cells-for-reporting-services-1-4-3-release-notes/
---
{{% alert color="primary" %}} 

На этой странице содержатся примечания к выпуску [Aspose.Cells для Reporting Services 1.4.3|https://downloads.aspose.com/cells/reportingservices/new-releases/-aspose.cells-for-reporting-services-1.4.3/]{{% /alert %}} 

 Мы рады объявить Aspose.Cells для Reporting Services v1.4.3!

 Что изменилось

? 

· Aspose.Cells.ReportingServices Client поддерживает формат отчетов Reporting Services 2008.

 · Улучшена производительность рендеринга.

 · Включает стиль элементов отчета в отчет, созданный дизайнером отчетов.

 · Aspose.Cells.ReportingServices Client улучшен с точки зрения подключения к серверу отчетов.

 · 13 исправлений и других улучшений.



 Проблемы, решенные в Aspose.Cells для Reporting Services 1.4.

\3. 

|**Идентификатор проблемы** |**Составная часть** |**Резюме** |
|:- |:- |:- |
|11837 | Оказывать| Функции, поддерживаемые Aspose.Cells.ReportingServices|
|11563 | Оказывать| Сохранение подписки SSRS в файловой системе как проблема XLS|
|11085 | Оказывать| Aspose.Cells 64-битные проблемы|
|11090 | Оказывать| Программные строки|
|10710 | Оказывать| Экспорт отчета в .xls|
|10678 | Оказывать| Проблемы с Aspose.Cells для служб отчетности|
|10446 | Оказывать| Объединить и скрыть Cells|
|10264 | Оказывать| Срочно -- Цвет фона и изображения|
|10262 | Оказывать| Срочные-Атрибуты|
|10242 | Оказывать| Макросы в файле excel, загруженном из RDL, не работают|
|10241 | Оказывать| Невозможно добавить более одного набора данных|
|10118 | Оказывать| Скорость экспорта в Excel|
|10063 | Оказывать| Проблемы с подготовкой отчета|

