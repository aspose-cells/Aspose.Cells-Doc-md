...
Aspose.Cells для служб Reporting Services в основном содержит два компонента: Aspose.Cells.Report.Designer и Aspose.Cells.Renderer для служб Reporting Services. Первый используется для разработки отчетов непосредственно в Microsoft Excel, а второй отвечает за визуализацию отчета RDL в Microsoft Excel.
...
При выборе отчета для предварительного просмотра на сервере отчетов вам будет предложено указать, в какой формат файла его экспортировать (например, Microsoft Excel 97-2003 двоичный формат XLS, SpreadsheetML или Microsoft Excel 2007 XLSX), а также любые созданные входные параметры отчета. во время оформления отчета. После этого отчет заполняется данными, предоставленными Reporting Services.
