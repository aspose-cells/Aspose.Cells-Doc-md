---
title: Aspose.Cells для примечаний к выпуску CPP 22.5 Linux
type: docs
weight: 8
url: /ru/cpp/aspose-cells-for-cpp-22-5-release-notes-linux/
---
{{% alert color="primary" %}}

На этой странице содержатся примечания к выпуску Aspose.Cells для C++ 22.5 для Linux.

{{% /alert %}}

Мы рады объявить о выпуске Aspose.Cells для C++ 22.5 для Linux.
|**Ключ**|**Резюме**|**Категория**|
|:- |:- |:- |
|CELLSCPP-778| Оптимизировать xml-модуль|Оптимизировать|
|CELLSCPP-784| Исправить ошибку DateTime::parseUTC|Ошибки|
|CELLSCPP-786| Функции синхронизации с версией .Net|Поддерживать|
