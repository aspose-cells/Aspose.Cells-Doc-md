...
GridJs создаст файл кэша с указанным идентификатором в папке **`Config.getFileCacheDirectory()`** ,
...
GridJs также сохраняет все формы/изображения в zip-архивный файл в папке **`Config.getPictureCacheDirectory()`** для последующего отображения форм/изображений в клиентском интерфейсе.
...
GridJs будет создавать и сохранять файл в папке **`Config.getFileCacheDirectory()`** , которая является кэш-директорией по умолчанию, которую мы можем установить.
