...
type: docs
weight: 250
url: /ru/java/aspose-cells-gridjs/how-to-use-gridjs-client-api/
...
| `isCollaborative` | Поддержка совместной работы. | `false` | Да |
...
| `token` | Установить токен аутентификации. Когда токен не равен null, в заголовки запроса автоматически добавляется `Authorization: Bearer {token}`. Можно использовать `xs.refreshToken(token)` для установки нового токена. | Нет | Да |    
| `showBottombarStats` | Показывать статистику в нижней панели.<br>Значение по умолчанию — true. | `true` | Да |   
| `showRowAppenderToolbar` | Показывать панель инструментов для добавления строк.<br>Значение по умолчанию — true. | `true` | Да |   
...
```
- настройка пользовательского тоста
```javascript
...
- Настроить параметры совместной работы в совместном режиме, убедитесь, что setCollaborativeSetting вызван перед setUniqueId  
```javascript
...
```
- Удалить содержимое ячейки или очистить стиль ячейки  
```javascript
...
- Удалить содержимое ячейки или очистить стиль выбранной области ячейок
```javascript
