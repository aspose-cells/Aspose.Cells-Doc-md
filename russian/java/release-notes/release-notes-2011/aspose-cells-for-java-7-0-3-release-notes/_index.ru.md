...
40022 - Чтение файла шаблона XLS с установленным параметром: LoadDataOnly=true дает исключение
 ...
 31303 - Cell значение с символами '"' и ',' было сохранено неправильно для CSV-файла
 ...
 40013 - Символы автофильтра дублировались и становились больше при повторном сохранении файла XLS

 40014 - Линии сетки не отображались на сгенерированном изображении для листа.

 40016 - Сохраненный файл XLS приводит к тому, что MS Excel выдает предупреждающие сообщения -1

 40020 - Сохраненный файл XLS приводит к тому, что MS Excel выдает предупреждающие сообщения -2

40021 - Сохраненный файл XLS приводит к тому, что MS Excel выдает предупреждающие сообщения -3
 ...
 40085 — Когда строка/столбец превышает ограничение файла XLS, сохраненный XLS вызывает: «Excel обнаружил нечитаемое содержимое» при открытии в MS Excel
