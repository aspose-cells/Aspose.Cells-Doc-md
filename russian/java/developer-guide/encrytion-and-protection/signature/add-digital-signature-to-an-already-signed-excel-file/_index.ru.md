...
В следующем примере кода объясняется, как использовать[**Workbook.addDigitalSignature(DigitalSignatureCollection digitalSignatureCollection)**](https://reference.aspose.com/cells/java/com.aspose.cells/workbook#addDigitalSignature(com.aspose.cells.DigitalSignatureCollection)) метод добавления цифровой подписи к уже подписанному файлу Excel. Пожалуйста, проверьте[образец файла Excel](50528287.xlsx)используется в этом коде. Этот файл уже имеет цифровую подпись. Пожалуйста, проверьте[выходной файл Excel](50528288.xlsx)генерируется кодом. Мы использовали демонстрационный сертификат с именем[AsposeTest.pfx](50528289.pfx)в этом коде, у которого есть пароль*предполагать*На снимке экрана показано влияние примера кода на образец файла Excel после выполнения.
