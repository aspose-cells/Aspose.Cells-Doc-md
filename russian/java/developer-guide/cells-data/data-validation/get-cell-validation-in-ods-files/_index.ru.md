---
title: Получить Cell Проверка в файлах ODS
type: docs
weight: 190
url: /ru/java/get-cell-validation-in-ods-files/
---
## **Получить Cell Проверка в файлах ODS**

С помощью Aspose.Cells вы можете применить проверку к ячейке в файлах ODS. Для этого API предоставляет[**Получитьпроверку**](https://reference.aspose.com/cells/java/com.aspose.cells/cell#getValidation()) метод[**Cell**](https://reference.aspose.com/cells/java/com.aspose.cells/Cell)учебный класс.

В следующем примере кода показано использование[**Получитьпроверку**](https://reference.aspose.com/cells/java/com.aspose.cells/cell#getValidation()) путем загрузки[источник ОРВ](SampleBook1.ods)файл и чтение проверки ячейки A9.

### **Образец кода**

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-AsposeCellsExamples-Worksheets-GetCellValidationInODS-1.java" >}}
