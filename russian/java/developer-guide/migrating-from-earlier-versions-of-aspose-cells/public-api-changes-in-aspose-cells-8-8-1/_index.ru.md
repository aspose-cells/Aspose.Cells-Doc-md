...
Aspose.Cells API-интерфейсы уже предоставляют возможность рендеринга диаграмм в PDF при использовании метода Chart.toPdf. В этом выпуске API представлена еще одна перегруженная версия указанного метода, которая может принимать экземпляр OutputStream, позволяя пользователям сохранять PDF-файл диаграммы в экземпляре ByteArrayOutputStream.
