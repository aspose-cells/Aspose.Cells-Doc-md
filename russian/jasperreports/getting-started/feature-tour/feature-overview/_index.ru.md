---
title: Обзор функций
type: docs
weight: 10
url: /ru/jasperreports/feature-overview/
---
{{% alert color="primary" %}}

Aspose.Cells for JasperReports — это продукт для экспорта отчетов из JasperReports и JasperServer в формат Microsoft Excel. Aspose.Cells for JasperReports наследует все функции встроенного экспортера, а также компенсирует некоторые недостатки оригинального средства визуализации XLS JasperReports. В этом разделе представлено подробное описание функций Aspose.Cells for JasperReports. Нажмите на соответствующие ссылки в таблице, чтобы узнать больше о функциях.

{{% /alert %}}

|**Особенность** |**Описание** |
|:- |:- |
|[Экспорт XLS или XLSX](/cells/ru/jasperreports/xls-or-xlsx-export/)| Экспортируйте отчеты в формате Microsoft Excel XLS из вашего приложения, не используя Microsoft Excel.|
|[Высокая точность дизайна отчета](/cells/ru/jasperreports/high-fidelity-to-the-report-design/)| Макет и внешний вид отчета XLS/XLSX соответствуют оригиналу, подобно PDF или другим форматам.|
|[Простое и легкое развертывание](/cells/ru/jasperreports/easy-and-lightweight-deployment/)| Aspose.Cells для[ДжасперРепортс](https://community.jaspersoft.com/project/jasperreports-library) это[ДжасперРепортс](https://community.jaspersoft.com/project/jasperreports-library) и экспортер JasperServer в формат Microsoft Excel.|
|[Простое и доступное лицензирование](/cells/ru/jasperreports/simple-and-affordable-licensing/)| Мы предоставляем доступное лицензирование, которое легко использовать в исходном коде.|
|[Бесплатная техническая поддержка мирового уровня](/cells/ru/jasperreports/world-class-free-technical-support/)| Неограниченная техническая поддержка предоставляется непосредственно разработчиками продукта.|
|[Разнообразный](/cells/ru/jasperreports/miscellaneous/)| Пользователям предоставляются более эффективные функции.|
