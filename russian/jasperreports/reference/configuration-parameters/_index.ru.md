...
|ЯВЛЯЕТСЯ_УДАЛЯТЬ_ПУСТОЙ_ПРОСТРАНСТВО_ BETWEEN_ROWS| Логическое значение, указывающее, следует ли удалять пустые места, которые могут появляться между строками.|
|ЯВЛЯЕТСЯ_УДАЛЯТЬ_ПУСТОЙ_ПРОСТРАНСТВО_ BETWEEN_COLUMNS| Логическое значение, указывающее, следует ли удалять пустые пространства, которые могут появляться между столбцами.|
|ЯВЛЯЕТСЯ_БЕЛЫЙ_ PAGE_BACKGROUND| Логическое значение, указывающее, должен ли фон страницы быть белым или фоновым цветом XLS по умолчанию. Цвет фона XLS может различаться в зависимости от свойств средства просмотра XLS или цветовой схемы операционной системы.|
|ЯВЛЯЕТСЯ_ОБНАРУЖИТЬ_ CELL_TYPE| Флаг, используемый для указания, должен ли экспортер учитывать тип выражений исходного текстового поля и соответствующим образом устанавливать типы и значения ячеек.|
