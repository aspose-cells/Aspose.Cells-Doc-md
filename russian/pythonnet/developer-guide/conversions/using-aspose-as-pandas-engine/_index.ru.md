---
title: Использование Aspose.Cells для Python via .NET в качестве движка Excel для pandas
type: docs
weight: 40
url: /ru/python-net/using-aspose-as-pandas-engine/
description: Использование Aspose.Cells для Python via .NET в качестве движка Excel для pandas.
keywords: Использование Aspose.Cells для Python via .NET в качестве движка Excel для pandas.
---

{{% alert color="primary" %}}

Этот гид показывает, как интегрировать [Aspose.Cells для Python via .NET](https://products.aspose.com/cells/python-net/) как пользовательский движок Excel внутри библиотеки `pandas`, что позволяет парсить файлы `.xlsx`, `.xls` и др. с высокой точностью.

{{% /alert %}}

## Почему использовать Aspose.Cells для Python via .NET?

Aspose.Cells предлагает:

- Продвинутая поддержка Excel (формулы, диаграммы, форматирование, объединенные ячейки и т. д.)
- Поддержка множества форматов: `.xls`, `.xlsx`, `.xlsb`, `.ods`, `.csv`, `.html`
- Лучше точность для сложных таблиц по сравнению с `openpyxl` или `xlrd`
...
```
- `cd` в каталог исходников pandas, который вы только что создали с помощью команды clone
...
```
и затем добавьте следующий код в ` _engines: Mapping[str, Any]`
...
> ✅ Если при сборке возникнет ошибка компиляции, например:**Cython.Compiler.Errors.InternalError: Internal compiler error: 'free_threading_config.pxi' not found**, попробуйте выполнить следующую команду и затем повторно скомпилировать.
```bash
...
print(df)
```
После выполнения команды вы должны получить результат, похожий на этот.
...
- [Aspose.Cells для Python via .NET Документы](https://docs.aspose.com/cells/python-net/)
