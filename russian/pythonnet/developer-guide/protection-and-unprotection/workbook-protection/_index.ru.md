---
title: Защита и снятие защиты структуры рабочей книги
type: docs
weight: 40
url: /ru/python-net/protect-and-unprotect-workbook-structure/
description: Защита и снятие защиты структуры рабочей книги файлов Excel с использованием кодов на CSharp.
---


{{% alert color="primary" %}}
Чтобы предотвратить просмотр скрытых листов, добавление, перемещение, удаление или скрытие листов другими пользователями и переименование листов, вы можете защитить структуру своей рабочей книги Excel паролем.
{{% /alert %}}


## **Защита и снятие защиты структуры рабочей книги в MS Excel**

**![защита и снятие защиты структуры рабочей книги](protect-and-unprotect-workbook-structure.png)**

1. Нажмите **Обзор > Защитить книгу**.
1. Введите пароль в **поле Пароль**.
1. Выберите **OK**, введите пароль для подтверждения, затем снова выберите **OK**.


## **Защитить структуру рабочей книги с помощью Aspose.Cells for Python via .NET**
Для реализации защиты структуры рабочей книги Excel достаточно следующих простых строк кода.

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "Protection-and-unprotection-Protect-Workbook-Structure.py" >}}

## **Снять защиту структуры рабочей книги с помощью Aspose.Cells for Python via .NET**
Снять защиту структуры рабочей книги легко с помощью API Aspose.Cells for Python via .NET.

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "Protection-and-unprotection-Unprotect-Workbook-Structure.py" >}}

{{% alert color="primary" %}}
Примечание: требуется правильный пароль.
{{% /alert %}}

