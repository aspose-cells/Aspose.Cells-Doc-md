...
Этот сценарий начинается с той же структуры сводной таблицы, что и в сценарии 1, но поле `Amount` добавляется в область данных дважды. Оба поля данных ссылаются на один и тот же исходный столбец, однако для второго поля с помощью сеттера `PivotField.function` переопределяется функция итогов так, что оно становится `Count` вместо `Sum` по умолчанию.
```python
...
При наличии двух полей данных `PivotTable.values_field` становится доступным. В этом сценарии агрегатное виртуальное поле перетаскивается в область столбцов так, чтобы каждая мера из области данных отображалась в виде собственного блока столбцов рядом с `Year`.
```python
import aspose.cells as ac
...
## Связанные статьи
- [Поля строк и столбцов сводной таблицы в Aspose.Cells for Python via .NET](/cells/ru/python-net/row-and-column-fields/)
