...
На практике отдавайте предпочтение API на основе кэша, а не устаревшему методу `refresh_data()` для отдельных таблиц. Они учитывают общие кэши, избегают избыточных обращений к источнику и позволяют выбрать минимальный объём, удовлетворяющий вашим требованиям к обновлению.{{< app/cells/assistant language="python" >}}
