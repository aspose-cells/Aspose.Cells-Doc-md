...
type: docs
weight: 250
url: /ru/python-net/aspose-cells-gridjs/client/
keywords: GridJs, пользовательский, логотип, настройки, API, gridjs, python, редактирование, таблица, просмотр, просмотрщик, редактор, excel, js API, клиентский API
...
| `isCollaborative` | Поддерживать ли совместный режим. | `false` | Да |
...
| `token` | Установить токен аутентификации. Когда токен не null, заголовок `Authorization: Bearer {token}` будет автоматически добавлен к заголовкам запроса. Можно использовать `xs.refreshToken(token)` для установки нового токена. | Нет | Да | 
| `showBottombarStats` | Показывать вкладку статистики в нижней панели.<br>Значение по умолчанию — true. | `true` | Да |   
| `showRowAppenderToolbar` | Показывать панель инструментов для массового добавления строк.<br>Значение по умолчанию — true. | `true` | Да |   
...
```
- установить пользовательский тост
```javascript
...
```
- установить вызов функции перед сохранением 
```javascript
...
- настроить совместную работу в совместном режиме, убедитесь, что установили setCollaborativeSetting перед setUniqueId  
```javascript
...
- Установить редактируемый/только для чтения диапазон
```javascript
...
-  Установить стиль для нужной области ячеек
```javascript
...
```
-  Удалить содержимое ячейки или очистить стиль в выбранной ячейке  
```javascript
...
-  Удалить содержимое ячейки или очистить стиль в нужной области ячеек
```javascript
...
```
## API для объекта фигуры
-  Изменить цвет фона объекта фигуры
```javascript
...
-  Применить настройки шрифта к текстовому объекту
```javascript
