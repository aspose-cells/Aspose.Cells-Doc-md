...
type: docs
weight: 10
url: /ru/nodejs-cpp/how-to-format-number-to-accounting/
description: В этой статье рассказывается о том, как форматировать число в бухгалтерский учет с помощью API Aspose.Cells for Node.js via C++.
...
## ** Как форматировать число в бухгалтерский учет в Aspose.Cells for Node.js via C++**
Для форматирования чисел в бухгалтерский учет в Aspose.Cells for Node.js via C++ можно использовать объект `Style`, связанный с ячейкой или диапазоном ячеек. Объект `Style` позволяет задавать различные параметры форматирования, включая форматы чисел. Формат бухгалтерского учета обычно имеет код формата, который может различаться в зависимости от требований (например, отображение символов валют, десятичных знаков и т.д.).

Вот пример, как применить формат числа в бухгалтерский учет к ячейке в Aspose.Cells for Node.js via C++:

1. **Ссылки на Aspose.Cells**: Убедитесь, что у вас подключена библиотека Aspose.Cells for Node.js via C++ к проекту. Ее можно скачать с сайта Aspose.
...
Этот пример показывает, как оформить отдельную ячейку так, чтобы числа отображались в формате учета с долларами США. Строка формата может быть скорректирована для отображения других символов валют или форматов учета по необходимости. Главное — метод `style.setCustom`, где вы указываете пользовательский код формата чисел для учета.
