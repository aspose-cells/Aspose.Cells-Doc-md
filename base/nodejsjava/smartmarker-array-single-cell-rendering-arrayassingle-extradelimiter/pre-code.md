---
title: SmartMarker Single Cell Array Rendering | Aspose.Cells for Node.js via Java
description: Learn how to render array data into a single cell using the ArrayAsSingle and ExtraDelimiter attributes in Smart Markers with Aspose.Cells for Node.js via Java.
...
type: docs
weight: 195
url: /nodejs-java/smartmarker-array-single-cell-rendering-arrayassingle-extradelimiter/
---
...
<!-- CODE_BLOCK:0:Create a JavaScript example that demonstrates rendering a string array (such as a Tags property containing values like "JavaScript", "Aspose", "SmartMarker", "Excel") into a single cell. The code should: (1) Define a sample data class named Product with a string array property called Tags. (2) Create an instance of Product and populate the Tags array with 4-5 string values. (3) Create a new Workbook and obtain the first Worksheet. (4) Set a header cell such as "Tags" in the worksheet. (5) In the cell below the header, write a Smart Marker string that uses &=Product.Tags(arrayasSingle=true, extraDelimiter=", "). (6) Instantiate a WorkbookDesigner, assign the workbook, call setDataSource("Product", productInstance), then call process(). (7) Save the workbook to a file such as output_arraySingle.xlsx. Expected output: the cell under "Tags" should contain a single concatenated string like "JavaScript, Aspose, SmartMarker, Excel" rather than four separate cells. APIs to use: Workbook, Worksheet, Cells, WorkbookDesigner, WorkbookDesigner.setDataSource(string, object), WorkbookDesigner.process(). -->

### **Code Example 2 — Numeric Array with Custom Delimiter**

<!-- CODE_BLOCK:1:Create a JavaScript example that uses a numeric array (such as int[] Scores = { 95, 88, 76, 100, 67 }) and renders it into one cell using a hyphen-based delimiter. The code should: (1) Define a data class named Student with an int[] Scores property. (2) Create a new Workbook and access the first worksheet. (3) Place a header label such as "Scores" in one cell and the Smart Marker &=Student.Scores(arrayasSingle=true, extraDelimiter=" - ") in the cell directly below. (4) Use WorkbookDesigner with setDataSource to bind the data, then call process(). (5) Save the output as output_numericArray.xlsx. Expected output: the cell displays a single concatenated string such as "95 - 88 - 76 - 100 - 67". APIs to use: Workbook, Worksheet, WorkbookDesigner, WorkbookDesigner.setDataSource, WorkbookDesigner.process. -->

### **Code Example 3 — Comparing Default vs. ArrayAsSingle Behavior**

<!-- CODE_BLOCK:2:Create a JavaScript example that demonstrates the difference between the default spreading behavior and the new single-cell rendering using the same data source. The code should: (1) Define a data class named Order with a string[] Items property containing several item names. (2) Create a Workbook and use the first worksheet. (3) In one section of the sheet, place the default Smart Marker &=Order.Items (no attributes) so that values spread horizontally across cells. (4) In another section of the same sheet, place the attribute-enriched marker &=Order.Items(arrayasSingle=true, extraDelimiter="; ") so that all values collapse into one cell. (5) Bind the data via WorkbookDesigner and call process(). (6) Save the file as output_comparison.xlsx. Expected output: the first marker populates multiple cells with individual array values, while the second marker collapses all values into a single cell separated by semicolons. APIs to use: Workbook, Worksheet, Cells, WorkbookDesigner, WorkbookDesigner.setDataSource, WorkbookDesigner.process. -->
