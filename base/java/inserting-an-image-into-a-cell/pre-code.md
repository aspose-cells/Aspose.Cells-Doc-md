...
description: Aspose.Cells is a Java library for working with spreadsheet files. This article explains how to fit a picture exactly to a single cell size using two different approaches: placing a floating picture over the cell, or embedding the image directly into the cell.
...
type: docs
weight: 80
url: /java/inserting-an-image-into-a-cell/
---
...
- **Approach 1 — Place a floating picture over a cell.** Add a `Picture` to the worksheet, set its `Placement` to `MOVE_AND_SIZE`, and adjust its anchor cells (`getUpperLeftRow`, `getUpperLeftColumn`, `getLowerRightRow`, `getLowerRightColumn`) so the picture covers exactly one cell.
- **Approach 2 — Embed an image directly in a cell.** Assign image bytes to the cell's `getEmbeddedImage()` setter. The image automatically scales to fit the cell's display area and travels with the cell.
...
- `Picture.getUpperLeftRow()` — the row index of the picture's top edge.
- `Picture.getUpperLeftColumn()` — the column index of the picture's left edge.
- `Picture.getLowerRightRow()` — the row index of the picture's bottom edge. To make the picture's bottom edge sit at the bottom of row `r`, set this to `r + 1`.
- `Picture.getLowerRightColumn()` — the column index of the picture's right edge. To make the picture's right edge sit at the right of column `c`, set this to `c + 1`.
...
`Picture.getPlacement()` returns an enum of type `PlacementType` that controls how the picture behaves when the user resizes the row or column beneath it. The recommended value for a single-cell picture is `PlacementType.MOVE_AND_SIZE`, which causes the picture to move and resize together with its underlying cell, preserving the exact fit.

### **Step-by-Step Instructions**

1. Create a new `Workbook` (or open an existing one).
2. Access the target `Worksheet` from `workbook.getWorksheets().get(0)`.
3. Open the image file from disk into an `InputStream` (such as a `FileInputStream`) using a try-with-resources block so the stream is closed properly.
...
<!-- CODE_BLOCK:0:Fit a floating picture exactly to a single cell (C6) by adding it via Worksheet.getPictures().add(row, column, stream), then setting its four anchor properties (setUpperLeftRow, setUpperLeftColumn, setLowerRightRow, setLowerRightColumn) so the picture's bounding rectangle covers exactly one cell. Set picture.setPlacement(PlacementType.MOVE_AND_SIZE) so the picture resizes with the cell when column widths or row heights change. Save the resulting workbook as an .xlsx file. Input: an image file on disk (e.g., logo.png). Output: an .xlsx file in which cell C6 is visually filled by the picture, with the picture staying aligned to C6 if the user resizes the column or row. -->

## **Approach 2: Embed an Image Directly in a Cell**

Aspose.Cells also exposes a simpler mechanism for cell-bound images: the `Cell.setEmbeddedImage(byte[])` method. Assigning image bytes to this property attaches the image to the cell itself, as if it were inline content.
...
This makes `setEmbeddedImage()` the most concise option when your goal is simply "an image that lives inside this cell."

### **Step-by-Step Instructions**

1. Create a new `Workbook` (or open an existing one).
2. Access the target `Worksheet` from `workbook.getWorksheets().get(0)`.
3. Read the image file from disk into a `byte[]` array (for example, by reading the file via `Files.readAllBytes()` from `java.nio.file`).
4. Get a reference to the target cell — either through `worksheet.getCells().get("C6")` or `worksheet.getCells().get(5, 2)`.
5. Assign the byte array to the cell using `cell.setEmbeddedImage(bytes)`.
...
<!-- CODE_BLOCK:1:Embed an image directly into a single cell (C6) by reading the image into a byte array (e.g., via Files.readAllBytes()) and assigning it to the cell using setEmbeddedImage(byte[]). No anchor coordinates, placement settings, or PictureCollection calls are needed. Optionally adjust the row height and column width of the target row and column so the embedded image is more visible. Save the resulting workbook as an .xlsx file. Input: an image file on disk (e.g., logo.png). Output: an .xlsx file in which cell C6 contains an embedded image that automatically fills the cell's display area. -->
