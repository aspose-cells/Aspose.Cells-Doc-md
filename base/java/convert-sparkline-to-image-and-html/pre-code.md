...
<!-- CODE_BLOCK:0:Create a new Workbook and populate cells A1:E1 with five numeric sample values, then add three SparklineGroup entries to the first worksheet by calling worksheet.getSparklineGroups().add(...): a Line group anchored at F1, a Column group anchored at G1, and a Stacked (win/loss) group anchored at H1, all referencing the A1:E1 data range. Build an ImageOrPrintOptions instance and call setImageType(ImageType.PNG) so each sparkline is rendered as a transparent PNG, then iterate the three groups and call Sparkline.toImage(ByteArrayOutputStream, ImageOrPrintOptions) on group.getSparklines().get(0) for each, convert each ByteArrayOutputStream to a byte array, and assign the bytes via cell.setEmbeddedImage(byte[]) on cells F2, G2, and H2 (accessed through worksheet.getCells().get("F2"), worksheet.getCells().get("G2"), and worksheet.getCells().get("H2")). Finally, call workbook.save("output_with_sparklines.xlsx") to write the workbook to disk using a standard xlsx save format. The code should include the necessary import statements for java.io, com.aspose.cells, com.aspose.cells.charts, and com.aspose.cells.drawing. -->
...
<!-- CODE_BLOCK:1:Load the previously saved "output_with_sparklines.xlsx" file into a new Workbook instance, instantiate an HtmlSaveOptions object, call htmlOptions.setExportActiveWorksheetOnly(true) so that only the currently active worksheet is exported, and then call workbook.save("sparklines.html", htmlOptions) to write the sparkline-rich worksheet to disk as a self-contained HTML file. The code should include the necessary import statements for java.io and com.aspose.cells. -->
