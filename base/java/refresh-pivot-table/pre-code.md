---
title: Refreshing Pivot Tables in Aspose.Cells for Java
...
<!-- CODE_BLOCK:4:A complete, self-contained Java class with a public static void main(String[] args) method that starts with import java.lang.System; import com.aspose.cells.Workbook; and import com.aspose.cells.pivot.*; creates a new Workbook and gets workbook.getWorksheets().get(0); writes a Fruit/Year/Amount header row into A1:C1 and approximately 9 data rows into A2:C10; adds the first pivot table named Pivot1 at destination cell E3 via worksheet.getPivotTables().add("A1:C9", "E3", "Pivot1"); assigns Fruit to the Row area, Year to the Column area, and Amount to the Data area; adds a second pivot table named Pivot2 at destination cell E15 via worksheet.getPivotTables().add("A1:C9", "E15", "Pivot2") using the same source range; assigns the same Fruit/Year/Amount fields to Pivot2; demonstrates shared-cache equality by writing to console (or asserting) that pivotTable1.getPivotCache() == pivotTable2.getPivotCache() evaluates to true; calls pivotTable1.getPivotCache().getPivotTables() to retrieve the collection of pivot tables sharing this cache; iterates the returned collection and demonstrates that it contains BOTH Pivot1 and Pivot2 (for example, by writing each pivot table's name to console); calls workbook.save("output.xlsx"); the code must compile against Aspose.Cells 26.7.0 -->
...
- [Reading and Writing DBF Files](/cells/java/dbf/)
