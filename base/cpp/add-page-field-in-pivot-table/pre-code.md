...
description: Learn how to add and configure page fields in pivot tables using Aspose.Cells for C++, including adding page fields, single-select filtering, and multi-select filtering.
...
Aspose.Cells for Aspose.Cells for C++ exposes two equivalent ways to register a page field. The high-level API is `PivotTable.AddFieldToArea(PivotFieldType.Page, "fieldName")`, which takes the source-column name and adds the field in a single call. The lower-level API is `PivotTable.PageFields.Add(PivotField)`, which is used when you already hold a `PivotField` reference and want to add the same field instance to the page area. Both APIs end up populating the same `PageFields` collection, and the remainder of this article demonstrates how to choose between them and how to drive each filtering mode.
...
<!-- CODE_BLOCK:1:Reference the same pivot construction as in Scenario 1a, obtain the Year PivotField from the BaseFields collection, pass that PivotField instance to pivotTable.PageFields.Add, refresh the pivot, and save the file -->
...
<!-- CODE_BLOCK:2:Reference the same pivot construction as in Scenario 1a, set pivotTable.PageFields[0].CurrentPageItem to the special sentinel 0x7FFD with an inline comment explaining that this value represents all items, then save the workbook -->
...
<!-- CODE_BLOCK:3:Reference the same pivot construction as in Scenario 1a, set pivotTable.PageFields[0].CurrentPageItem to the integer 1 with an inline comment noting that 1 corresponds to the second sorted item, then save the workbook -->
...
<!-- CODE_BLOCK:4:Reference the same pivot construction as in Scenario 1a, enable multi-select by setting pivotTable.PageFields[0].IsMultipleItemSelectionAllowed to true, retrieve the PivotItems collection, then run Part A loop that sets IsHidden to false on every item, followed by Part B loop that uses a switch on GetStringValue to whitelist specific source values (for example 2020, grape, and blueberry) and hides all other items, then refresh and save the workbook -->
