...
description: Aspose.Cells is a C++ library for working with spreadsheet files, which supports splitting a single Excel file into multiple files. This article will introduce how to split Excel files by copying each worksheet to a separate workbook and by copying specific cell ranges to other workbooks.
keywords: Aspose.Cells, C++ library, spreadsheet, split Excel file, copy worksheet, copy range, multiple workbooks, save as separate files
type: docs
weight: 195
url: /cpp/splitting-excel-files-into-multiple-files/
---
...
<!-- CODE_BLOCK:0:Open an existing Excel file (book1.xls) from the data directory by constructing a Workbook object with the file path (value-based style, e.g., Workbook srcWorkbook(book1Path);), iterate through every Worksheet in its Worksheets collection using a for loop with GetCount() and Get(index), and for each worksheet create a new destination Workbook using the default constructor (Workbook destWorkbook;), access or add a worksheet named after the source sheet via GetWorksheets(), copy the source worksheet's cell content (values, and optionally formatting) into the destination worksheet using destSheet.GetCells().Copy(srcSheet.GetCells()) or by iterating with PutValue, then save the destination workbook to a file whose name is derived from the source worksheet's name in the data directory using destWorkbook.Save(outputPath). The expected output is one .xls file per source worksheet, each containing the data of a single sheet. -->
...
<!-- CODE_BLOCK:1:Open an existing Excel file from the data directory by constructing a Workbook object with the file path (value-based style, e.g., Workbook srcWorkbook(srcPath);), retrieve the first Worksheet via srcWorkbook.GetWorksheets().Get(0), define or obtain the source cell range (for example A1:C10 using srcSheet.GetCells().CreateRange(u"A1:C10") or a named range) to be extracted, then create a new destination Workbook using the default constructor (Workbook destWorkbook;), access its first worksheet via destWorkbook.GetWorksheets().Get(0), and use destSheet.GetCells().Copy(sourceRange) to copy the source range into the destination worksheet starting at cell A1. Finally, save the destination workbook to a new file such as outputrange.xls in the data directory using destWorkbook.Save(outputPath). The expected output is a single new .xls file that contains only the specified range of data from the source workbook, with no other content from the source file. -->
...
- [Copying Rows and Columns](/cells/cpp/copying-rows-and-columns/)
