...
<!-- CODE_BLOCK:0:Fit a floating picture exactly to a single cell (C6) by adding it via Worksheet.getPictures().add(row, column, stream), then setting its four anchor properties (setUpperLeftRow, setUpperLeftColumn, setLowerRightRow, setLowerRightColumn) so the picture's bounding rectangle covers exactly one cell. Set Picture.setPlacement to PlacementType.MOVE_AND_SIZE so the picture resizes with the cell when column widths or row heights change. Save the resulting workbook as an .xlsx file. Input: an image file on disk (e.g., logo.png). Output: an .xlsx file in which cell C6 is visually filled by the picture, with the picture staying aligned to C6 if the user resizes the column or row. -->
