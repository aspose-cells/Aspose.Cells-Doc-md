...
type: docs
weight: 220
url: /python-java/pivot-table-row-and-column-fields/
---
...
After fields are added, you can access them through the `PivotTable.getRowFields()` and `PivotTable.getColumnFields()` methods. Each method returns a `PivotFieldCollection`. The field at index 0 of `RowFields` is the outermost row field, and subsequent indices represent fields nested inside it. The same indexing convention applies to `ColumnFields`.
...
<!-- CODE_BLOCK:0:Complete top-level Python program that imports the necessary Aspose.Cells classes (for example from aspose.cells import Workbook, PivotFieldType, PivotFieldSubtotalType), creates a new Workbook, gets Worksheets[0] and renames it to "Data" via setName("Data"), writes a header row at A1:D1 (Category, Item, Year, Amount) and eight data rows at A2:D9 using cell.putValue(value) with the shared dataset (Fruit/Apple with year 2020 amount 100, Fruit/Apple 2021 150, Fruit/Banana 2020 80, Fruit/Banana 2021 90, Vegetable/Carrot 2020 50, Vegetable/Carrot 2021 60, Vegetable/Daikon 2020 40, Vegetable/Daikon 2021 45) where Year and Amount are ints and Category and Item are strings, adds a pivot table covering A1:D9 placed at cell F3 named "PivotTable1" via worksheet.getPivotTables().add(...), adds Category to the Row region first, then Item to the Row region second (so Category is outer and Item is inner), adds Year to the Column region, adds Amount to the Data region (all four via pivotTable.addFieldToArea(PivotFieldType.X, "name")), retrieves the outer row field via PivotField categoryField = pivotTable.getRowFields()[0], explicitly calls categoryField.setSubtotals(PivotFieldSubtotalType.AUTOMATIC, True) using the 2-arg overload, then calls pivotTable.refreshData() and pivotTable.calculateData() and saves the workbook as output_automatic.xlsx. Expected output: the pivot renders a default subtotal row for each Category group (Fruit and Vegetable) between the inner Item rows and the next Category. -->
