...
`setAutoFormatType` is only honored when the workbook is saved as `.xls`. When the same workbook is saved as `.xlsx`, `.xlsm`, or `.xlsb`, Excel ignores this setting and falls back to the `setPivotTableStyleType` and `setPivotTableStyleName` settings.
...
<!-- CODE_BLOCK:0:Apply a legacy XLS preset autoformat using pivotTable.setAutoFormatType(PivotTableAutoFormatType.REPORT_5), then save as .xls. The code imports the required classes (com.aspose.cells.Workbook, com.aspose.cells.pivot.PivotTableAutoFormatType, com.aspose.cells.pivot.PivotFieldType), creates a new Workbook, populates cells A1:C10 with a header row (Fruit, Year, Amount) plus 9 data rows containing grape/blueberry/kiwi/cherry across years 2020 and 2021 with sample amounts, adds a pivot table at E3 named "Pivot1" with source "A1:C10", assigns Fruit to the Row area, Year to the Column area, Amount to the Data area, calls pivotTable.setAutoFormatType(PivotTableAutoFormatType.REPORT_5), and calls workbook.save("output.xls"). The code must include a leading comment indicating the scenario, the API in use, and the target format, plus the GitHub reference comment line "For complete examples and data files, please go to https://github.com/aspose-cells/Aspose.Cells-for-Python-via-Java". -->

## **Apply a Modern Named Preset Pivot Table Style**

The `setPivotTableStyleType` method on a pivot table accepts a value from the `com.aspose.cells.PivotTableStyleType` enumeration. The enumeration covers light themes `PIVOT_TABLE_STYLE_LIGHT_1` through `PIVOT_TABLE_STYLE_LIGHT_28` and dark themes `PIVOT_TABLE_STYLE_DARK_1` through `PIVOT_TABLE_STYLE_DARK_28`. The styles added in Excel 2017 (the second wave of light and dark themes) are reachable through the same enumeration.
...
The following example uses the same Fruit/Year/Amount data, creates an identical pivot table, applies `PivotTableStyleType.PIVOT_TABLE_STYLE_DARK_1`, and saves the workbook as `.xlsx`.

<!-- CODE_BLOCK:1:Apply a modern Excel 2007+ named preset style using pivotTable.setPivotTableStyleType(PivotTableStyleType.PIVOT_TABLE_STYLE_DARK_1), then save as .xlsx. The code imports the required classes (com.aspose.cells.Workbook, com.aspose.cells.PivotTableStyleType, com.aspose.cells.pivot.PivotFieldType), creates a new Workbook, populates cells A1:C10 with the identical Fruit/Year/Amount sample data (header row plus 9 data rows), adds a pivot table at E3 named "Pivot1" with source "A1:C10", assigns Fruit to the Row area, Year to the Column area, Amount to the Data area, calls pivotTable.setPivotTableStyleType(PivotTableStyleType.PIVOT_TABLE_STYLE_DARK_1) (note this enum is in the com.aspose.cells namespace, not com.aspose.cells.pivot), and calls workbook.save("output.xlsx"). The code must include a leading comment indicating the scenario, the API in use, and the target format, plus the GitHub reference comment line. -->
...
<!-- CODE_BLOCK:2:Define a custom pivot table style with addPivotTableStyle, add WHOLE_TABLE and GRAND_TOTAL_ROW elements with setElementStyle, apply via setPivotTableStyleName, then save as .xlsx. The code imports the required classes (com.aspose.cells.Workbook, com.aspose.cells.Style, com.aspose.cells.BorderType, com.aspose.cells.CellBorderType, com.aspose.cells.TableStyleElementType, com.aspose.cells.pivot.PivotFieldType, java.awt.Color), creates a new Workbook, populates cells A1:C10 with the Fruit/Year/Amount sample data (header row plus 9 data rows), adds a pivot table at E3 named "Pivot1" with source "A1:C10", assigns Fruit to the Row area, Year to the Column area, Amount to the Data area. It then calls workbook.getWorksheets().getTableStyles().addPivotTableStyle("CustomPivotStyle") to add a custom style and captures the returned index. It retrieves the TableStyle from workbook.getWorksheets().getTableStyles().get(styleIndex), adds a WHOLE_TABLE element via tableStyle.getTableStyleElements().add(TableStyleElementType.WHOLE_TABLE) and captures its index, creates a Style via workbook.createStyle(), configures thin black borders on all four sides (Top, Bottom, Left, Right) using style.getBorders().getByType(BorderType.TOP_BORDER).setColor(Color.BLACK) and CellBorderType.THIN for each, and calls setElementStyle(style) on the WHOLE_TABLE element. It then adds a GRAND_TOTAL_ROW element similarly, creates another Style, calls style.getFont().setBold(true) and style.getFont().setColor(Color.RED), and calls setElementStyle(style) on the GRAND_TOTAL_ROW element. Finally it calls pivotTable.setPivotTableStyleName("CustomPivotStyle") (not setPivotTableStyleType) and calls workbook.save("output.xlsx"). The code must include a leading comment indicating the scenario, the API in use, and the target format, plus the GitHub reference comment line. -->
...
<!-- CODE_BLOCK:3:Apply a single Style to every pivot table cell using pivotTable.formatAll, then save as .xlsx. The code imports the required classes (com.aspose.cells.Workbook, com.aspose.cells.Style, com.aspose.cells.BorderType, com.aspose.cells.CellBorderType, com.aspose.cells.BackgroundType, com.aspose.cells.pivot.PivotFieldType, java.awt.Color), creates a new Workbook, populates cells A1:C10 with the Fruit/Year/Amount sample data (header row plus 9 data rows), adds a pivot table at E3 named "Pivot1" with source "A1:C10", assigns Fruit to the Row area, Year to the Column area, Amount to the Data area. It then creates a Style via workbook.createStyle(), configures a yellow solid fill via style.setForegroundColor(Color.YELLOW) and style.setPattern(BackgroundType.SOLID), sets a bold dark-blue font via style.getFont().setBold(true) and style.getFont().setColor(Color.DARK_BLUE), and adds thin black borders on all four sides (Top, Bottom, Left, Right) using CellBorderType.THIN. It then calls pivotTable.formatAll(style) to override any prior style settings, and calls workbook.save("output.xlsx"). The code must include a leading comment indicating the scenario, the API in use, and the target format, plus the GitHub reference comment line. -->
...
- [Refreshing Pivot Tables in Aspose.Cells for Aspose.Cells for Python via Java](/cells/python-java/refresh-pivot-table/)
