...
description: Aspose.Cells is a .NET library for working with spreadsheet files. This article explains how to fit a picture exactly to a single cell size using two different approaches: placing a floating picture over the cell, or embedding the image directly into the cell.
...
type: docs
weight: 80
url: /net/inserting-an-image-into-a-cell/
---
...
1. Add the picture using `Worksheet.Pictures.Add(int row, int column, Stream stream)`, which anchors the new picture to the given cell.
...
3. Open the image file from disk into a `FileStream` using a `using` block so the stream is disposed properly.
...
<!-- CODE_BLOCK:0:Fit a floating picture exactly to a single cell (C6) by adding it via Worksheet.Pictures.Add(row, column, stream), then setting its four anchor properties (UpperLeftRow, UpperLeftColumn, LowerRightRow, LowerRightColumn) so the picture's bounding rectangle covers exactly one cell. Set Picture.Placement to PlacementType.MoveAndSize so the picture resizes with the cell when column widths or row heights change. Save the resulting workbook as an .xlsx file. Input: an image file on disk (e.g., logo.png). Output: an .xlsx file in which cell C6 is visually filled by the picture, with the picture staying aligned to C6 if the user resizes the column or row. -->
...
1. Create a new `Workbook` (or open an existing one).
2. Access the target `Worksheet` from `workbook.Worksheets[0]`.
3. Read the image file from disk into a `byte[]` array (for example, by using `File.ReadAllBytes`).
...
- [How to Insert Picture in Cell](/cells/net/how-to-place-image-to-cell/)
- [How to Fit Image to Cell Width and Height](/cells/net/how-to-fit-image-to-cell-width-height/)
