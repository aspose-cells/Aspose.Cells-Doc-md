...
description: Aspose.Cells is a Python library for working with spreadsheet files. This article explains how to fit a picture exactly to a single cell size using two different approaches: placing a floating picture over the cell, or embedding the image directly into the cell.
...
<!-- CODE_BLOCK:0:Fit a floating picture exactly to a single cell (C6) by adding it via Worksheet.pictures.add(row, column, stream), then setting its four anchor properties (upper_left_row, upper_left_column, lower_right_row, lower_right_column) so the picture's bounding rectangle covers exactly one cell. Set Picture.placement to PlacementType.MOVE_AND_SIZE so the picture resizes with the cell when column widths or row heights change. Save the resulting workbook as an .xlsx file. Input: an image file on disk (e.g., logo.png). Output: an .xlsx file in which cell C6 is visually filled by the picture, with the picture staying aligned to C6 if the user resizes the column or row. -->
