...
description: Aspose.Cells is a Python via .NET library for working with spreadsheet files, which supports splitting a single Excel file into multiple files. This article will introduce how to split Excel files by copying each worksheet to a separate workbook and by copying specific cell ranges to other workbooks.
keywords: Aspose.Cells, Python via .NET library, spreadsheet, split Excel file, copy worksheet, copy range, multiple workbooks, save as separate files
type: docs
weight: 195
url: /python-net/splitting-excel-files-into-multiple-files/
---
...
2. Iterate through the `Workbook.worksheets` collection using a `for` loop so that every `Worksheet` in the source file is processed.
...
6. Construct an output file path that incorporates the source worksheet's name (for example, `dataDir + worksheet.name + ".xls"`) so that each generated file has a unique name.
...
<!-- CODE_BLOCK:0:Open an existing Excel file (book1.xls) from the data directory using the Workbook class, iterate through every Worksheet in its worksheets collection, and for each worksheet create a new Workbook, add a new worksheet named after the source sheet, copy the source worksheet's cell content (values, and optionally formatting) into the destination worksheet using the Cells.copy method or by iterating with put_value, then save the destination workbook to a file whose name is derived from the source worksheet's name in the data directory. The expected output is one .xls file per source worksheet, each containing the data of a single sheet. -->
...
3. Identify the range to be copied. This can be a hard-coded cell range such as `A1:C10`, or a named range obtained through the `Worksheet.cells` collection, or a range created via `Worksheet.cells.create_range`.
4. Create a new destination `Workbook` instance.
5. Access the first `Worksheet` of the destination workbook (the default sheet).
6. Copy the source range into the destination worksheet, typically starting from cell `A1`. The `Cells.copy` method on the destination `Cells` collection can be used to copy an entire range, or you can iterate through the source range's cells and write their values into the destination cells with `put_value`. Optional `CopyOptions` may be supplied to control what is transferred (values only, values and styles, formulas, and so on).
7. Save the destination workbook to a new file path on disk using the `Workbook.save` method.

### **Code Example**

<!-- CODE_BLOCK:1:Open an existing Excel file from the data directory using the Workbook class, retrieve the first Worksheet, define or obtain the source cell range (for example A1:C10 or a named range) to be extracted, then create a new destination Workbook, access its first worksheet, and use the Cells.copy method to copy the source range into the destination worksheet starting at cell A1. Finally, save the destination workbook to a new file such as outputrange.xls in the data directory. The expected output is a single new .xls file that contains only the specified range of data from the source workbook, with no other content from the source file. -->
