##Json
Learn how to convert excel workbook to json with Aspose.Cells for Python via .NET API.
Aspose.Cells for Python via .NET supports converting a workbook to Json(JavaScript Object Notation) file.
## **Convert Excel Workbook to JSON**
The Aspose.Cells for Python via .NET API provides support for converting spreadsheets to JSON format. To export the workbook to JSON, pass [**SaveFormat.JSON**](https://reference.aspose.com/cells/python-net/aspose.cells/saveformat) as the second parameter of [**Workbook.save**](https://reference.aspose.com/cells/python-net/aspose.cells/workbook/save/#str-aspose.cells.SaveOptions) method. You may also use [**JsonSaveOptions**](https://reference.aspose.com/cells/python-net/aspose.cells/JsonSaveoptions) class to specify additional settings for exporting worksheet to JSON.
The following code example demonstrates exporting active worksheet to Json by using [**SaveFormat.JSON**](https://reference.aspose.com/cells/python-net/aspose.cells/saveformat) enumeration member. Please see the code to convert [source file](book1.xlsx) to the [output Json file](book1.Json) generated by the code for reference.
## **Advance topics**
- [Convert CSV to JSON](https://docs.aspose.com/cells/python-net/convert-csv-to-json/)
- [Convert-Excel-to-JSON](https://docs.aspose.com/cells/python-net/convert-excel-to-json/)
- [Convert JSON to CSV](https://docs.aspose.com/cells/python-net/convert-json-to-csv/)
- [Convert-JSON-to-Excel](https://docs.aspose.com/cells/python-net/convert-json-to-excel/)
