##Convert Text Numeric Data to Number with Golang via C++
Learn how to convert numbers stored as text in Excel to numbers by using the Aspose.Cells for C++ API.
...
Aspose.Cells provides the [**Cells.ConvertStringToNumericValue()**](https://reference.aspose.com/cells/go-cpp/cells/convertstringtonumericvalue/) method which can be used to convert all string or text numeric data into numbers.
The following screenshot shows string numbers in cells **A1:A17**. String numbers are aligned to the left.
These string numbers have been converted to numbers using [**Cells.ConvertStringToNumericValue()**](https://reference.aspose.com/cells/go-cpp/cells/convertstringtonumericvalue/) in the following screenshot. As you can see, they are now right-aligned.
