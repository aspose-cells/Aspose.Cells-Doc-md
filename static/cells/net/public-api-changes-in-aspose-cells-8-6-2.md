##Public API Changes in Aspose.Cells 8.6.2
...
Aspose.Cells for .NET 8.6.2 has exposed the Chart.ToPdf method that can be used to [directly render the Chart shape to PDF format](https://docs.aspose.com/cells/net/convert-an-excel-chart-to-image/). The said method currently accepts a parameter of of type string as file path location to store the resultant file on disk.
...
Aspose.Cells for .NET 8.6.2 has exposed the Workbook.RemoveUnusedStyles method that can be used to [remove all unused Style objects from the pool of styles](https://docs.aspose.com/cells/net/remove-unused-styles-inside-the-workbook/).
Following is the simple usage scenario.
**C#**
//Load spreadsheet
Workbook workbook = new Workbook(inputFilePath);
