...
description: Aprenda a añadir y configurar campos de página en tablas dinámicas con Aspose.Cells for Node.js via Java, incluyendo cómo añadir campos de página, filtrado de selección única y filtrado de selección múltiple.
keywords: Aspose.Cells, Node.js via Java, tabla dinámica, campo de página, PivotFieldType.Page, PageFields, IsMultipleItemSelectionAllowed, CurrentPageItem, PivotItem, IsHidden, filtro
type: docs
weight: 250
url: /es/nodejs-java/add-page-field-in-pivot-table/
ai_search_scope: cells_nodejsjava
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
---

{{% alert color="primary" %}}
Aspose.Cells admite el ciclo de vida completo de los campos de página en tablas dinámicas. Puede añadir un campo de página mediante una API de alto nivel o mediante la colección de bajo nivel `PageFields`, y puede controlar el filtro de página en modo de selección única, borrarlo para mostrar todos los elementos de página, o cambiar el campo a selección múltiple para que los usuarios puedan elegir varios elementos de página a la vez a través de la interfaz de casillas de verificación en Excel.
{{% /alert %}}

## **Introducción**

Un campo de página es un campo dinámico que controla *qué subconjunto* de los datos de origen se muestra en el cuerpo de la tabla dinámica. Los usuarios finales lo ven como un desplegable en la parte superior de una tabla dinámica renderizada en Excel, y al seleccionar uno de los elementos de página disponibles, el cuerpo de la tabla dinámica se reconstruye de modo que solo se resumen los registros que pertenecen a ese elemento de página. Un campo dinámico se convierte en un campo de página cuando se registra como `PivotFieldType.Page` en lugar de `PivotFieldType.Row`, `PivotFieldType.Column` o `PivotFieldType.Data`.
...
Aspose.Cells for Node.js via Java expone dos formas equivalentes para registrar un campo de página. La API de alto nivel es `pivotTable.addFieldToArea(PivotFieldType.Page, "fieldName")`, que toma el nombre de la columna de origen y añade el campo en una sola llamada. La API de bajo nivel es `pivotTable.getPageFields().add(PivotField)`, que se utiliza cuando ya tiene una referencia a `PivotField` y desea añadir la misma instancia de campo al área de página. Ambas API terminan rellenando la misma colección `PageFields`, y el resto de este artículo demuestra cómo elegir entre ellas y cómo controlar cada modo de filtrado.

## **Añadir un campo de página**

Hay dos formas de registrar un campo dinámico en el área de página. La llamada de alto nivel toma el nombre de la columna de origen como una cadena y es la ruta más común. La llamada de bajo nivel acepta una instancia existente de `PivotField` y es conveniente cuando la misma instancia de campo debe reutilizarse en múltiples áreas de la tabla dinámica. Ambas llamadas colocan el campo en `pivotTable.getPageFields()`, tras lo cual aparece como el desplegable de página en la parte superior de la tabla dinámica renderizada.

### Añadir un campo de página con addFieldToArea

El siguiente ejemplo construye un pequeño conjunto de datos de Fruta / Año / Cantidad, coloca una tabla dinámica en la celda E3 con `Fruit` en el área de filas, `Amount` en el área de datos y `Year` en el área de página, actualiza la tabla dinámica y guarda el libro de trabajo.
...
### Añadir un campo de página con getPageFields().add

Cuando ya trabaja con una instancia de `PivotField`, puede pasarla directamente a `pivotTable.getPageFields().add`. La tabla dinámica y el campo de página se construyen exactamente como en el escenario anterior; solo se reemplaza el registro final del área de página con la llamada a la API de bajo nivel.
...
// Enfoque de bajo nivel: obtener el PivotField Year existente de BaseFields
// y registrarlo en el área Page mediante PageFields.Add(PivotField).
...
En el comportamiento predeterminado de selección única, el campo de página se renderiza como un único desplegable y el entero `PivotField.CurrentPageItem` selecciona qué elemento de página controla el cuerpo de la tabla dinámica. Asignar un índice específico selecciona ese único elemento; asignar el centinela especial `0x7FFD` (decimal 32765) borra el filtro para que todos los elementos de página se resuman a la vez. La selección única es el modo predeterminado; no necesita habilitarla explícitamente.

### Mostrar todos los elementos

Establecer `CurrentPageItem` al valor mágico `0x7FFD` equivale a borrar el filtro de página; el cuerpo de la tabla dinámica resume todos los elementos de página como si no se aplicara ningún filtro.
...
// Configurar campos de la tabla dinámica: Fruta→Fila, Cantidad→Datos, Año→Página
pivotTable.addFieldToArea(AsposeCells.PivotFieldType.Row, "Fruit");
pivotTable.addFieldToArea(AsposeCells.PivotFieldType.Data, "Amount");
pivotTable.addFieldToArea(AsposeCells.PivotFieldType.Page, "Year");
...
// Limpiar el filtro de página para que todos los elementos del campo de página sean visibles.
// 0x7FFD (decimal 32765) es el valor centinela especial que significa "todos los elementos" —
// equivalente a seleccionar "(Todos)" en el desplegable del campo de página de Excel.
...
Establecer `CurrentPageItem` a un índice real selecciona solo ese elemento de página. El índice es la posición del elemento en la lista ordenada de elementos del campo de página, por lo que, por ejemplo, `1` selecciona el segundo elemento tras ordenar.
...
pivotTable.getPageFields().get(0).setCurrentPageItem(1); // 1 = segundo elemento en orden ascendente (por ejemplo, "2021")
...
El filtrado de selección múltiple convierte el desplegable de página en una lista de casillas de verificación y permite al usuario final elegir varios elementos de página simultáneamente. Aspose.Cells expone dos propiedades que trabajan juntas. `PivotField.IsMultipleItemSelectionAllowed` debe establecerse en `true` antes de que la interfaz de selección múltiple surta efecto. Una vez habilitada, `PivotItem.IsHidden` controla qué elementos aparecen en la lista de casillas de verificación, por lo que puede mostrar todos los elementos o crear una lista blanca solo con elementos específicos.

El código a continuación habilita la selección múltiple en el mismo campo de página Year construido en el Escenario 1a, y luego muestra dos patrones: la Parte A revela todos los elementos de página dejando `IsHidden` establecido en `false` para cada entrada, mientras que la Parte B crea una lista blanca solo con los valores de origen que elija y oculta todo lo demás mediante un bloque `switch (pivotItems[i].getStringValue())`.
...
> **Nota:** Cuando utilice el filtrado de selección múltiple mediante `PivotItem.IsHidden`, **al menos un `PivotItem` debe permanecer visible** (`IsHidden == false`). Si todos los elementos están ocultos, Excel se bloqueará al abrir el archivo o renderizará una tabla dinámica en blanco. Verifique siempre que su lista blanca de selección múltiple incluya al menos un elemento de sus datos de origen.
...
|---|---|---|---|
| Añadir un campo de página por nombre de columna de origen (más común) | `pivotTable.addFieldToArea(PivotFieldType.Page, "fieldName")` | n/a | Alto nivel, una sola línea. Use esto a menos que necesite una referencia a `PivotField`. |
| Añadir un campo de página cuando ya tiene un objeto `PivotField` | `pivotTable.getPageFields().add(PivotField)` | n/a | Use cuando el objeto de campo se obtuvo en otro lugar o necesita reutilizarse. |
| Filtrar a un único elemento de página (modo predeterminado) | `PivotField.CurrentPageItem` | establecer a un índice específico | Por ejemplo, `1` muestra el segundo elemento de la lista ordenada. |
| Mostrar todos los elementos / borrar el filtro de página | `PivotField.CurrentPageItem` | establecer a `0x7FFD` | El valor mágico `0x7FFD` (decimal 32765) es el centinela para "todos los elementos". |
