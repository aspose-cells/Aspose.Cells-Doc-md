---
title: Aspose.Cells para CPP 22.8 Notas de la versión Linux
type: docs
weight: 5
url: /es/cpp/aspose-cells-for-cpp-22-8-release-notes-linux/
---
{{% alert color="primary" %}}

Esta página contiene notas de la versión Aspose.Cells para C++ 22.8 para Linux.

{{% /alert %}}

Nos complace anunciar el lanzamiento de Aspose.Cells para C++ 22.8 para Linux.
|**Llave**|**Resumen**|**Categoría**|
|:- |:- |:- |
|CELLSCPP-859| Solucionar el problema de referencia API|Insecto|
|CELLSCPP-880| Solucione el error de formato de archivo de fods abierto|Insecto|
|CELLSCPP-881| Funciones de sincronización con la versión .Net|Apoyo|


