---
title: Agregar un marcado de agua de WordArt a la hoja de cálculo
linktitle: Gestionar WordArt
type: docs
weight: 180
url: /es/python-net/add-wordart-watermark-to-worksheet/
---

{{% alert color="primary" %}} 

Utilice WordArt para agregar efectos especiales de texto a las hojas de cálculo. Por ejemplo, estire un título en la parte superior del archivo, decore texto y ajuste texto a una forma predefinida, o aplique texto a una hoja de Excel como marca de agua de fondo. El WordArt se convierte en un objeto que puede mover o posicionar en las hojas de cálculo para agregar decoración.

{{% /alert %}} 

El siguiente ejemplo muestra cómo agregar una forma WordArt para establecer una marca de agua de fondo para una hoja de cálculo.



{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "Shapes-WordArt-AddWordArtWatermarkToWorksheet-1.py" >}}

## **Temas avanzados**
- [Añadir texto de Word Art con estilos integrados](/cells/es/python-net/add-word-art-text-with-built-in-styles/)
- [Bloquear marca de agua WordArt](/cells/es/python-net/locking-wordart-watermark/)
- [Establecer un estilo de WordArt preestablecido al texto de la forma](/cells/es/python-net/set-preset-wordart-style-to-the-text-of-the-shape/)
{{< app/cells/assistant language="python-net" >}}
