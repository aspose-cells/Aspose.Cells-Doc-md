---
title: cómo personalizar la apariencia de la UI para el mensaje de toast en GridJs  
type: docs
weight: 250
url: /es/python-net/aspose-cells-gridjs/how-to-customize-toast/
description: Este artículo describe cómo personalizar la apariencia de la UI para el toast de mensajes en GridJs.
...
Guía para toast personalizado en GridJs

## Resumen

GridJs ofrece un sistema flexible de notificaciones tipo toast personalizadas que permite reemplazar los mensajes predeterminados por tus propias implementaciones de notificación. Esta guía demuestra diferentes enfoques para personalizar notificaciones toast según las necesidades de tu aplicación.
...
- [Casos de Uso Avanzados](#advanced-use-cases)
  - [Registro y Analíticas](#6-logging-and-analytics)
  - [Toast Amigable para Móviles](#7-mobile-friendly-toast)
...
**Parámetros:**
- `title` (cadena): El título de la notificación toast
- `content` (cadena): El contenido del mensaje toast
- `callback` (función, opcional): Función de devolución de llamada para ejecutar después de cerrar la notificación toast
...
- `positionClass`: Controla la posición de las notificaciones (arriba-derecha, abajo-izquierda, etc.)
...
- Posicionamiento en la parte inferior (amigable para el pulgar)
- Ancho porcentual (máximo 80%)
...
Para una mejor experiencia de usuario, utiliza notificaciones tipo toast no bloqueantes en lugar de alertas modales:
...
Elige duraciones de auto-descarte apropiadas según la longitud del contenido:
...
Envuelve tu función de toast en try-catch para evitar que la hoja de cálculo se bloquee:
...
Siempre limpia los elementos DOM y los escuchadores de eventos:
...
### 7. Restaurar predeterminado cuando sea necesario

Proporcione una manera de restaurar el comportamiento predeterminado del toast:
...
**Solución:** Limpie los elementos DOM y los escuchadores de eventos:
```javascript
const remove = () => {
  document.body.removeChild(toast);
...
**Problema:** El toast se ve mal en dispositivos móviles.

**Soluciones:**
1. Usa anchos porcentuales: `maxWidth: '80%'`
