...
type: docs
weight: 250
url: /es/python-net/aspose-cells-gridjs/client/
keywords: GridJs,personalizado,logo,configuración,api,gridjs,python,editar,hoja de cálculo,vista,visor,editor,excel,apijs,cliente api
...
| `isCollaborative` | Si soportar modo colaborativo. | `false` | Sí |
...
| `token` | Establece el token de autenticación. Cuando el token no es null, la cabecera `Authorization: Bearer {token}` se añadirá automáticamente a las cabeceras de la solicitud. Puedes usar `xs.refreshToken(token)` para establecer un nuevo token. | Ninguno | Sí | 
| `showBottombarStats` | Mostrar las estadísticas en la barra inferior.<br>El valor predeterminado es true. | `true` | Sí |   
| `showRowAppenderToolbar` | Mostrar la barra de herramientas de inserción en lote de filas.<br>El valor predeterminado es true. | `true` | Sí |   
...
- Configurar las opciones colaborativas en modo colaborativo, asegurarse de usar setCollaborativeSetting antes de setUniqueId  
```javascript
...
- Establecer el estilo para el área de celda deseada
```javascript
...
```
- Borrar el contenido de la celda o limpiar el estilo en la celda seleccionada  
```javascript
...
- Borrar el contenido de la celda o limpiar el estilo en el área de celda deseada
```javascript
...
- Aplicar configuraciones de fuente para objetos de cuadro de texto
```javascript
