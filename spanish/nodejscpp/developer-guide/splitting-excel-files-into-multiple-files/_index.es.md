...
1. Abra el archivo de Excel de origen instanciando un objeto `Workbook` y pasando la ruta del archivo a su constructor.
2. Itere a través de la colección `Workbook.Worksheets` utilizando un bucle `for` o `forEach` para que se procese cada `Worksheet` del archivo de origen.
...
1. Abra el archivo de Excel de origen instanciando un objeto `Workbook` con la ruta del archivo.
2. Recupere la `Worksheet` de origen que contiene el rango que desea copiar, ya sea por índice (por ejemplo, la primera hoja) o por nombre de la colección `Worksheets`.
...
6. Copie el rango de origen en la hoja de cálculo de destino, normalmente comenzando desde la celda `A1`. El método `Cells.copy` en la colección `Cells` de destino se puede usar para copiar un rango completo, o puede iterar a través de las celdas del rango de origen y escribir sus valores en las celdas de destino con `putValue`. Se pueden suministrar `CopyOptions` opcionales para controlar qué se transfiere (solo valores, valores y estilos, fórmulas, etc.).
...
- [Copiar filas y columnas](/cells/es/nodejs-cpp/copying-rows-and-columns/)
- [Combinar y separar celdas](/cells/es/nodejs-cpp/merging-and-unmerging-cells/)
