...
type: docs
weight: 250
url: /es/net/aspose-cells-gridjs/how-to-use-gridjs-client-api/
...
| `isCollaborative` | Si soporta modo colaborativo. | `false` | Sí |
...
| `token` | Establece el token de autenticación. Cuando el token no sea nulo, la cabecera `Authorization: Bearer {token}` se añadirá automáticamente a las cabeceras de la solicitud. Puedes usar `xs.refreshToken(token)` para establecer un nuevo token. | Ninguno | Sí |    
| `showBottombarStats` | Mostrar las estadísticas de la barra inferior.<br>El valor predeterminado es verdadero. | `true` | Sí |   
| `showRowAppenderToolbar` | Mostrar la barra de herramientas de inserción de filas en lote.<br>El valor predeterminado es verdadero. | `true` | Sí |   
...
```
- configurar notificación personalizada
```javascript
...
```
- Establecer la llamada a la función antes de guardar 
```javascript
...
- Configurar los ajustes colaborativos en modo colaborativo, asegurándose de establecer configuraciones colaborativas antes de establecer el ID único  
```javascript
...
- Establecer comentario en la celda
```javascript
...
- Eliminar comentario en la celda
```javascript
...
- Establecer estilo para el área de celdas deseada
```javascript
...
```
- Eliminar contenido de la celda o borrar estilo en la celda seleccionada  
```javascript
...
- Eliminar contenido de la celda o borrar estilo en el área de celdas deseada
```javascript
...
```
## API para objetos de forma
- Cambiar color de fondo para objeto de forma
```javascript
...
- Aplicar configuraciones de fuente para el objeto caja de texto
```javascript
