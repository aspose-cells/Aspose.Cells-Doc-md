...
type: docs
weight: 10
url: /tr/python-net/how-to-format-number-to-accounting/
description: Bu makale, Aspose.Cells for Python via .NET API kullanarak Sayıyı Muhasebe Formatına dönüştürmeyi tanıtacaktır.
...
## **Python via .NET için Aspose.Cells'te Sayıyı Muhasebe Formatına Nasıl Biçimlendirilir**
Python via .NET için Aspose.Cells'te sayıları muhasebe biçimine biçimlendirmek için, hücre veya hücre aralığıyla ilişkili `Style` nesnesini kullanabilirsiniz. `Style` nesnesi, çeşitli biçimlendirme seçenekleri ayarlamanıza olanak tanır, bunlar arasında sayı biçimleri de vardır. Muhasebe biçimi genellikle belirli gereksinimlere göre değişebilen bir format koduna sahiptir (örneğin, para birimi sembolleri gösterilsin veya gösterilmesin, ondalık basamak sayısı gibi).

Aspose.Cells for Python via .NET kullanılarak bir hücreye muhasebe numarası biçimi uygulama temel örneği burada:

1. **Aspose.Cells'i Referans Alın**: Projenizde Aspose.Cells for Python via .NET'yi referans aldığınızdan emin olun.
