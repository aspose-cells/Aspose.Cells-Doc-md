---
title: Aspose.Cells for C++'ta Pivot Tablolarına Stil Uygulama
linktitle: Aspose.Cells for C++'ta Pivot Tablolarına Stil Uygulama
description: Aspose.Cells for C++'ta pivot tablolarına yerleşik ve özel stillerin nasıl uygulanacağını öğrenin; eski XLS otomatik biçimleri, modern Excel 2007+ adlandırılmış stiller, özel pivot tablosu stilleri ve FormatAll kısayolu ele alınmaktadır.
...
{{% alert color="primary" %}}
Aspose.Cells, hem eski pivot otomatik biçimlerini (`.xls` dosyaları için tasarlanmış) hem de modern adlandırılmış veya özel pivot tablosu stillerini (`.xlsx`, `.xlsm` ve `.xlsb` dosyaları için tasarlanmış) uygulamayı destekler. Çağıracağınız API, çalışma kitabının yüklendiği formata göre değil, kaydedildiği dosya formatına göre belirlenir.
{{% /alert %}}

## **Giriş**
Aspose.Cells, pivot tabloları için iki paralel stil API'si sunar. Aralarındaki seçim, çalışma kitabını okuduğunuz formata göre değil, kaydettiğiniz formata göre belirlenir. Bir `.xls` dosyasından yüklenen çalışma kitabı `.xlsx` olarak yeniden kaydedilebilir; bu durumda eski stil API'si yerine modern stil API'si geçerli olur.
...
- `PivotTable.PivotTableStyleName`, `Worksheets.TableStyles.AddPivotTableStyle(...)` aracılığıyla kendinizin tanımladığı özel bir stili seçer. Hazır ayarların izin verdiğinin ötesinde renk, kenarlık veya yazı tipi değişiklikleri istediğinizde özel stiller gereklidir.
Ayrıca, `PivotTable.FormatAll(Style)`, tek bir `Style` nesnesini pivotun her hücresine uygulayan ve yukarıdaki stil adı API'lerinden herhangi biriyle yapılan ayarları geçersiz kılan bir kısayoldur. Bu, altta yatan temadan bağımsız olarak tek tip bir görünüm gerektiğinde kullanışlıdır.

## **Eski XLS Hazır Otomatik Biçimini Uygulama**
`PivotTable.AutoFormatType`, `Aspose.Cells.Pivot.PivotTableAutoFormatType` numaralandırmasından bir değer kabul eder. Kullanılabilir değerler `Report1`'den `Report10`'a, `Classic` ve `Table1`'den `Table10`'a kadardır.
Aşağıdaki örnek, yeni bir çalışma kitabı yükler, Fruit/Year/Amount örnek verilerini ekler, bir pivot tablo oluşturur, `PivotTableAutoFormatType.Report5` uygular ve sonucu `.xls` olarak kaydeder.

{{% alert color="primary" %}}
**Neden sütun alanı yok?** Rapor serisi otomatik biçimleri (`Report1`–`Report10`, `Table1`–`Table10`), klasik Excel'de yalnızca **tek boyutlu pivot tablolar** için tasarlanmıştır ve yalnızca satır alanları ve değerler içerir; sütun alanı başlıkları için yerleşik bir biçimlendirme içermez. Pivotunuz sütun alanları gerektiriyorsa, bunun yerine modern Excel'in kullandığı iki boyutlu düzen için tasarlanmış [Senaryo 2](#apply-a-modern-named-preset-pivot-table-style) içindeki modern `PivotTableStyleType` hazır ayarlarını kullanın.
{{% /alert %}}
...
int main() {
    ...
    // Başlık satırı (Meyve, Yıl, Miktar) ve 2020 ile 2021 yılları için üzüm, yaban mersini, kivi, kirazı kapsayan 9 veri satırı ile kaynak verileri doldur
    ...
    // Hedef hücre E3'e "Pivot1" adında, A1:C10 kaynak aralığını kullanan bir özet tablo ekle
    ...
    // Eski XLS ön ayar biçimi "Report5"i uygula
...
## **Modern Adlandırılmış Hazır Pivot Tablosu Stilini Uygulama**
...
2. Stili, `TableStyle.TableStyleElements.Add(TableStyleElementType)` aracılığıyla öğeler (`WholeTable` veya `GrandTotalRow` gibi) ekleyerek yapılandırın, ardından `TableStyleElement.SetElementStyle(Style)` ile her öğeye bir `Style` atayın.
3. `PivotTable.PivotTableStyleName` değerini stilin adına ayarlayarak özel stili pivota uygulayın. Burada `PivotTableStyleType` kullanmayın; çünkü bu özellik yerleşik hazır ayarları seçer.
...
Aşağıdaki örnek, `WholeTable` üzerinde ince siyah kenarlık ve `GrandTotalRow` üzerinde kalın kırmızı yazı tipi içeren özel bir pivot stili tanımlar, ardından bunu `PivotTableStyleName` aracılığıyla uygular ve `.xlsx` olarak kaydeder.
...
int main() {
    ...
    // A1:C10'dan kaynaklanan, E3'e sabitlenmiş ve "Pivot1" adında pivot tablo ekle
    int pivotIndex = worksheet.GetPivotTables().Add(u"A1:C10", u"E3", u"Pivot1");
    PivotTable pivotTable = worksheet.GetPivotTables().Get(pivotIndex);
    pivotTable.AddFieldToArea(PivotFieldType::Row, u"Fruit");
    pivotTable.AddFieldToArea(PivotFieldType::Column, u"Year");
...
{{% alert color="primary" %}}
`FormatAll`, hem `PivotTableStyleType` hem de `PivotTableStyleName` değerlerini geçersiz kılar. Pivotun tamamında temadan bağımsız, tek tip bir görünüm gerektiğinde kullanın.
{{% /alert %}}
...
|---|---|---|
| `.xls` (eski) | `PivotTable.AutoFormatType` | `Aspose.Cells.Pivot.PivotTableAutoFormatType` değerleri (ör. `Report1`–`Report10`, `Classic`, `Table1`–`Table10`). Modern formatlara kaydederken yok sayılır. |
