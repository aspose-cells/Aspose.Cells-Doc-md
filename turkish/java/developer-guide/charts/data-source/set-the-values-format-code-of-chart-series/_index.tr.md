---
title: Grafik Serisinin Değer Biçim Kodunu Ayarlayın
type: docs
weight: 60
url: /tr/java/set-the-values-format-code-of-chart-series/
---
## **Olası Kullanım Senaryoları**

kullanarak grafik serisinin değer biçim kodunu ayarlayabilirsiniz.[**Series.ValuesFormatCode**](https://reference.aspose.com/cells/java/com.aspose.cells/series#ValuesFormatCode)Emlak. Bu özellik, yalnızca çalışma sayfası içindeki aralığa dayalı diziler için yararlı olmakla kalmaz, aynı zamanda bir dizi değerle oluşturulan diziler için de iyi çalışır.

## **Grafik Serisinin Değer Biçim Kodunu Ayarlayın**

Aşağıdaki örnek kod, daha önce serisi olmayan boş grafiğe bir seri ekler. Değer dizisini kullanarak seriyi ekler. Diziyi ekledikten sonra $#,##0 koduyla biçimlendirir.[**Series.ValuesFormatCode**](https://reference.aspose.com/cells/java/com.aspose.cells/series#ValuesFormatCode)mülk ve 10000 sayısı 10.000 $ olur. Ekran görüntüsü, kodun üzerindeki etkisini gösterir.[örnek excel dosyası](51740736.xlsx)ve[çıktı excel dosyası](51740735.xlsx)infazdan sonra.

![yapılacaklar:resim_alternatif_metin](set-the-values-format-code-of-chart-series_1.png)

## **Basit kod**

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-AsposeCellsExamples-Charts-SetValuesFormatCodeOfChartSeries.java" >}}
