...
| `token` | Kimlik doğrulama jetonunu ayarlayın. Jeton null değilse, istek başlıklarına otomatik olarak `Authorization: Bearer {token}` eklenir. `xs.refreshToken(token)` kullanarak yeni bir jeton ayarlayabilirsiniz. | Yok | Evet |    
| `showBottombarStats` | Alt çubuk istatistiklerini göstermek isteyip istemediğiniz. <br>Varsayılan değer true. | `true` | Evet |   
| `showRowAppenderToolbar` | Toplu satır ekleyici araç çubuğunu göstermek isteyip istemediğiniz.<br>Varsayılan değer true. | `true` | Evet |   
...
- İşbirlikçi modda çalışma ayarlarını yapın, setUniqueId'den önce setCollaborativeSetting'i ayarladığınızdan emin olun  
```javascript
...
xs.sheet.freeze(ri,ci)
	...
	ci:column index
```
- Bölmeyi dondurmayı kaldır
```javascript
xs.sheet.freeze(0,0)
```

- Düzenlenebilir/yalnız okunabilir aralık ayarla
```javascript
...
- İstenen hücre alanı için stili ayarla
```javascript
...
```
- Seçilen hücredeki içerikleri sil veya stili temizle  
```javascript
...
- İstenen hücre alanında içerikleri sil veya stili temizle
```javascript
...
```
## Şekil nesnesi API'leri
- Şekil nesnesi için arka plan rengini değiştir
```javascript
