---
title: Çalışma Sayfaları Arasında Şekil Kopyalama ile JavaScript kullanarak C++
linktitle: Şekilleri Kopyalama
type: docs
weight: 200
url: /tr/javascript-cpp/copy-shapes-between-worksheets/
description: Resimler ve çizelgeler gibi şekilleri çalışma sayfaları arasında kopyalamayı öğrenin, Aspose.Cells for JavaScript ile C++ kullanarak.
---

{{% alert color="primary" %}}

 Bazen, çalışma sayfasında öğeleri kopyalamanız gerekir, örneğin resimler, çizelgeler ve diğer çizim nesneleri, çalışma sayfaları arasında. Aspose.Cells for JavaScript ile C++ bu özelliği destekler. Çizelgeler, görüntüler ve diğer nesneler en yüksek hassasiyetle kopyalanabilir.
...
Bir resmi bir çalışsayfasından diğerine kopyalamak için aşağıdaki örnek kodda gösterildiği gibi [**PictureCollection.add(number, number, number, number, Uint8Array)**](https://reference.aspose.com/cells/javascript-cpp/picturecollection/#add-number-number-number-number-uint8array-) yöntemini kullanın.
...
Aşağıdaki kod, bir çalışsayfasından diğerine bir grafik kopyalamak için [**ShapeCollection.addCopy(Shape, number, number, number, number)**](https://reference.aspose.com/cells/javascript-cpp/shapecollection/#addCopy-shape-number-number-number-number-) yönteminin kullanımını göstermektedir.
...
Kontroller ve diğer çizim nesnelerini kopyalamak için, aşağıdaki örnekte gösterildiği gibi [**ShapeCollection.addCopy(Shape, number, number, number, number)**](https://reference.aspose.com/cells/javascript-cpp/shapecollection/#addCopy-shape-number-number-number-number-) yöntemini kullanın.
