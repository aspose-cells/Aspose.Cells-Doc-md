---
title: Aspose.Cells for .NET
type: docs
weight: 10
url: /tr/net/
is_root: true
description: A powerful and fast Speadsheet programming library written in C#. It supports the popular Excel (XLS, XLSX, XLSM, XLSB, XLTX, XLTM,SpreadsheetML,CSV), OpenOffice(ODS),HTML and JSON file formats and allows exporting Excel files to PDF,DOCX, PPTX, JSON, XPS, HTML, MHTML, Düz Metin ve TIFF, JPG, PNG, BMP and SVG dahil popüler resim biçimleri.
---
{{% alert color="primary" %}}

**![Aspose.Cells for .NET Ürün Logosu](home_1.png)**

**Hoşgeldiniz Aspose.Cells for .NET**

 Aspose.Cells for .NET, yazılım geliştiricilerin elektronik tablo dosyalarını kendi uygulamalarında değiştirmelerine ve işlemelerine olanak tanıyan bir programlama sınıfı kitaplığıdır.

API'ler ve GUI kontrolleriyle birleştiğinde Aspose.Cells for .NET, Microsoft Excel® programlama ve dönüştürmeyi hızlandırır.

Aspose.Cells for .NET, işletmenizin her gün kullandığı popüler e-tablo (XLS, XLSX, XLSM, XLSB, XLTX, XLTM, CSV, SpreadsheetML, ODS) dosya biçimlerini destekler. Ayrıca Excel dosyalarının PDF, DOCX, PPTX, JSON, XPS, HTML, MHTML, JSON, Düz Metin ve TIFF, JPG, PNG, BMP and SVG dahil olmak üzere popüler resim formatlarına dışa aktarılmasına izin verir.


{{% /alert %}}


<div class="row">
	<div class="col-md-4">
		<p><b>Aspose.Cells hakkında</b></p>
			<hr><p>genel bakış</p></hr>
			<ul>
				<li><a href="/cells/tr/net/what-s-new-in-aspose-cells-for-net/">Aspose.Cells'deki yenilikler for .NET</a></li>
				<li><a href="/cells/tr/net/product-overview/">Ürüne Genel Bakış</a></li>
				<li><a href="/cells/tr/net/licensing/">Lisanslama ve Abonelik</a></li>
			    <li><a href="https://releases.aspose.com/tr/cells/net/release-notes/">Aspose.Cells for .NET Sürüm Notları</a></li>
			</ul>            
	        <p>Yararlı Kaynaklar</p>
			<ul>
				<li><a href="https://www.nuget.org/packages/Aspose.Cells/">İndir Aspose.Cells for .NET at NuGet</a></li>
				<li><a href="https://downloads.aspose.com/cells/net">İndir Aspose.Cells for .NET</a></li>
			</ul>
	</div>
	<div class="col-md-4">
		<p><b>Başlamak</b></p>
			<hr><p><a href="/cells/tr/net/getting-started/">Başlarken</a></p></hr>
			<ul>
				<li><a href="/cells/tr/net/supported-file-formats/">Desteklenen dosya Biçimleri</a></li>
				<li><a href="/cells/tr/net/aspose-cells-features/">Özellikleri</a></li>
				<li><a href="/cells/tr/net/system-requirements/">sistem gereksinimleri</a></li>
				<li><a href="/cells/tr/net/getting-started/#how-to-install">Kurulum</a></li>
				<li><a href="/cells/tr/net/installation/">Aspose Cells ila NuGet'i yükleyin</a></li>
			</ul>
			<p>Öğrenmek</p>
			<ul>
				<li><a href="/cells/tr/net/how-to-run-the-examples/">Örnekler Nasıl Çalıştırılır?</a></li>
				<li><a href="/cells/tr/net/how-to-run-aspose-cells-in-docker/">Docker'da Aspose.Cells Nasıl Çalıştırılır?</a></li>
				<li><a href="/cells/tr/net/how-to-run-aspose-cells-in-aws-lambda/">AWS Lambda'da Aspose.Cells Nasıl Çalıştırılır</a></li>
				<li><a href="/cells/tr/net/plugins/">Eklentiler</a></li>
			</ul>
	</div>
	<div class="col-md-4">
		<p><b>İşlev Geliştirme</b></p>
			<hr><p><a href="/cells/tr/net/developer-guide/">Rehberlik etmek</a></p></hr>
			<ul>
				<li><a href="/cells/tr/net/introduction-of-aspose-cells-for-net/">Aspose.Cells for .NET tanıtımı</a></li>
				<li><a href="/cells/tr/net/loading-saving-and-managing/">Yükle ve Kaydet</a></li>
				<li><a href="/cells/tr/net/convert-workbook-to-different-formats/">Oluşturma ve Dönüştürme</a></li>
				<li><a href="/cells/tr/net/print-and-preview/">Yazdır ve Önizle</a></li>
				<li><a href="https://reference.aspose.com/cells/net">Aspose.Cells for .NET API Başvuru Kılavuzu</a></li>
			</ul>
			<p>Teknik Destek</p>
			<ul>
				<li><a href="https://forum.aspose.com/c/cells/9">Aspose.Cells for .NET Ücretsiz Destek Forumu</a></li>
				<li><a href="https://helpdesk.aspose.com/">Aspose.Cells for .NET Ücretli Destek Yardım Masası</a></li>
			</ul>
	</div>
</div>
