...
description: Bu makale, C# API ve .NET Kitaplığı kullanılarak Excel Çalışma Sayfası Sayfası Yapısı özelliğinin Yazdırma Seçeneklerinin program aracılığıyla nasıl ayarlanacağını gösterir. Yazdırma Alanı, Yazdırma Başlıkları ve Sayfa Sırasını ayarlayabilirsiniz.
...
Varsayılan olarak, yazdırma alanı, çalışma sayfasının veri içeren tüm alanlarını içerir. Geliştiriciler, çalışma sayfasının belirli bir yazdırma alanını oluşturabilir.
 ...
 Belirli bir yazdırma alanı seçmek için[**Sayfa ayarı**](https://reference.aspose.com/cells/net/aspose.cells/pagesetup) sınıf'[**Alanı yazdır**](https://reference.aspose.com/cells/net/aspose.cells/pagesetup/properties/printarea)mülk. Bu özelliğe yazdırma alanını tanımlayan bir hücre aralığı atayın.
