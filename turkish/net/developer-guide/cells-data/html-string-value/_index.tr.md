---
title: Cell içine HTML Zengin Metin ekleme
linktitle: Html Dizi Değeri
type: docs
weight: 50
url: /tr/net/adding-html-rich-text-inside-the-cell/
---
{{% alert color="primary" %}}

Aspose.Cells, Microsoft Excel odaklı HTML'in XLS/XLSX formatına dönüştürülmesini destekler. Bunun anlamı, Microsoft Excel tarafından oluşturulan HTML, Aspose.Cells kullanılarak tekrar XLS/XLSX formatına dönüştürülebilir.

 Benzer şekilde, bazı basit HTML varsa, Aspose.Cells bunu HTML Zengin Metin'e dönüştürebilir. Aspose.Cells sağlar[**Cell.HtmlString**](https://reference.aspose.com/cells/net/aspose.cells/cell/properties/htmlstring) Bu kadar basit bir HTML alabilen ve biçimlendirilmiş hücre metnine dönüştürebilen özellik.

{{% /alert %}}

Aşağıdaki kod örneği, hücrenin içine HTML zengin metnin nasıl ekleneceğini gösterir. Lütfen çıktı Excel dosyasının ekran görüntüsüne bakın.

![yapılacaklar:resim_alternatif_metin](adding-html-rich-text-inside-the-cell_1)

{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Examples-CSharp-Articles-ManagingRowsColumnsCells-AddingHTMLRichTextInCell-AddingHTMLRichTextInCell.cs" >}}

## İlgili Makaleler

- [Cell Değerini HTML kullanarak ayarlayarak Madde İşaretlerini Görüntüle](/cells/tr/net/display-bullets-by-setting-cell-value-using/)
- [Cell'den HTML5 dizesini alın](/cells/tr/net/get-html5-string-from-cell/)
