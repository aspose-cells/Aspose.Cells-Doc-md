...
type: docs
weight: 10
url: /tr/net/how-to-format-number-to-accounting/
description: Bu makale, Aspose.Cells for .NET API sini kullanarak Sayıları Muhasebe formatına nasıl biçimlendireceğinizi anlatacaktır.
...
## **Excel'de Sayıları Muhasebe Formatında Biçimlendirme Aspose.Cells for .NET**
Aspose.Cells for .NET sayısını muhasebe biçimine çevirmek için, bir hücre veya hücre aralığı ile ilişkili `Stil` nesnesini kullanabilirsiniz. `Stil` nesnesi, farklı biçimlendirme seçenekleri ayarlamanıza olanak tanır; bunlar arasında sayı biçimleri de bulunur. Muhasebe biçimi genellikle, gösterilecek para sembolleri, ondalık basamağı gibi özelliklere bağlı olarak değişen bir biçim koduna sahiptir.

Aspose.Cells for .NET'de hücreye muhasebe sayı biçimi uygulamanın temel bir örneği:
