...
type: docs
weight: 10
url: /de/nodejs-cpp/how-to-format-number-to-accounting/
description: Dieser Artikel stellt vor, wie man Zahlen mit Aspose.Cells for Node.js via C++ API in Buchhaltungssätze umwandelt.
...
## **Wie man Zahlen in Aspose.Cells for Node.js via C++ in Buchhaltung umwandelt**
Um Zahlen im Buchhaltungsformat in Aspose.Cells for Node.js via C++ zu formatieren, können Sie das `Style`-Objekt verwenden, das mit einer Zelle oder einem Zellbereich verbunden ist. Das `Style`-Objekt ermöglicht es Ihnen, verschiedene Formatierungsoptionen festzulegen, einschließlich Zahlenformate. Das Buchhaltungssformat hat in der Regel einen Formatcode, der je nach Anforderungen variieren kann (z.B. Anzeige von Währungssymbolen, Dezimalstellen usw.).

Hier ein einfaches Beispiel, wie man ein Buchhaltungszahlenformat auf eine Zelle in Aspose.Cells for Node.js via C++ anwendet:

1. **Referenzieren Sie Aspose.Cells**: Stellen Sie sicher, dass Sie Aspose.Cells for Node.js via C++ in Ihr Projekt eingebunden haben. Sie können es von der Aspose-Website erhalten.
...
Dieses Beispiel zeigt, wie man eine einzelne Zelle im US-Dollar-Buchhaltungsformat formatiert. Der Formatstring kann angepasst werden, um unterschiedliche Währungssymbole oder Buchhaltungsformate zu erfüllen. Der wichtigste Teil ist die Methode `style.setCustom`, bei der Sie den benutzerdefinierten Zahlformat-Code für die Buchhaltung angeben.
