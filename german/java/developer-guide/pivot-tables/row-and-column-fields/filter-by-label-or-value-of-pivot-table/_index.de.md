...
description: Aspose.Cells for Java unterstützt umfassende Filterfunktionen für Pivot-Tabellen. Dieser Artikel erläutert, wie Pivot-Tabellendaten mit Beschriftungsfiltern, Datumsfiltern, Wertfiltern, Top-10-Filtern sowie durch das Ein- und Ausblenden einzelner Pivot-Elemente gefiltert werden.
...
{{% alert color="primary" %}}
Aspose.Cells bietet fünf praxisnahe Strategien zum Filtern der in einer Pivot-Tabelle angezeigten Daten. Sie können Beschriftungsfilter auf textbasierte Zeilen- oder Spaltenfelder anwenden, Datumsfilter verwenden, wenn das Feld ausschließlich Datum-/Uhrzeit-Zellen oder leere Zellen enthält, Wertfilter auf aggregierte Zahlen anwenden, Top-10-Filter zur Rangfolge nach einem Wertfeld nutzen oder einzelne Pivot-Elemente über die Eigenschaft `IsHidden` manuell ein- und ausblenden. Jede Strategie wird über dedizierte APIs auf den Klassen `PivotField` und `PivotItem` bereitgestellt.
{{% /alert %}}
...
2. **Datumsfilter** — filtert Zeilen- oder Spaltenfelder, die ausschließlich Datums-/Uhrzeitwerte (oder leere Werte) enthalten.
...
5. **Pivot-Elemente ein-/ausblenden** — steuert manuell die Sichtbarkeit jedes einzelnen Elements in einem Feld.
Jeder Ansatz verwendet eine andere Methode auf der Klasse `PivotField` oder eine Eigenschaft auf der Klasse `PivotItem`. Nach Anwendung eines beliebigen Filters müssen Sie `refreshData()` und `calculateData()` auf der Pivot-Tabelle aufrufen, damit die zwischengespeicherten Daten und berechneten Werte den neuen Filterzustand widerspiegeln.
...
## **Datumsfilter**
Datumsfilter ermöglichen es Ihnen, eine Pivot-Tabelle anhand datumsbezogener Kriterien wie heute, letzte Woche, diesen Monat, nächstes Quartal oder einen bestimmten Datumsbereich einzugrenzen. Dies sind spezialisierte Filter, die nur bei Feldern funktionieren, die Datums-/Uhrzeitinformationen speichern.
...
// (Datumsfilter funktioniert nur, wenn der Zeilen-/Spaltenbereich nur Datum-Uhrzeit-Zellen oder Leerzeichen enthält)
...
// Findet den Datenfeldindex manuell, da PivotFieldCollection keine IndexOf-Methode hat
...
## **Top-10-Filter**
Der Top-10-Filter ist eine spezialisierte Form des Wertfilters, der nur die obersten oder untersten N Elemente basierend auf einem ausgewählten Wertfeld beibehält. Er wird häufig für Ranking-Berichte verwendet, etwa „die 10 umsatzstärksten Produkte" oder „die 5 regionen mit dem geringsten Verkaufsvolumen".
...
Das folgende Beispiel lädt eine Arbeitsmappe mit einer Pivot-Tabelle, die ein Wertfeld enthält, wendet einen Top-10-Filter an, um nur die 10 umsatzstärksten Elemente beizubehalten, aktualisiert die Pivot-Tabelle und speichert die Arbeitsmappe.
...
PivotField rowField = pivotTable.getRowFields().get(0);
// Das erste (und einzige) Datenfeld befindet sich am Index 0; Top 10 sortiert danach.
...
```java
import com.aspose.cells.*;
// Laden einer vorhandenen Arbeitsmappe, die eine Pivot-Tabelle enthält
...
// Abrufen des Ziel-PivotFields (das erste Zeilenbeschriftungsfeld, in dem wir Elemente ausblenden/einblenden)
...
for (int i = 0; i < itemCount; i++)
{
    ...
    // Einblenden demonstrieren: ein zuvor ausgeblendetes Pivot-Element erneut anzeigen
    if (item.getName() == "Item3")
    {
        item.setHidden(false);
    }
}
// Aktualisieren und Neuberechnen der Pivot-Tabelle, damit die Änderungen wirksam werden
pivotTable.refreshData();
// Speichern der Arbeitsmappe - ausgeblendete Elemente bleiben in den zugrundeliegenden Daten
...
- [Pivot-Tabellen und Pivot-Caches in Aspose.Cells for Java aktualisieren](/cells/de/java/refresh-pivot-table/)
