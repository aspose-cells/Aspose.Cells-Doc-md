---
title: Fehlende Funktionen in OpenXML
type: docs
weight: 20
url: /de/net/missing-features-in-openxml/
---

Dieser Abschnitt umfasst die folgenden Themen

- [Formeln dynamisch berechnen oder neu berechnen](/cells/de/net/calculate-or-recalculate-formulas-dynamically/)
- [Konvertieren zwischen Excel-Formaten](/cells/de/net/convert-between-excel-formats/)
- [Verschlüsselung von Excel-Dateien mit Aspose.Cells](/cells/de/net/encrypting-excel-files-using-aspose-cells/)
- [Berichte erstellen, Dokumente mit Daten befüllen](/cells/de/net/generate-reports-populate-documents-with-data/)
  - [Daten aus Dokument importieren und exportieren](/cells/de/net/import-export-data-from-document/)
  - [Zeilen oder Spalten einfügen oder löschen](/cells/de/net/insert-or-delete-rows-or-columns/)
  - [Smart Markers-Funktion in Aspose.Cells](/cells/de/net/smart-markers-feature-in-aspose-cells/)
    - [Gruppierung von Daten in Aspose.Cells](/cells/de/net/grouping-data-in-aspose-cells/)
    - [Bildmarkierungen in Aspose.Cells](/cells/de/net/image-markers-in-aspose-cells/)
    - [Verwenden von anonymen Typen oder benutzerdefinierten Objekten in Aspose.Cells](/cells/de/net/using-anonymous-types-or-custom-objects-in-aspose-cells/)
    - [Verwendung von verschachtelten Objekten in Aspose.Cells](/cells/de/net/using-nested-objects-in-aspose-cells/)
- [Programmierung auf hohem Niveau mit einem Arbeitsmappen-Objektmodell](/cells/de/net/high-level-programming-with-a-workbook-object-model/)
  - [Zusammenstellen von Tabellenkalkulationen](/cells/de/net/assemble-spreadsheets/)
  - [Kopieren von Fragmenten und Arbeitsblättern zwischen Arbeitsmappen](/cells/de/net/copy-fragments-and-worksheets-between-workbooks/)
    - [Arbeitsblätter zwischen Arbeitsmappen kopieren](/cells/de/net/copy-worksheets-between-workbooks/)
    - [Arbeitsblätter innerhalb einer Arbeitsmappe kopieren](/cells/de/net/copy-worksheets-within-a-workbook/)
    - [Arbeitsblätter innerhalb einer Arbeitsmappe verschieben](/cells/de/net/move-worksheets-within-workbook/)
  - [Suchen und Ersetzen in Tabellenkalkulationen](/cells/de/net/find-and-replace-in-spreadsheet/)
- [Verwalten von Diagrammen und Formen in Aspose.Cells](/cells/de/net/managing-charts-and-shapes-in-aspose-cells/)
  - [Hinzufügen von WordArt-Wasserzeichen zu Diagrammen in Aspose.Cells](/cells/de/net/add-wordart-watermark-to-chart-in-aspose-cells/)
  - [Hinzufügen von WordArt-Wasserzeichen zu Arbeitsblättern in Aspose.Cells](/cells/de/net/add-wordart-watermark-to-worksheet-in-aspose-cells/)
  - [Kopieren von Formen zwischen Arbeitsblättern in Aspose.Cells](/cells/de/net/copy-shapes-between-worksheets-in-aspose-cells/)
  - [Sperren von WordArt-Wasserzeichen in Aspose.Cells](/cells/de/net/locking-wordart-watermark-in-aspose-cells/)
- [Rendern und Drucken in Aspose.Cells](/cells/de/net/rendering-and-printing-in-aspose-cells/)
  - [Konvertieren von Diagrammen in Bilder](/cells/de/net/convert-charts-to-images/)
  - [Tabellenkalkulationen drucken](/cells/de/net/print-spreadsheets/)
  - [Angabe von Bildauflösung, Qualität, Komprimierung und anderen Optionen](/cells/de/net/specify-image-resolution-quality-compression-and-other-options/)
- [Arbeitsmappe im Text- oder CSV-Format speichern mit Aspose.Cells](/cells/de/net/save-workbook-to-text-or-csv-format-using-aspose-cells/)
- [Externe Verknüpfungen in Formeln in Aspose.Cells setzen](/cells/de/net/set-external-links-in-formulas-in-aspose-cells/)
- [Einstellen des Formelberechnungsmodus des Arbeitsblatts in Aspose.Cells](/cells/de/net/setting-formula-calculation-mode-of-workbook-in-aspose-cells/)
- [Einstellen von gemeinsamen Formeln in Aspose.Cells](/cells/de/net/setting-shared-formula-in-aspose-cells/)
