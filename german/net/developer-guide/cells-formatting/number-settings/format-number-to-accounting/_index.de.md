...
type: docs
weight: 10
url: /de/net/how-to-format-number-to-accounting/
description: Dieser Artikel führt ein, wie man Nummern mit der Aspose.Cells for .NET API in Buchhaltung umformatiert.
...
## **Wie man Nummern in der Aspose.Cells for .NET in Buchhaltung formatiert**
Um Nummern im Buchhaltungsformat in Aspose.Cells for .NET zu formatieren, können Sie das `Style`-Objekt verwenden, das mit einer Zelle oder einem Zellbereich verknüpft ist. Das `Style`-Objekt ermöglicht es, verschiedene Formatierungsoptionen, einschließlich Zahlenformate, festzulegen. Das Buchhaltungsformat hat typischerweise einen Formatcode, der je nach Anforderungen variieren kann (z.B. Währungssymbole, Dezimalstellen usw.).

Hier ist ein einfaches Beispiel, wie man ein Buchhaltungszahlenformat auf eine Zelle in Aspose.Cells for .NET anwendet:
