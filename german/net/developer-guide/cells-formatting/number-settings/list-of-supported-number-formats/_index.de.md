---
title: Liste der unterstützten Zahlenformate
type: docs
weight: 30
url: /de/net/list-of-supported-number-formats/
---
## **Aspose.Cells**
 Aspose.Cells-Komponente bietet einige integrierte Zahlenformate, um die Anzeigeformate der Zahlen und Daten zu konfigurieren. Diese integrierten Zahlenformate können mithilfe von angewendet werden**Anzahl** Eigentum der**Stil** Objekt. Allen integrierten Zahlenformaten werden eindeutige numerische Werte zugewiesen. Entwickler können dem einen beliebigen numerischen Wert zuweisen*Anzahl* Eigentum von**Stil** Objekt und damit das Anzeigeformat übernommen. Dieser Ansatz ist schneller. Die integrierten Zahlenformate, die von der Komponente Aspose.Cells unterstützt werden, sind unten aufgeführt:

|**Wert**|**Typ**|**Zeichenkette formatieren**|
|:- |:- |:- |
|0 | Allgemein| Allgemein|
|1 | Dezimal|0 |
|2 | Dezimal|0.00 |
|3 | Dezimal|# ,##0
|
|4 | Dezimal|# ,##0.00
|
|5 | Währung|$#,##0;$-#,##0 |
|6 | Währung|$#,##0;$-#,##0 |
|7 | Währung|$#,##0.00;$-#,##0.00 |
|8 | Währung|$#,##0.00;$-#,##0.00 |
|9 | Prozentsatz|0% |
|10 | Prozentsatz|0.00% |
|11 | Wissenschaftlich| 0.00E+00|
|12 |Fraktion|# ?/?
|
|13 |Fraktion|# */*
|
|14 | Datum| M/T/JJ|
|15 | Datum| t-mm-jj|
|16 | Datum| d-mm|
|17 | Datum| mmm-jj|
|18 | Zeit| h:mm AM/PM|
|19 | Zeit| h:mm:ss AM/PM|
|20 | Zeit| hmm|
|21 | Zeit| h:mm:ss|
|22 | Zeit| m/d/jj h:mm|
|37 | Währung|# ,##0;-#,##0
|
|38 | Währung|# ,##0;-#,##0
|
|39 | Währung|# ,##0.00;-#,##0.00
|
|40 | Währung|# ,##0.00;-#,##0.00
|
|41 | Buchhaltung|_ * #,##0_ ;_ * "_ ;_ @_ |
|42 | Buchhaltung|_ $* #,##0_ ;_ $* "_ ;_ @_ |
|43 | Buchhaltung|_ * #,##0.00_ ;_ * "??_ ;_ @_ |
|44 | Buchhaltung|_ $* #,##0.00_ ;_ $* "??_ ;_ @_ |
|45 | Zeit| mm:ss|
|46 | Zeit| h :mm:ss|
|47 | Zeit| mm:ss.0|
|48 | Wissenschaftlich|## 0.0E+00
|
|49 | Text|@ |
## **Aspose.Cells Grid-Suite**
Wie wir wissen, gibt es zwei Aspose.Cells-Grid-Steuerelemente: Aspose.Cells.GridDesktop & Aspose.Cells.GridWeb. Beide Steuerelemente unterstützen eine große Anzahl von Zahlenformaten, die in Bezug auf jedes Steuerelement wie folgt in zwei Abschnitte unterteilt sind:

- In Aspose.Cells.GridDesktop unterstützte Zahlenformate
- In Aspose.Cells.GridWeb unterstützte Zahlenformate
### **In Aspose.Cells.GridDesktop unterstützte Zahlenformate**
Aspose.Cells.GridWeb unterstützt auch 59 Arten von Zahlenformaten, die unten aufgeführt sind:

|**Index**|**Zahlenformate**|
|:- |:- |
|0 | Allgemein|
|1 |0 |
|2 |0.00 |
|3 |# ,##0
|
|4 |# ,##0.00
|
|5 |\"$\"#,##0_);(\"$\"#,##0) |
|6 |\"$\"#,##0_);(\"$\"#,##0) |
|7 |\"$\"#,##0.00_);(\"$\"#,##0.00) |
|8 |\"$\"#,##0.00;\"$\"-#,##0.00 |
|9 |0% |
|10 |0.00% |
|11 | 0.00E+00|
|12 |# ?/?
|
|13 |# */*
|
|14 | M/T/JJ|
|15 | t-mm-jj|
|16 | d-mm|
|17 | mmm-jj|
|18 | h:mm AM/PM|
|19 | h:mm:ss AM/PM|
|20 | hmm|
|21 | h:mm:ss|
|22 | m/d/jj h:mm|
|23 |\"$\"#,##0;(\"$\"#,##0) |
|24 |\"$\"#,##0.00;(\"$\"#,##0.00) |
|25 |\"$\"#,##0.00;(\"$\"#,##0.00) |
|26 |\"$\"#,##0.00_);(\"$\"#,##0.00) |
|27 |M月D日|
|28 |M月D日|
|29 |M月D日|
|30 |M/T/JJ|
|31 |YYYY年M月D日|
|32 | 时 mm 分|
|33 |h\ 时 \"mm\" 分 \"ss\" 秒 \"|
|34 | tth 时 mm 分|
|35 |tth 时 mm 分 ss 秒|
|36 |YYYY年M月|
|37 |_(#,##0 );(#,##0) |
|38 |_(#,##0 );(#,##0) |
|39 |_(#,##0.00 );(#,##0.00) |
|40 |_(#,##0.00 );(#,##0.00) |
|41 |*(\"$\"* ***#,##0** **);*** **(\"$\"** *_(#,##0);* (\"$\"*\"-\" *);* (@_) |
|42 |*(* ***#,##0** **);*** **(** *_(#,##0);* (*-\" *);* (@_) |
|43 |*(* ***#,##0.00** **);*** **(** *_(#,##0.00);* (*-\"?? *);* (@_) |
|44 |*(\"$\"* ***#,##0.00** **);*** **(\"$\"** *_(#,##0.00);* (\"$\"*\"-\"?? *);* (@_) |
|45 | mm:ss|
|46 | h:mm:ss|
|47 | mm:ss.0|
|48 |## 0.0E+0
|
|49 |@ |
|50 |YYYY年M月|
|51 |M月D日|
|52 |YYYY年M月|
|53 |M月D日|
|54 |M月D日|
|55 | tth 时 mm 分|
|56 |tth 时 mm 分 ss 秒|
|57 |YYYY年M月|
|58 |M月D日|

{{% alert color="primary" %}} 

In einigen Zahlenformaten fallen Ihnen möglicherweise einige Zeichen wie 月 auf. Dies sind tatsächlich chinesische Schriftzeichen und können in chinesischen und japanischen Versionen von MS Excel verwendet werden.

{{% /alert %}} 
### **In Aspose.Cells.GridWeb unterstützte Zahlenformate**
Aspose.Cells.GridWeb unterstützt auch 59 Arten von Zahlenformaten, die unten aufgeführt sind:

|**Typen von Zahlenformaten**|**Zahlenformate**|
|:- |:- |
| Allgemein| Allgemein|
| Dezimal1|0 |
| Dezimal2|0.00 |
| Dezimal3|# ,##0
|
| Dezimal4|# ,##0.00
|
| Währung1|$#,##0;$-#,##0 |
| Währung2|$#,##0;$-#,##0 |
| Währung3|$#,##0.00;$-#,##0.00 |
| Währung4|$#,##0.00;$-#,##0.00 |
| Währung5|# ,##0;-#,##0
|
| Währung6|# ,##0;-#,##0
|
| Währung7|# ,##0.00;-#,##0.00
|
| Währung8|# ,##0.00;-#,##0.00
|
| Währung9|$#,##0;($#,##0) |
| Währung10|$#,##0;($#,##0) |
| Währung11|$#,##0.00;($#,##0.00) |
| Währung12|$#,##0.00;($#,##0.00) |
| Prozentsatz1|0% |
| Prozentsatz2|0.00% |
| Wissenschaftlich1| 0.00E+00|
| Wissenschaftlich2|## 0.0E+00
|
| Bruchteil1|# ?/?
|
| Bruchteil2|# */*
|
| Datum1| M/T/JJ|
| Datum2| t-mm-jj|
| Datum3| d-mm|
| Datum4| mmm-jj|
| Zeit1| h:mm AM/PM|
| Zeit2| h:mm:ss AM/PM|
| Zeit3| hmm|
| Zeit4| h:mm:ss|
| Zeit5| m/d/jj h:mm|
| Zeit6| mm:ss|
| Zeit7| h:mm:ss|
| Zeit8| mm:ss.0|
| Buchhaltung1|_ * #,##0_ ;_ * "_ ;_ @_ |
| Buchhaltung2|_ $* #,##0_ ;_ $* "_ ;_ @_ |
| Buchhaltung3|_ * #,##0.00_ ;_ * "??_ ;_ @_ |
| Buchhaltung4|_ $* #,##0.00_ ;_ $* "??_ ;_ @_ |
| Text|@ |
| EasternDate1| JJJJ?M?|
| EasternDate2| M?D?|
| EasternDate3| M?D?|
| EasternDate4|M/T/JJ|
| EasternDate5| JJJJ?M?D?|
| EasternDate6| JJJJ?M?|
| EasternDate7| JJJJ?M?|
|EasternDate8| M?D?|
| EasternDate9| JJJJ?M?|
| EasternDate10| M?D?|
| EasternDate11| M?D?|
| EasternDate12| JJJJ?M?|
| EasternDate13| M?D?|
| EasternTime1| hmm?|
| EasternTime2| h?mm?ss?|
| EasternTime3| tth?mm?|
| EasternTime4| tth?mm?ss?|
| EasternTime5| tth?mm?|
| EasternTime6| tth?mm?ss?|

