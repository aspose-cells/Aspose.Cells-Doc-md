---
title: XML Map innerhalb der Arbeitsmappe mit der XmlMapCollection.Add Methode hinzufügen
type: docs
weight: 10
url: /de/python-net/add-xml-map-inside-the-workbook-using-xmlmapcollection-add-method/
---

## **Mögliche Verwendungsszenarien**

Aspose.Cells für Python via .NET bietet die Methode [**XmlMapCollection.add()**](https://reference.aspose.com/cells/python-net/aspose.cells/xmlmapcollection/add/#str), mit der Sie Ihren XML-Map innerhalb des Arbeitsbuchs importieren können.

## **XML Map innerhalb der Arbeitsmappe mit der XmlMapCollection.Add Methode hinzufügen**

Der folgende Beispielcode fügt mithilfe der Methode [**XmlMapCollection.add()**](https://reference.aspose.com/cells/python-net/aspose.cells/xmlmapcollection/add/#str) eine XML-Map in die Arbeitsmappe ein und speichert sie als [Ausgabedatei](5115434.xlsx). Der Screenshot zeigt die XML-Map, die aus [sample.xml](5115433.xml) in die Ausgabedatei importiert wurde.

![add-xml-map](add-xml-map.png)

{{< gist "aspose-cells-gists" "7bb30376b4d40cdfd596286870fb9752" "XmlMaps-AddXmlMapInsideWorkbook.py" >}}

