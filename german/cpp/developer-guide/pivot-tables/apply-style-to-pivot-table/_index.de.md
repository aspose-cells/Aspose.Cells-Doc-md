---
title: Anwenden von Stilen auf Pivot-Tabellen in Aspose.Cells for C++
linktitle: Anwenden von Stilen auf Pivot-Tabellen
description: Erfahren Sie, wie Sie integrierte und benutzerdefinierte Stile auf Pivot-Tabellen in Aspose.Cells for C++ anwenden, einschließlich Legacy-XLS-Autoformaten, modernen benannten Stilen aus Excel 2007+, benutzerdefinierten Pivot-Tabellenstilen und der FormatAll-Verknüpfung.
keywords: Aspose.Cells C++ Pivot-Tabellenstil, PivotTableStyleType, AutoFormatType, FormatAll, benutzerdefinierter Stil, PivotTableStyleName, TableStyles
type: docs
weight: 200
...
{{% alert color="primary" %}}
Aspose.Cells unterstützt sowohl das Anwenden von legacy Pivot-Autoformaten (vorgesehen für `.xls`-Dateien) als auch von modernen benannten oder benutzerdefinierten Pivot-Tabellenstilen (vorgesehen für `.xlsx`-, `.xlsm`- und `.xlsb`-Dateien). Welche API Sie aufrufen sollten, hängt vom Dateiformat ab, in dem die Arbeitsmappe gespeichert wird, nicht vom Format, aus dem sie geladen wurde.
{{% /alert %}}
...
- `PivotTable.PivotTableStyleName` wählt einen benutzerdefinierten Stil aus, den Sie selbst über `Worksheets.TableStyles.AddPivotTableStyle(...)` definieren. Benutzerdefinierte Stile sind erforderlich, wenn Sie Farben, Rahmen oder Schriftarten ändern möchten, die über die Voreinstellungen hinausgehen.
...
int main() {
    ...
    // Füllen Sie die Quelldaten mit einer Kopfzeile (Frucht, Jahr, Betrag)
    // und 9 Datenzeilen, die Trauben, Heidelbeeren, Kiwi, Kirsche über 2020 und 2021 abdecken
    ...
    // Wenden Sie das alte XLS-Voreinstellungs-Autoformat "Report5" an
    pivotTable.SetAutoFormatType(PivotTableAutoFormatType::Report5);
    // Speichern Sie die Arbeitsmappe im alten .xls-Format
...
## **Anwenden eines modernen benannten Voreinstellungs-Pivot-Tabellenstils**
...
2. Konfigurieren Sie den Stil, indem Sie Elemente (wie `WholeTable` oder `GrandTotalRow`) über `TableStyle.TableStyleElements.Add(TableStyleElementType)` hinzufügen und jedem Element über `TableStyleElement.SetElementStyle(Style)` einen `Style` zuweisen.
...
int main() {
    ...
    // Schritt 3: Ein GrandTotalRow-Element hinzufügen und eine fettrote Schrift anwenden
    ...
    // Schritt 4: Den benutzerdefinierten Stil nach Namen anwenden (NICHT nach PivotTableStyleType, der für eingebaute Voreinstellungen ist)
...
|---|---|---|
| `.xls` (Legacy) | `PivotTable.AutoFormatType` | Werte aus `Aspose.Cells.Pivot.PivotTableAutoFormatType` (z. B. `Report1`–`Report10`, `Classic`, `Table1`–`Table10`). Beim Speichern in modernen Formaten ignoriert. |
