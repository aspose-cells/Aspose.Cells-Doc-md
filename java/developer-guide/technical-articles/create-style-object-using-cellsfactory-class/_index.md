---
title: Create Style object using CellsFactory class
type: docs
weight: 1040
url: /java/create-style-object-using-cellsfactory-class/
---

## **Create Style object using CellsFactory class**
The following sample code creates [Style](https://apireference.aspose.com/java/cells/com.aspose.cells/Style) object using [CellsFactory](https://apireference.aspose.com/java/cells/com.aspose.cells/CellsFactory) class and then sets the Default Style of the workbook. Please download the [output excel file](attachments/5276196/5472583.xlsx) to see the results of this code for your reference.
## **Sample Code**
{{< gist "aspose-com-gists" "a20e8fa273e7cfa37d032b8211fcf8bf" "Examples-src-main-java-com-aspose-cells-examples-articles-CreateStyleobjectusingCellsFactoryclass-CreateStyleobjectusingCellsFactoryclass.java" >}}
