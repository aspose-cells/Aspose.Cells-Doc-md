---
title: Support for XAdES Signature
type: docs
weight: 110
url: /java/support-for-xades-signature/
---

# **Support for XAdES Signature**
Aspose.Cells provides supports signing workbooks with XAdES Signature. For this, the API provides [DigitalSignature](https://apireference.aspose.com/java/cells/com.aspose.cells/DigitalSignature) class and [XAdESType](https://apireference.aspose.com/java/cells/com.aspose.cells/XAdESType) enumeration.

The following code snippet demonstrates the use of the [DigitalSignature](https://apireference.aspose.com/java/cells/com.aspose.cells/DigitalSignature) class to sign the [source](sourceFile.xlsx) workbook.
### **Sample Code**
{{< gist "aspose-com-gists" "439a68a5e4305388c50ca306ef238de5" "Examples-src-AsposeCellsExamples-Workbook-XAdESSignatureSupport-1.java" >}}
