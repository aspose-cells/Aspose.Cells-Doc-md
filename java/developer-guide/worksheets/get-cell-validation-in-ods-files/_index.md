---
title: Get Cell Validation in ODS Files
type: docs
weight: 190
url: /java/get-cell-validation-in-ods-files/
---

## **Get Cell Validation in ODS Files**
With Aspose.Cells,  you can get the validation applied to a cell in ODS files. For this, the API provides the [GetValidation](https://apireference.aspose.com/java/cells/com.aspose.cells/cell#getValidation\(\)) method of the [Cell](https://apireference.aspose.com/java/cells/com.aspose.cells/Cell) class.

The following code sample demonstrates the use of the [GetValidation](https://apireference.aspose.com/java/cells/com.aspose.cells/cell#getValidation\(\)) method by loading the [source ODS](SampleBook1.ods) file and reading the validation of the cell A9.
#### **Sample Code**
{{< gist "aspose-com-gists" "439a68a5e4305388c50ca306ef238de5" "Examples-src-AsposeCellsExamples-Worksheets-GetCellValidationInODS-1.java" >}}
