...
title: 支持的数字格式列表
linktitle: 支持的数字格式列表
description: Aspose.Cells 是 Node.js 中处理电子表格文件的库，支持多种数字格式。本文提供了支持的数字格式列表，方便用户根据需要选择合适的格式。
keywords: Aspose.Cells，Node.js库，电子表格，数字格式，列表，支持
type: docs
weight: 30
url: /zh/nodejs-cpp/list-of-supported-number-formats/
---

## **Aspose.Cells**
Aspose.Cells 组件提供一些内置的数字格式，用于配置数字和日期的显示格式。这些内置数字格式可以通过使用 **Style** 对象的 **Number** 属性来应用。所有内置数字格式都具有唯一的数字值。开发者可以将任何想要的数字值赋给 **Style** 对象的 **Number** 属性，从而应用相应的显示格式。这种方法速度更快。Aspose.Cells 支持的内置数字格式如下：
...
在某些数字格式中，你可能会注意到一些字符，比如月。这实际上是中文字符，可以在 MS Excel 的中文版和日文版中使用。
