---
title: Aspose.Cells for .NET的新功能
type: docs
description: "Aspose.Cells for .NET不断扩展和增强。您可以在此页面了解产品的巨大和最有趣的特性。"
weight: 5
...
* 支持删除空白行/列的更多选项
* 支持获取或更新 GridWeb 在 .Net6 中的服务器端信息
...
* 支持转置范围内的数据
* 支持获取高级筛选设置的属性
* 支持将单元格中放置的图片呈现为pdf/图片
* 支持将样式复制到GridJS的自动填充
* 支持GridJS的图表颜色设置

## Aspose.Cells for .NET 24.4
* 为用户的便利提供中断监视器的简单实现
* 在将OLE对象转换为pdf时支持嵌入附件
* 支持将ChartX的自动形状呈现为图片
* 支持将图片嵌入单元格（ms excel中的"放置在单元格内"操作）
* 改进导入Numbers文件：合并单元格，隐藏行/列，……等

## Aspose.Cells for .NET 24.3
* 支持导入AZW3文件
* 支持以odf 1.3标准保存ods
* 支持计算EVALUATE函数
* 支持GridDesktop的筛选功能：前10，日期，文本
* 在GridJS的插入菜单和帮助提示中添加更多公式
* 为GridJS的公式菜单添加分类特性
当计算结果为日期时间时, 自动将单元格设置为GridJS的日期格式

## Aspose.Cells for .NET 24.2
* 支持新的函数: TEXTAFTER, TEXTBEFORE, TEXTSPLIT, TOROW, TOCOL
* 支持检测oxps格式
* 改进对大型数据集排序的性能
* 增强查询操作功能: GridDesktop的数字过滤, 关键词搜索, 清除筛选

## Aspose.Cells for .NET 24.1

* 支持 NET8
* 支持导入包含用户指定标头行/列和每个工作表总行/列的csv数据
* 支持导出带有自定义样式的html
* 支持合并区域的自动填充
* 支持从右向左、从下往上自动填充数据
...
* 支持转换为pdf时符合PDF/A-2和PDF/A-3标准
* 支持对数据透视表进行离散分组计算
* 支持将单元格数据导出为通用列表
* 支持使用链接表导入交叉引用的智能标记
* 支持公式的MAKEARRAY函数
* 支持GridJS渲染具有旋转角度的单元格文本
* 对于GridDesktop导入/导出数据表格，支持更多选项
* 支持在GridDesktop中旋转图片渲染
* 支持调整GridDesktop中工作表的顺序
* 支持在GridDesktop中显示/隐藏公式警告消息的选项 

## Aspose.Cells for .NET 23.11

* 支持将电子表格转换为PDF文件版本1.7(ISO32000-1:2008)
* 支持在GridJS中插入表单控件
* 支持在GridJS中显示单元格背景的渐变填充
* 支持在GridDesktop中复制/删除/插入/重命名工作表
* 在GridDesktop进行粘贴操作时支持自动扩展最大行/列
* 支持GridDesktop的SheetPrintingPreview
* 提供GridDesktop的.NET Core构建 

## Aspose.Cells for .NET 23.10

* 支持在目标范围的大小与源范围不同时进行自适应粘贴操作
* 支持将超链接直接设置为一个范围
* 支持同时复制多个工作表
* 支持检查一个范围的单元数据是否为空
...
* 支持在公式中计算新函数：LAMBDA,LET,MAP,SCAN
* 支持导入/导出EPUB文件
* 支持过滤图表系列
* 为GridDesktop支持本地化
* 为.net7-windows平台提供GridWeb

## Aspose.Cells for .NET 23.8

* 支持保留公式中的填充空格
* 支持以富文本格式替换/插入文本的Cell
* 支持在GridJs中呈现活动控件和旋钮的表单控件
* 为GridJs支持更多类型的验证: 文本长度/小数，选择列表，公式表达式

## Aspose.Cells for .NET 23.7

* 对GridJS的许多功能进行增强，例如渲染控件，combobox的服务器端保存，...
* 对一些新格式进行了增强的文件格式检测，如visio文件，OneNote包文件。
* 为CSharp X509Certificate2支持XAdES签名
* 支持在Blazor Web Assembly中使用

## Aspose.Cells for .NET 23.6

* 支持导出和突出显示修订日志
* 支持按相反顺序从RowCollection中遍历现有的Row对象
* 支持刷新数据透视表的多个合并范围
* 在GridJs中支持显示图表工作表
* 在GridJs中支持显示单元格溢出内容
* 在GridJs中支持显示背景图片
...
* 支持更新外部工作簿中外部工作表数据的外部引用到当前工作簿的本地工作表
* 支持将复选框控件呈现为交互控件而不是静态图片
* 支持选择数据透视区域并进行格式化
* 支持在转换为pdf时添加水印
...
* 支持计算函数 ENCODEURL
* 支持逐步插入相关节点完成方程形状的构建
* 支持计算公式表达式作为数组公式而无需将其设置为单元格对象
* 支持在LightCells模式下保存xlsb文件
* 支持检查某一字体是否已安装或可用
* 支持在列表对象中将公式设置到指定的单元格
* 支持将工作簿保存为xhtml
* 支持为GridWeb渲染activex控件和表单控件

## Aspose.Cells for .NET 23.3

* 支持设置/读取/保存新函数: LET, ENCODEURL
* 调用公式计算时，支持将数据源和外部链接关联

## Aspose.Cells for .NET 23.2

* 支持设置/读取/保存新函数: SCAN, LAMBDA
* 支持将样式导出为json

## Aspose.Cells for .NET 23.1

* 支持新函数：CHOOSEROWS，CHOOSECOLS，HSTACK，VSTACK
* 支持从修订版本中获取作者和日期时间

## Aspose.Cells for .NET 22.12

* 支持将Excel导出为具有多层次结构的JSON
...
* 支持将注释和批注导出为显示在工作表上的HTML
* 支持指定要转换为PDF/XPS的工作表

## Aspose.Cells for .NET 22.7

* 支持将HTML导出为带有页面页眉/页脚的单个文件
* 支持获取Range的绝对位置和大小(以点为单位)

## Aspose.Cells for .NET 22.6

* 改进了在公式计算链中计算和跟踪公式的性能

## Aspose.Cells for .NET 22.5

* 改进了在LightCells模式下设置单元格公式时的内存性能

## Aspose.Cells for .NET 22.4

* 增强了控制导出CSV中空单元格的选项。
* 支持对条件格式进行复杂的拆分和更新以进行插入/删除操作

## Aspose.Cells for .NET 22.3

* 在启用工作簿的计算链时，支持重新计算依赖于已定义名称的单元格
* 根据公式的计算链递归地获取单元格的叶子

## Aspose.Cells for .NET 22.2

* 支持按指定范围过滤器从NameCollection中提取已定义名称

## Aspose.Cells for .NET 22.1

* 支持生成可编辑的PPTX
* 在数据行/列超出限制时，支持将大型CSV导入到多个工作表
* 支持在LightCells中导入CSV
* 支持移除指定外部链接
* 在移除外部链接时保留公式
...
* 支持将Excel转换为SQL脚本
* 支持导入和导出纯xml数据
* 支持从损坏的xls文件中尽可能多地读取数据
* 支持中断SmartMarker的过程
* 改进添加条件格式和验证范围的性能
* 将新添加的格式条件设置为最高优先级，就像ms excel的行为一样
