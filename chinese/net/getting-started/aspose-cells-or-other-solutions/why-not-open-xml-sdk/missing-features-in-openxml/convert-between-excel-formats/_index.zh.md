...
Aspose.Cells for .NET 支持独立于其他软件从电子表格转换为PDF。使用 Workbook 类的 Save 方法将 Excel 文件保存为 PDF。 Save 方法提供将本机 Excel 文件转换为 PDF 格式的 SaveFormat.Pdf 枚举成员。
...
1. 您可以**打开/加载**一个现有的模板文件，如果您是从头开始创建工作簿，则跳过此步骤。
...
**HTML格式**将普通 HTML 与外部资源（即通常链接进来的内容，如图像、动画、音频等）合并到一个文件中。它们用于文件扩展名为 .mht 的电子邮件。
