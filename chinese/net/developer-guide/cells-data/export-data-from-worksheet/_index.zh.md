...
本文介绍了如何使用C#将工作表数据导出到数据表。它涵盖了以下主题

_格式_: **Excel**
- [C#将Excel转换为数据表](#csharp-excel-to-datatable)
- [C#将Excel转换为数据表](#csharp-convert-excel-to-datatable)
- [C#将Excel导入到数据表](#csharp-import-excel-to-datatable)
- [C#从Excel导出到数据表](#csharp-export-to-datatable-from-excel)

_格式_: **XLS**
- [C#将XLS转换为数据表](#csharp-xls-to-datatable)
- [C#将XLS转换为数据表](#csharp-convert-xls-to-datatable)
- [C#将XLS导入到数据表](#csharp-import-xls-to-datatable)
- [C#从XLS导出到数据表](#csharp-export-to-datatable-from-xls)

_格式_: **XLSX**
- [C#将XLSX转换为数据表](#csharp-xlsx-to-datatable)
- [C#将XLSX转换为数据表](#csharp-convert-xlsx-to-datatable)
- [C#将XLSX导入到数据表](#csharp-import-xlsx-to-datatable)
- [C#从XLSX导出到数据表](#csharp-export-to-datatable-from-xlsx)

_格式_: **ODS**
- [C#将ODS转换为数据表](#csharp-ods-to-datatable)
- [C#将ODS转换为数据表](#csharp-convert-ods-to-datatable)
- [C#将ODS导入到数据表](#csharp-import-ods-to-datatable)
- [C#从ODS导出到数据表](#csharp-export-to-datatable-from-ods)

## **如何使用C#导出Excel数据**

{{% alert color="primary" %}}

本文讨论了开发人员通过Aspose.Cells可以访问的一些数据导出技术。

{{% /alert %}}

## **如何从工作表导出数据**

Aspose.Cells不仅使用户能够从外部数据源导入数据到工作表，还允许他们将工作表数据导出到[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)。正如我们所知，[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8) 是ADO.NET的一部分，用于保存数据。一旦数据存储在[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8) 中，就可以根据用户的需求以任何方式使用。开发人员还可以直接将这些数据（存储在[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8) 中）存储到数据库中，如果他们愿意的话。因此，我们可以看到，如果将工作表数据导出到[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)，那么开发人员更容易操纵工作表数据。

## **如何使用Aspose.Cells将数据导出到数据表中**

开发人员可以通过调用[**Cells**](https://reference.aspose.com/cells/net/aspose.cells/cells)类的[**ExportDataTable**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatable/index)或[**ExportDataTableAsString**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatableasstring/index)方法，轻松地将他们的工作表数据导出到[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)对象中。这两种方法在不同的场景中使用，在下面会更详细地讨论。

## **包含强类型数据的列**

我们知道电子表格将数据存储为行和列的序列。如果工作表的列中所有值都是强类型的（即列中的所有值必须具有相同的数据类型），那么我们可以通过调用[**Cells**](https://reference.aspose.com/cells/net/aspose.cells/cells)类的[**ExportDataTable**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatable/index)方法导出工作表内容。[**ExportDataTable**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatable/index)方法接受以下参数，将工作表数据导出为[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)对象：

- **行号**，将要从中导出数据的第一个单元格的行号。
- **列号**，将要从中导出数据的第一个单元格的列号。
- **行数**，要导出的行数。
- **列数**，要导出的列数。
- **导出列名**，一个布尔属性，指示是否应将工作表的第一行数据导出为[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)的列名。
...
1. 在[Workbook](https://reference.aspose.com/cells/net/aspose.cells/workbook/)对象中加载您的Excel文件。
   - [Workbook](https://reference.aspose.com/cells/net/aspose.cells/workbook/)对象可以加载多种Excel文件格式，如XLS，XLSX，XLSM，ODS等。
3. 访问Excel文件中的第一个[Worksheet](https://reference.aspose.com/cells/net/aspose.cells/worksheet/)。
4. 选择您的导出区域，例如从**DataTable**的第1个单元格开始，选择7行和2列。
...
如果工作表的所有列中的值都不是强类型的(这意味着列中的值可能具有不同的数据类型)，那么我们可以通过调用[**Cells**](https://reference.aspose.com/cells/net/aspose.cells/cells)类的[**ExportDataTableAsString**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatableasstring/index)方法来导出工作表内容。[**ExportDataTableAsString**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatableasstring/index)方法与[**ExportDataTable**](https://reference.aspose.com/cells/net/aspose.cells/cells/methods/exportdatatable/index)方法具有相同的参数集，用于将工作表数据导出为[**DataTable**](https://docs.microsoft.com/en-gb/dotnet/api/system.data.datatable?view=netframework-4.8)对象。
...
## **高级主题**
- [将Excel数据导出到DataTable，不带任何格式](/cells/zh/net/export-excel-data-to-datatable-without-any-formatting/)
...
- [在导出工作表数据到数据表时忽略隐藏列](/cells/zh/net/ignore-hidden-columns-while-exporting-worksheet-data-to-data-table/)
