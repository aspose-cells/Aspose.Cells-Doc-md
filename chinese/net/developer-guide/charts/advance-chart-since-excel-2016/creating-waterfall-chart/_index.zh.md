...
瀑布图是一种特殊类型的图表，通常用于演示起始位置如何增加或减少。 Microsoft Excel 有许多预定义的图表类型，包括柱形图、折线图、饼图、条形图、雷达图等，但瀑布图超出了基本图表的范围，可以使用现有的图表类型创建，只需很少或更多的自定义。

{{% /alert %}} 

 Aspose.Cells API 允许借助折线图创建瀑布图。 API 还允许自定义图表外观，通过设置[**系列.UpBars**](https://reference.aspose.com/cells/net/aspose.cells.charts/series/properties/upbars) & [**系列.DownBars**](https://reference.aspose.com/cells/net/aspose.cells.charts/series/properties/downbars)特性。
