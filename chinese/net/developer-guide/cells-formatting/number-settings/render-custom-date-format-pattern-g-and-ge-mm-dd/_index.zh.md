---
title: 呈现自定义日期格式模式 g 和 ge mm dd
type: docs
weight: 160
url: /zh/net/render-custom-date-format-pattern-g-and-ge-mm-dd/
---
{{% alert color="primary" %}}

 Aspose.Cells 现在能够呈现自定义日期格式模式，如 g、ge.mm.dd 等。请检查附件[源文件](5112361.xlsx)和[转换后的PDF](5112360.pdf) by Aspose.Cells 供您参考。

{{% /alert %}}

下面的示例代码将[源文件](5112361.xlsx)其中包含具有自定义格式模式的日期值，例如 g 和 ge.mm.dd[输出PDF](5112360.pdf).

{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Examples-CSharp-Articles-StylingAndDataFormatting-RenderCustomDateFormat-RenderCustomDateFormatPatterngandgemmdd.cs" >}}
