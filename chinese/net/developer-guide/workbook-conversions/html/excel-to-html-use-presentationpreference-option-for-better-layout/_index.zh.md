---
title: Excel 到 HTML - 使用 PresentationPreference 选项以获得更好的布局
type: docs
weight: 220
url: /zh/net/excel-to-html-use-presentationpreference-option-for-better-layout/
---
{{% alert color="primary" %}} 

Aspose.Cells 为需要在将 Microsoft Excel 文件保存为 HTML 或 MHT 格式时呈现更好布局的开发人员提供了一个有用的 HtmlSaveOptions.PresentationPreference 属性。该属性的默认值为 false。我们建议将此属性设置为 true 以获得更具吸引力的 Excel 报告显示。

{{% /alert %}} 

请参阅下面的示例代码，该代码演示了如何在显示首选项打开的情况下从 Excel 报告中呈现 HTML 文件。



{{< gist "aspose-cells-gists" "88c9872508ec3150c552eb5155edf06e" "Examples-CSharp-Articles-UsePresentationPreferenceOption-1.cs" >}}
