...
Microsoft Excel 支持 Web 浏览器不支持的某些类型的虚线边框。当您使用 Aspose.Cells 将此类 Excel 文件转换为 HTML 时，将删除此类边框。但是，Aspose.Cells也可以支持显示这样的边框[**HtmlSaveOptions.ExportSimilarBorderStyle**](https://reference.aspose.com/cells/net/aspose.cells/htmlsaveoptions/properties/exportsimilarborderstyle)财产。请将其值设置为**真的**并且不支持的边框也将导出到 HTML 文件。
