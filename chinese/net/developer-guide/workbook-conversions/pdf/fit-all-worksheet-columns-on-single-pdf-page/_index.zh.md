...
有时您想要生成一个 PDF 文件，将工作表的所有列都放在一页上。这[**PdfSaveOptions.AllColumnsInOnePagePerSheet**](https://reference.aspose.com/cells/net/aspose.cells/pdfsaveoptions/properties/allcolumnsinonepagepersheet)属性以非常易于使用的方式提供此功能。输出 PDF 的高度和宽度等复杂计算在内部处理，并基于工作表中的数据。

{{% /alert %}}

## **在单个页面上调整工作表列 PDF**

[**PdfSaveOptions.AllColumnsInOnePagePerSheet**](https://reference.aspose.com/cells/net/aspose.cells/pdfsaveoptions/properties/allcolumnsinonepagepersheet)确保工作表中的所有列都呈现到单个 PDF 页面，但根据工作表中的数据，行可能会扩展到多个页面。
