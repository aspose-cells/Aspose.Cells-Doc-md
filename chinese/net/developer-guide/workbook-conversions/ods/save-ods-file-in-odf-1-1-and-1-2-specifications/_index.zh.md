...
Aspose.Cells支持在ODF（OpenDocument Format）1.1和1.2规格中保存ODS文件（OpenDocument Spreadsheet）。Aspose.Cells具有[**OdsSaveOptions.IsStrictSchema11**](https://reference.aspose.com/cells/net/aspose.cells/odssaveoptions/properties/isstrictschema11)属性，指定在保存ODS文件时使用ODF 1.1规格。此属性的默认值为false，因此不使用此设置保存的ODS文件将使用1.2规格。
