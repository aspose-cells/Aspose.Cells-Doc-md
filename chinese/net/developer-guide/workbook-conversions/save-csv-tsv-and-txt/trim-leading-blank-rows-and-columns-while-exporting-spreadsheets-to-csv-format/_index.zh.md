...
下面的示例代码加载[源文件](sampleTrimBlankColumns.xlsx)其中有两个前导空白列。它首先将 excel 文件保存为 CSV 格式，不做任何更改，然后设置**[TxtSaveOptions.TrimLeadingBlankRowAndColumn](https://reference.aspose.com/cells/net/aspose.cells/txtsaveoptions/properties/trimleadingblankrowandcolumn)**财产给**真的**并再次保存。屏幕截图显示[源文件](sampleTrimBlankColumns.xlsx), [输出 CSV 文件而不修剪](outputWithoutTrimBlankColumns.csv)和[输出带修整的 CSV 文件](outputTrimBlankColumns.csv).
