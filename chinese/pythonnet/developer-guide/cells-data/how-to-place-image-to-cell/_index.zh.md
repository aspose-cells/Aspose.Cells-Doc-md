...
description: 了解如何使用Aspose.Cells for Python via .NET API将图片插入单元格。
keywords: Python Excel库，Python如何在单元格中插入图片，Python在单元格上插入图片，Python在单元格中放置图片，Python在单元格上放置图片，Python如何将图像放入单元格，Python如何在单元格上放置图像，Python在图片在单元格中和在单元格上放置之间如何切换，Python在放置在单元格和放在单元格上之间如何切换。
---
...
## **如何使用Aspose.Cells for Python Excel库在单元格中插入图片**
使用Aspose.Cells for Python via .NET插入单元格中的图片。请参阅以下示例代码。执行示例代码后，将在单元格中插入一张图片。
1. 实例化一个工作簿对象。 
2. 获取要插入图片的单元格。
