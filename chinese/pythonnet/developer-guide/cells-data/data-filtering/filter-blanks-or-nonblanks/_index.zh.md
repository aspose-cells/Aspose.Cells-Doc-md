...
description: 学习如何使用 Aspose.Cells for Python via .NET API 筛选空白和非空白。
keywords: Python Excel 库，Python 筛选空白，Python 筛选非空白，Python 工作表中筛选空白，Python 工作表中筛选非空白，Python Excel 中筛选空白，Python Excel 中筛选非空白，Python Excel 中筛选空白和非空白
---
...
## **如何使用Aspose.Cells for Python Excel库筛选空白**
如果某列包含文本，以至于少数单元格为空白，并且需要筛选出仅包含空白单元格的行，则可以使用[AutoFilter.match_blanks(field_index)](https://reference.aspose.com/cells/python-net/aspose.cells/autofilter/match_blanks/#int)和[AutoFilter.add_filter(field_index, criteria)](https://reference.aspose.com/cells/python-net/aspose.cells/autofilter/add_filter/#int)函数，如下所示。 
...
## **如何使用Aspose.Cells for Python Excel库筛选非空**

请参阅以下示例代码，该代码加载包含一些虚拟数据的[示例Excel文件](sample.xlsx)。加载文件后，调用[AutoFilter.match_non_blanks(field_index)](https://reference.aspose.com/cells/python-net/aspose.cells/autofilter/match_non_blanks/#int)函数来过滤非空数据，最后将工作簿另存为[输出Excel文件](FilteredNonBlanks.xlsx)。 
