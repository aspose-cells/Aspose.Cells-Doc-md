---
title: 支持的数字格式列表
description: Aspose.Cells 是支持多种数字格式的 Python 电子表格处理库。本文提供了支持的数字格式列表，供用户根据需求选择适合的格式。
keywords: Aspose.Cells，Python 电子表格库，数字格式，列表，支持
type: docs
weight: 30
url: /zh/python-net/list-of-supported-number-formats/
---

## **Aspose.Cells for Python via .NET**
Aspose.Cells for Python via .NET 提供一些内置的数字格式，用于配置数字和日期的显示格式。这些内置格式可以通过 `Style` 对象的 **Number** 属性应用。所有内置数字格式都具有唯一的数字值。开发者可以为 **Number** 属性赋值任何所需的数字，从而应用显示格式。这种方法更快。以下是 Aspose.Cells for Python via .NET 支持的内置数字格式：
