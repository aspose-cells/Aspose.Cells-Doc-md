...
type: docs
weight: 10
url: /zh/python-net/how-to-format-number-to-time/
description: 本文将介绍如何用 Aspose.Cells for Python via .NET API 将数字格式化为时间。
...
## **如何在 Aspose.Cells for Python via .NET 中将数字格式化为时间**
在 Aspose.Cells for Python via .NET 中将数字格式化为时间是一个简单的过程，涉及给单元格或单元格区域应用自定义数字格式。Aspose.Cells 是一个强大的库，允许您在无需安装 Microsoft Excel 的情况下处理 Excel 文件。以下是格式化时间的步骤：
