---
title: 在使用 Golang 通过 C++ 导入数据到工作表时指定公式字段
linktitle: 在导入数据到工作表时指定公式字段
type: docs
weight: 300
url: /zh/go-cpp/specify-formula-fields-while-importing-data-to-worksheet/
...
您可以使用[**ImportTableOptions.GetIsFormulas()**](https://reference.aspose.com/cells/go-cpp/importtableoptions/getisformulas/)在导入数据到工作表时指定公式字段。此属性接受布尔数组，其中值true表示该字段是公式字段。例如，如果第三个字段是公式字段，则数组中的第三个值将为true。
