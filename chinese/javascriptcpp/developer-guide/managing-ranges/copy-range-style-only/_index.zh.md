---
title: 仅复制范围样式，通过 JavaScript 由 C++ 实现
linktitle: 仅复制范围样式
type: docs
weight: 620
url: /zh/javascript-cpp/copy-range-style-only/
description: 学习如何在操作数据的同时，仅复制范围的样式，使用 C++ 的脚本实现。 
---

{{% alert color="primary" %}}

[仅复制范围数据](/cells/zh/javascript-cpp/copy-range-data-only/) 和 [复制范围带样式](/cells/zh/javascript-cpp/copy-range-data-with-style/) 说明了如何单独或完整地将数据（以及格式）从一个范围复制到另一个范围。也可以只复制格式。本文将展示方法。
