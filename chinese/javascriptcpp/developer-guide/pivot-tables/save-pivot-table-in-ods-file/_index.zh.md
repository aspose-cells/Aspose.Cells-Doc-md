...
Aspose.Cells for JavaScript通过C++支持将Pivot表保存为ODS格式文件。你只需将包含已有Pivot表的工作簿转换，或创建新的Pivot表，然后以ODS格式保存。确保在保存前调用[**PivotTable.calculateData**](https://reference.aspose.com/cells/javascript-cpp/pivottable/#calculatedata--)，以确保Pivot表在输出的ODS文件中正确渲染。以下代码片段演示了如何将Pivot表保存为ODS文件。
