---
title: 用JavaScript用C++实现扩展Aspose.Cells默认计算引擎的自定义计算引擎
linktitle: 实现自定义计算引擎以扩展Aspose.Cells的默认计算引擎
description: 本文介绍如何通过用JavaScript用C++利用Aspose.Cells库实现自定义计算引擎，扩展JavaScript中的默认计算引擎。加载现有Excel文件或创建新文件，使用提供的方法，保存修改后的Excel文件。
keywords: Aspose.Cells，Excel，自定义计算引擎，JavaScript用C++
type: docs
weight: 80
url: /zh/javascript-cpp/implement-custom-calculation-engine-to-extend-the-default-calculation-engine-of-aspose-cells/
---
...
以下代码实现了自定义计算引擎。它实现了接口 [**AbstractCalculationEngine**](https://reference.aspose.com/cells/javascript-cpp/abstractcalculationengine)，其中包含一个 [**calculate(CalculationData data)**](https://reference.aspose.com/cells/javascript-cpp/abstractcalculationengine/#calculate-calculationdata-) 方法。该方法会对所有公式调用。在此方法中，我们捕获 **TODAY** 函数并在系统日期基础上加一天。因此，如果当前日期是 2023/07/27，则自定义引擎会将 TODAY() 计算为 2023/07/28。
...
[无需在工作表中编写，直接计算自定义函数](/cells/zh/javascript-cpp/direct-calculation-of-custom-function-without-writing-it-in-a-worksheet/)
