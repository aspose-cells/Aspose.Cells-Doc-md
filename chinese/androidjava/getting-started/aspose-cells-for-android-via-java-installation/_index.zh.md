---
title: Aspose.Cells for Android 通过 Java 安装
type: docs
weight: 30
url: /zh/java/aspose-cells-for-android-via-java-installation/
---
## **系统要求**
Aspose.Cells for Android via Java 与平台无关，可以在安装了 Android Runtime 环境的任何平台上使用，并将在运行 Android OS 2.0 或更高版本的 Android 系统上运行。目前，该组件已通过以下测试：

- 安卓 5.1 v 22
## **通过 Maven 存储库中的 Java 安装适用于 Android 的 Aspose.Cells**
...
{{< /highlight >}}
## **如何通过 Java 为 Android 使用 Aspose.Cells**
本主题将指导您完成在 Android Studio IDE 中通过 Java 为 Android 设置 Aspose.Cells 的必要步骤，假设您已经在您的计算机上安装了最新版本的 Android Studio，并且您还通过 Java 为 Android 获取了最新版本的 Aspose.Cells包裹。
...
Aspose.Cells for Android via Java 包可以从下载[这里](https://downloads.aspose.com/cells/androidjava).请注意，通过 Java 发布的 Aspose.Cells for Android 的每个发布包主要包含 2 个文件，详述如下。

- **aspose-细胞-xxxjar**是包含从 Aspose.Cells for Android 到 Java API 的所有命名空间的主库文件。
- **aspose-细胞-xxx-libs.apk**是包含第 3 方 bcprov-jdk15-146.jar 的 APK，用于 Aspose.Cells 通过 Java API 为 Android 提供的加密和解密设施。

{{% /alert %}} 
### **通过 Android Studio 中的 Java 开始使用 Aspose.Cells for Android**
...
接下来，我们将通过 Java Jar 将适用于 Android 的 Aspose.Cells 添加到项目中。有 2 个重要步骤，详述如下。

- 通过 Java Jar 手动将 Aspose.Cells for Android 复制到**\应用\库**文件夹。
- 将 Aspose.Cells for Android 通过 Java Jar 作为库添加到模块中，如下所示。
