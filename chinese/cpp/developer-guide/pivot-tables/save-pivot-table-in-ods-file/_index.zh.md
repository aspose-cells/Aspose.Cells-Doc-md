...
Aspose.Cells 提供在 ODS 文件中保存数据透视表的功能。你可以将带有现有数据透视表的工作簿转换为文件，或者创建新的数据透视表并保存为 ODS 格式。在保存前调用 [**PivotTable::CalculateData()**](https://reference.aspose.com/cells/cpp/aspose.cells.pivot/pivottable/calculatedata/) 方法以确保数据透视表能在输出的 ODS 文件中正确渲染。以下代码示例演示如何将数据透视表保存为 ODS 文件。
