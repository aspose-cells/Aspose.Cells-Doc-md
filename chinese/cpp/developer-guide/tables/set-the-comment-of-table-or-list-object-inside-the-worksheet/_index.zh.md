---
title: 在工作表内设置表格或列表对象的备注
type: docs
weight: 20
url: /zh/cpp/set-the-comment-of-table-or-list-object-inside-the-worksheet/
---

{{% alert color="primary" %}} 

请使用[ListObject.SetComment](https://reference.aspose.com/cells/cpp/aspose.cells.tables/listobject/setcomment/)方法在工作表内设置表格或列表对象的备注。备注将显示在xl/tables/tableName.xml文件中。

{{% /alert %}} 
## **设置工作表内表格或列表对象的批注**
以下示例代码加载源Excel文件，并设置工作表内第一个表格或列表对象的注释。



{{< gist "aspose-cells-gists" "6f7d9819d85793c3a3b5d040af42e1a9" "Examples-C-main-SetCommentOfTableOrListObject-new.cpp" >}}
