...
如果你想要在Excel中关联切片器和数据透视表，你需要右键点击切片器并选择“报表连接...”项。在选项列表中，您可以对复选框进行操作。 类似地，如果您想要使用Aspose.Cells Java API以编程方式关联切片器和数据透视表，请使用[**Slicer.addPivotConnection(PivotTable pivot)**](https://reference.aspose.com/cells/java/com.aspose.cells/slicer#addPivotConnection(com.aspose.cells.PivotTable)/)方法。它将为切片器和数据透视表进行关联。
