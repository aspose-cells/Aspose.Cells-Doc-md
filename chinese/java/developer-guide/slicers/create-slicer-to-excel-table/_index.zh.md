...
切片器用于快速过滤数据。它可用于在表格或数据透视表中过滤数据。Microsoft Excel允许您通过选择表格或数据透视表，然后单击*插入 > 切片器* 来创建切片器。Aspose.Cells也允许您使用[**Worksheet.Slicers.Add()**](https://reference.aspose.com/cells/java/com.aspose.cells/slicercollection#add(com.aspose.cells.ListObject,%20com.aspose.cells.ListColumn,%20int,%20int))方法创建切片器。
