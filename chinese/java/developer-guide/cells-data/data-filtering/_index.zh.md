...
在每个工作表中，您只能指定一个筛选范围。这是由 Microsoft Excel 限制的。要进行自定义数据筛选，请使用 [**AutoFilter.Custom**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#custom(int,%20int,%20java.lang.Object) 方法。
...
Aspose.Cells提供了一个函数[**addFillColorFilter**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#addFillColorFilter(int,%20int,%20com.aspose.cells.CellsColor,%20com.aspose.cells.CellsColor))，可以根据单元格的填充颜色属性来过滤数据。在下面的示例中，使用了一个模板文件，模板文件的第一列有不同的填充颜色，用来测试颜色过滤功能。可以下载以下文件来检查功能。
...
可以实现不同类型的日期过滤，比如筛选所有行中日期为2018年1月的数据。以下示例代码演示了使用[**addDateFilter**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#addDateFilter(int,%20int,%20int,%20int,%20int,%20int,%20int,%20int))函数进行这种筛选。以下文件可用于测试此功能。
...
有时根据日期需要动态筛选，比如所有日期为一月的单元格，无论年份如何。在这种情况下，会使用[**DynamicFilter**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#dynamicFilter(int,%20int))函数，如下面示例代码中所示。以下文件可用于测试。
...
可以使用Aspose.Cells应用自定义过滤，比如选择处于给定范围内的数字。以下示例演示了使用[**custom()**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#custom(int,%20int,%20java.lang.Object))函数来过滤数字的用法。可以从以下链接下载示例文件。
...
如果一列包含文本，且需要选择包含特定文本的单元格，可以使用[**filter()**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#filter(int,%20java.lang.String))函数。在以下示例中，模板文件包含一组国家名称，要选择包含特定国家名称的行。以下代码演示了使用以下示例文件过滤文本的方法。
...
如果一列包含文本，使得很少的单元格为空白，并且需要选取仅包含空白单元格的行，则可以使用以下演示的[**matchBlanks()**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#matchBlanks(int))函数进行筛选。示例文件可从以下链接下载。
...
当需要筛选含有任何文本的单元格时，请使用[**MatchNonBlanks**](https://reference.aspose.com/cells/java/com.aspose.cells/autofilter#matchNonBlanks(int))筛选函数，如下所示。示例文件可从以下链接下载。
