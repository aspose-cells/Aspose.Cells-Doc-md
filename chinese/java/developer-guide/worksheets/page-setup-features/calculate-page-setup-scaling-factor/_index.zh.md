...
当您使用**按n页宽x m页高等比例缩放**选项设置页面设置缩放时，Microsoft Excel会计算页面设置缩放因子。您可以使用[SheetRender.getPageScale()](https://reference.aspose.com/cells/java/com.aspose.cells/sheetrender#PageScale)属性计算相同的值。此属性返回一个可以转换为百分比值的双精度值。例如，如果返回值为0.5079621076，则意味着缩放因子为51%。
