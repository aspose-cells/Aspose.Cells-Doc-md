---
title: 主轴和次轴
description: 学习如何在 Aspose.Cells for Java 中理解和处理主轴和次轴。我们的指南将帮助您了解主轴和次轴之间的区别，以及如何有效地配置和使用它们在您的图表中。
keywords: Aspose.Cells for Java，主轴，次轴，理解，区别，配置，使用。
type: docs
weight: 190
url: /zh/java/primary-and-second-axis/
---

## **可能的使用场景**
当图表中的数据系列之间的数值差异很大，或者当您有多种类型的数据（价格和成交量）时，请在次垂直（值）轴上绘制一个或多个数据系列。次垂直轴的刻度显示了相关数据系列的值。在显示柱形图和折线图的组合图中，次要轴效果很好。
## **处理主轴和第二轴，就像处理Microsoft Excel一样**
请参阅以下样本代码，创建一个新的Excel文件并在第一个工作表中放置图表的值。 
然后我们添加一个图表并显示第二轴。

![todo:image_alt_text](excel.png)

以下样本代码生成了 [输出Excel文件](PrimaryandSecondaryAxis.xlsx)。

## **示例代码**
{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "primary-and-second-axis.java" >}}
{{< app/cells/assistant language="java" >}}
