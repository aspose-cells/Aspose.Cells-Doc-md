...
description: 使用 Java 代码和 Aspose.Cells for Java API 将多个工作表合并为单个工作表。
...
有时，您需要将多个工作表合并为一个工作表。使用 Aspose.Cells API 可以轻松实现此目的。本文将向您展示一个代码示例，该示例读取源工作簿并将所有源工作表的数据组合到目标工作簿内的单个工作表中。
...
您可以使用任何源工作簿。对于此示例，我们使用包含三个工作表的源工作簿。
...
运行以下代码将提供一个工作簿，其中包含一个工作表，其中包含所有三个工作表的数据。
...
您可能会发现[将多个工作簿合并为一个工作簿](/cells/zh/java/combine-multiple-workbooks-into-a-single-workbook/)文章有助于了解更多信息。
