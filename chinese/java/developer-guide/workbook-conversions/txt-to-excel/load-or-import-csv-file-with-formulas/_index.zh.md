...
CSV 文件主要包含文本数据，不包含任何公式。然而，有时 CSV 文件也包含公式。此类 CSV 文件应通过设置[TxtLoadOptions.HasFormula](https://reference.aspose.com/cells/java/com.aspose.cells/txtloadoptions#HasFormula)至**真的** .一旦这个属性将被设置为**真的**Aspose.Cells 不会将公式视为简单文本。它们将被视为公式，Aspose.Cells 公式计算引擎将照常处理它们。

{{% /alert %}} 
## **使用公式加载或导入 CSV 文件**
以下代码说明了如何加载和导入包含公式的 CSV 文件。您可以使用任何 CSV 文件。为了说明的目的，我们使用[简单的csv文件](5472505.csv)其中包含此数据。如您所见，它包含一个公式。
