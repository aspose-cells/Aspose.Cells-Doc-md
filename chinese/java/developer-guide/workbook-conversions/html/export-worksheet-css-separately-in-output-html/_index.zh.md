...
Aspose.Cells 提供了在将 Excel 文件转换为 HTML 时单独导出工作表 CSS 的功能。为此，请使用 HtmlSaveOptions.ExportWorksheetCSSSeparately 属性，并在将 Excel 文件保存为 HTML 格式时将其设置为 true。
...
当使用 Aspose.Cells 将具有多个工作表的工作簿转换为 HTML 时，它会创建一个 HTML 文件以及一个包含 CSS 和多个 HTML 文件的文件夹。在浏览器中打开此 HTML 文件时，选项卡可见。具有单个工作表的工作簿在转换为 HTML 时需要相同的行为。之前没有为单页工作簿创建单独的文件夹，只创建了 HTML 文件。这样的 HTML 文件在浏览器中打开时不显示标签。 Excel 也为单个工作表创建了适当的文件夹和 HTML，因此使用 Aspose.Cells 实现了相同的行为。可以从以下链接下载示例文件，以便在下面的示例代码中使用：
