---
title: 将 Excel 转换为带有标题的 HTML
type: docs
weight: 10
url: /zh/java/convert-excel-to-html-with-headings/
---
## **将 Excel 转换为带有标题的 HTML**

Aspose.Cells 提供了在将 Excel 转换为 HTML 时导出标题的选项。为此，您可以设置[**HtmlSaveOptions.ExportHeadings**](https://reference.aspose.com/cells/java/com.aspose.cells/htmlsaveoptions#ExportHeadings)财产给**真的**.默认值[**HtmlSaveOptions.ExportHeadings**](https://reference.aspose.com/cells/java/com.aspose.cells/htmlsaveoptions#ExportHeadings)财产是**错误的**.下图显示了以下代码生成的输出文件。

![待办事项：图像_替代_文本](convert-excel-to-html-with-headings_1.jpg)

## **示例代码**

{{< gist "aspose-cells-gists" "5876dc77e47649b66bdb5deefb4b5639" "Examples-src-AsposeCellsExamples-HTML-PrintHeadings-1.java" >}}
