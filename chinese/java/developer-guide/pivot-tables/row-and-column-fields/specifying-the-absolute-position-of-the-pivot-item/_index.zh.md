...
- 添加了 [**PivotItem.move(int count, boolean isSameParent)**](https://reference.aspose.com/cells/java/com.aspose.cells/pivotitem#move-int-boolean-) 方法以根据计数值向上或向下移动项，其中计数是要向上或向下移动PivotItem的位置数。如果计数值小于零，项将向上移动，而如果计数值大于零，PivotItem将向下移动，布尔类型的 isSameParent 参数指定是否在同一父节点中执行移动操作或不执行移动操作。
- 废弃了 *PivotItem.move(int count)* 方法，因此建议使用新添加的 [**PivotItem.move(int count, boolean isSameParent)**](https://reference.aspose.com/cells/java/com.aspose.cells/pivotitem#move-int-boolean-) 方法。

请注意，在使用 [**PivotItem.setPosition()**](https://reference.aspose.com/cells/java/com.aspose.cells/pivotitem#Position)、[**PivotItem.setPositionInSameParentNode()**](https://reference.aspose.com/cells/java/com.aspose.cells/pivotitem#PositionInSameParentNode) 属性和 [**PivotItem.move(int count, boolean isSameParent)**](https://reference.aspose.com/cells/java/com.aspose.cells/pivotitem#move-int-boolean-) 方法之前，需要调用 [**PivotTable.refreshData**](https://reference.aspose.com/cells/java/com.aspose.cells/pivottable#refreshData--) 和 [**PivotTable.calculateData**](https://reference.aspose.com/cells/java/com.aspose.cells/pivottable#calculateData--) 方法。
