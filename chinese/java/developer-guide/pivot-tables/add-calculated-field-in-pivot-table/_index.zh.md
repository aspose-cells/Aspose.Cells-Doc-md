...
4. 在“名称”字段中输入计算字段的名称。
5. 在“Formula”字段中，输入要使用适当数据透视表字段名称和数学运算符执行的公式。 
<br>
...
## **在数据透视表中添加计算字段**
请参阅以下示例代码。代码首先设置原始数据并创建一个数据透视表，然后根据数据透视表中的现有PivotField创建计算字段，并将计算字段添加到数据区域。最后，将工作簿保存为[output XLSX](out.xlsx)格式。执行示例代码后，工作表中将添加一个带有计算字段的数据透视表。
