...
您可以使用 Microsoft Selfcert.exe 或任何其他工具创建个人数字签名，也可以购买数字签名。要签署电子表格，请在创建数字签名后将签名附加到您的工作簿。
 ...
 Aspose.Cells 用于 Reporting Services 支持数字签名。
...
- 当 DigitalSignature 设置为关闭时，Reporting Services 的 Aspose.Cells 将关闭数字签名功能。
例如：
...
- pfxFilename——指向 PFX 文件。文件名必须是完整的文件名。它不能设置为空白值。
- pfxPwd – 设置密码。它不能留空。
- 目的——解释签名的目的。它可以是空白的。
例如：
