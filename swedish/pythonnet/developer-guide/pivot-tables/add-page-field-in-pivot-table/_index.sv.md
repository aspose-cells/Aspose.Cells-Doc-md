...
description: Lär dig hur du lägger till och konfigurerar sidfält i pivottabeller med Aspose.Cells for Python via .NET, inklusive att lägga till sidfält, enkelvalsfiltrering och flervalsfiltrering.
keywords: Aspose.Cells, Python via .NET, pivottabell, sidfält, PivotFieldType.Page, PageFields, IsMultipleItemSelectionAllowed, CurrentPageItem, PivotItem, IsHidden, filter
type: docs
weight: 250
...
Aspose.Cells for Python via .NET exponerar två likvärdiga sätt att registrera ett sidfält. API:et på hög nivå är `PivotTable.add_field_to_area(PivotFieldType.PAGE, "field_name")`, som tar källkolonnens namn och lägger till fältet i ett enda anrop. API:et på lägre nivå är `PivotTable.page_fields.add(PivotField)`, som används när du redan har en `PivotField`-referens och vill lägga till samma fältinstans i sidområdet. Båda API:erna slutar med att fylla samma `page_fields`-samling, och resten av denna artikel visar hur du väljer mellan dem och hur du styr varje filtreringsläge.

## **Lägga till ett sidfält**

Det finns två sätt att registrera ett pivotfält i sidområdet. Anropet på hög nivå tar källkolonnens namn som en sträng och är den vanligaste vägen. Anropet på lägre nivå accepterar en befintlig `PivotField`-instans och är bekvämt när samma fältobjekt måste återanvändas över flera pivotområden. Båda anropen placerar fältet i `PivotTable.page_fields`, varefter det visas som sidans rullgardinsmeny överst i den renderade pivoten.
...
När du redan arbetar med en `PivotField`-instans kan du skicka den direkt till `PivotTable.page_fields.add`. Pivottabellen och sidfältet konstrueras exakt som i föregående scenario; endast den slutliga registreringen i sidområdet ersätts med API-anropet på lägre nivå.
...
|---|---|---|---|
| Lägg till ett sidfält efter källkolonnens namn (vanligast) | `PivotTable.add_field_to_area(PivotFieldType.PAGE, "field_name")` | n/a | Hög nivå, en rad. Använd detta om du inte behöver en `PivotField`-referens. |
| Lägg till ett sidfält när du redan har ett `PivotField`-objekt | `PivotTable.page_fields.add(PivotField)` | n/a | Använd när fältobjektet erhölls någon annanstans eller behöver återanvändas. |
