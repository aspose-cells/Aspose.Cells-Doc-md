...
description: Lär dig hur du renderar arraydata till en enskild cell med attributen ArrayAsSingle och ExtraDelimiter i Smart Markers med Aspose.Cells for Java.
keywords: Aspose.Cells, Java-bibliotek, kalkylblad, Smart Markers, ArrayAsSingle, ExtraDelimiter, enskild cell-array, arrayrendering, mall
type: docs
weight: 195
...
## **Introduction**
Smart Markers i Aspose.Cells är en kraftfull, mallbaserad funktion som låter dig dynamiskt fylla kalkylbladsdata med markörruttryck som `&=DataSource.Field`. Markören placeras i en designarbetsbok, och när mallen bearbetas av `WorkbookDesigner` ersätts markörerna med värden från den angivna datakällan.
Som standard, när en Smart Marker refererar till en arrayegenskap (till exempel `&=DataSource.Numbers`), expanderar motorn arrayen och placerar varje element i en separat intilliggande cell – antingen horisontellt över en rad eller vertikalt nedför en kolumn. Även om detta beteende är bekvämt i många scenarier, finns det situationer där du föredrar att rendera hela arrayen i en enda cell, med elementen sammanfogade och separerade med en valfri avgränsare.
...
När en Smart Marker refererar till en arrayegenskap expanderar Aspose.Cells arrayen över flera celler som standard. Till exempel kommer en markör som `&=Product.Tags` mot en `string[]` som innehåller fyra värden att placera varje värde i sin egen cell, vilket skjuter ut annan mallinnehåll och potentiellt förstör noggrant utformade rapportlayouter.
...
- **Nedströms pipelines** (CSV-export, PDF-rendering, sammanfogning) som förväntar sig ett enda konsoliderat värde per cell snarare än ett expanderat intervall.
- **Plattformsoberoende kompatibilitet**, där vissa konsumenter inte tolererar arrayer som sprider sig över flera celler.

### **The Gap It Fills**
Utan en inbyggd mekanism skulle utvecklare tvingas förbehandla data i Java – sammanfoga arrayer till avgränsade strängar innan de binds till arbetsboksdesignern. Detta duplicerar logik, komplicerar datamodeller och ökar risken för fel. Attributen `ArrayAsSingle` och `ExtraDelimiter` eliminerar denna lösning genom att hantera formateringen deklarativt inuti Smart Markern själv.
...
- **Mångsidiga datatyper**: Fungerar med strängar, nummer, datum och andra datatyper som kan sammanfogas med en avgränsare.
...
- Värdet `extraDelimiter` behandlas som en strängliteral; escape-eventuella specialtecken som din mallprocessor kan tolka.
...
- Placera Smart Markern i en cell som har tillräcklig bredd för att visa den resulterande sammanfogade strängen; annars kan innehållet visuellt svämma över till intilliggande celler beroende på formatet.
{{% /alert %}}
