---
title: Lägg till pivottabellens rad- och kolumnfält i Aspose.Cells for .NET
description: Lär dig hur du lägger till rad- och kolumnpivotfält i en pivottabell och styr pivotfältets delsummor med hjälp av PivotField.SetSubtotals med PivotFieldSubtotalType i Aspose.Cells for .NET.
linktitle: Rad- och kolumnfält
keywords: Aspose.Cells, .NET, pivottabell, radfält, kolumnfält, PivotField, SetSubtotals, PivotFieldSubtotalType, delsummor, C#, Excel-pivottabell
type: docs
weight: 220
...
Ordningen på fältnästningen spelar roll. Om du lägger till `Category` i radområdet först och sedan `Item` skapas en pivot där den yttre grupperingen är `Category` och den inre grupperingen är `Item`. Om du vänder på ordningen vänds hierarkin.

## **Pivotfältets delsummor**
Metoden `PivotField.SetSubtotals(PivotFieldSubtotalType subtotalType, bool shown)` styr vilka delsummarader som visas för ett pivotfält. Varje anrop växlar en enskild delsummatyp oberoende. Genom att skicka `shown = true` visas delsummen, medan `shown = false` döljer den. Eftersom varje anrop endast påverkar en typ, bygger du en anpassad delmängd av delsummor genom att anropa metoden flera gånger med olika `subtotalType`-värden.
...
{{% alert color="primary" %}}
Delsummor renderas endast när det finns två eller fler pivotfält i radområdet (eller i kolumnområdet). Ett enskilt fält har inget meningsfullt att göra delsummor mellan, så anrop till `SetSubtotals` har ingen synlig effekt i det fallet. Den här artikeln placerar därför två radfält (`Category` yttre, `Item` inre) i varje exempel så att delsummogränsen mellan varje `Category`-grupp syns.
{{% /alert %}}
...
Du är inte begränsad till en enda delsummatyp. Varje `SetSubtotals`-anrop fungerar oberoende på en typ, så att anropa metoden två gånger — en gång med `Sum` och en gång med `Average` — skapar en anpassad delmängd av två delsummarader för varje `Category`-grupp.
