---
title: Lägg till filterfält i en pivottabell i Aspose.Cells for .NET
linktitle: Lägg till filterfält i en pivottabell i Aspose.Cells for .NET
description: Lär dig hur du lägger till och konfigurerar filterfält i pivottabeller med Aspose.Cells for .NET, inklusive att lägga till filterfält, enkelvalfiltrering och flervalsfiltrering.
keywords: Aspose.Cells, .NET, pivottabell, filterfält, PivotFieldType.Page, PageFields, IsMultipleItemSelectionAllowed, CurrentPageItem, PivotItem, IsHidden, filter
type: docs
weight: 250
...
{{% alert color="primary" %}}
Aspose.Cells stöder hela livscykeln för filterfält i pivottabeller. Du kan lägga till ett filterfält via ett bekvämlighets-API på hög nivå eller via den underliggande `PageFields`-samlingen, och du kan styra filtret i enkelvalsläge, rensa det så att alla filterobjekt visas, eller växla fältet till flerval så att användare kan välja flera filterobjekt samtidigt via kryssrute-UI:t i Excel.
{{% /alert %}}

## **Introduktion**
Ett filterfält är ett pivotfält som styr *vilken delmängd* av källdatan pivottabellens kropp visar. Slutanvändare ser det som en rullgardinsmeny överst i en renderad pivottabell i Excel, och att välja ett av de tillgängliga filterobjekten bygger om pivottabellens kropp så att endast posterna som hör till det filterobjektet summeras. Ett pivotfält blir ett filterfält när det registreras som `PivotFieldType.Page` snarare än `PivotFieldType.Row`, `PivotFieldType.Column` eller `PivotFieldType.Data`.

## **Lägga till ett filterfält**

### Lägga till ett filterfält med AddFieldToArea
Följande exempel bygger en liten Fruit / Year / Amount-datamängd, placerar en pivottabell vid cell E3 med `Fruit` på radområdet, `Amount` på dataarea och `Year` på filterområdet, uppdaterar pivottabellen och sparar arbetsboken.
...
// Fylla 9 rader med exempeldata: Frukt, År, Belopp
...
När du redan arbetar med en `PivotField`-instans kan du skicka den direkt till `PivotTable.PageFields.Add`. Pivottabellen och filterfältet konstrueras exakt som i föregående scenario; endast den slutliga registreringen i filterområdet ersätts med det underliggande API-anropet.
...
//   underliggande alternativet till AddFieldToArea. Resultatet är
...
// Fruit -> Rad, Amount -> Data (Year kommer att läggas till Sida nedan)
pivotTable.AddFieldToArea(PivotFieldType.Row, "Fruit");
pivotTable.AddFieldToArea(PivotFieldType.Data, "Amount");
// Underliggande metod: hämta den befintliga Year-PivotField från BaseFields
...
I standardbeteendet för enkelval renderas filterfältet som en enda rullgardinsmeny och heltalet `PivotField.CurrentPageItem` väljer vilket filterobjekt som styr pivottabellens kropp. Att tilldela ett specifikt index väljer det objektet; att tilldela det speciella sentinelvärdet `0x7FFD` (decimalt 32765) rensar filtret så att alla filterobjekt summeras samtidigt. Enkelval är standardläget; du behöver inte aktivera det uttryckligen.

### Visa alla objekt
Att sätta `CurrentPageItem` till det magiska värdet `0x7FFD` är likvärdigt med att rensa filtret: pivottabellens kropp summerar alla filterobjekt som om inget filter vore tillämpat.
...
class Program
{
    static void Main()
    {
        ...
        pivotTable.AddFieldToArea(PivotFieldType.Page, "Year");
        pivotTable.CalculateData();
        // Rensa sidfiltret så att alla objekt i sidfältet är synliga.
        // 0x7FFD (decimal 32765) är det speciella sentinelvärdet som betyder "alla objekt" —
        // motsvarande att välja "(Alla)" i Excel:s sidfält-rullgardin.
...
Att sätta `CurrentPageItem` till ett verkligt index väljer bara det filterobjektet. Indexet är positionen för objektet i filterfältets sorterade objektlista, så till exempel `1` väljer det andra objektet efter sortering.
...
## **Flervalsfiltrering**
Flervalsfiltrering förvandlar filterrullgardinsmenyn till en kryssrutelista och låter slutanvändaren välja flera filterobjekt samtidigt. Aspose.Cells exponerar två egenskaper som fungerar tillsammans. `PivotField.IsMultipleItemSelectionAllowed` måste sättas till `true` innan flervals-UI:t börjar gälla. När det är aktiverat styr `PivotItem.IsHidden` vilka objekt som visas i kryssrutelistan, så du kan antingen visa alla objekt eller vitlista endast specifika objekt.
...
//   Nedan tillämpar vi multiselect-filtrering på sidfältet.
Workbook workbook = new Workbook();
Worksheet sheet = workbook.Worksheets[0];
...
pivotTable.AddFieldToArea(PivotFieldType.Data, "Amount");
pivotTable.AddFieldToArea(PivotFieldType.Page, "Year");
// — Aktivera multiselect på sidfältet
...
|---|---|---|---|
| Lägg till ett filterfält efter källkolumnnamn (vanligast) | `PivotTable.AddFieldToArea(PivotFieldType.Page, "fieldName")` | n/a | Hög nivå, en rad. Använd detta om du inte behöver en `PivotField`-referens. |
...
| Aktivera flervals-UI:t i Excel | `PivotField.IsMultipleItemSelectionAllowed` | sätt till `true` | Krävs innan några `IsHidden`-anrop börjar gälla. |
...
{{% alert color="primary" %}}
Kom alltid ihåg synlighetsbegränsningen när du konfigurerar flervalsfiltrering. Om alla `PivotItem` i ett filterfält med flerval är dolda kraschar Excel vid öppning eller renderar en tom pivottabell. Bygg din vitlista mot din källdata så att minst ett objekt förblir synligt, och dina sparade arbetsböcker kommer att öppnas tillförlitligt på alla maskiner.
{{% /alert %}}
