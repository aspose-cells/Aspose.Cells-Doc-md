---
title: SmartMarker-rendering av array i en cell | Aspose.Cells for Node.js via Java
description: Lär dig hur du renderar arraydata i en enskild cell med attributen ArrayAsSingle och ExtraDelimiter i Smart Markers med Aspose.Cells for Node.js via Java.
keywords: Aspose.Cells, Node.js via Java-bibliotek, kalkylblad, Smart Markers, ArrayAsSingle, ExtraDelimiter, enskild cell-array, arrayrendering, mall
type: docs
weight: 195
url: /sv/nodejs-java/smartmarker-array-single-cell-rendering-arrayassingle-extradelimiter/
---
...
- **Tagg-, etikett- eller nyckelordslistor** som behöver visas som kommaseparerade eller rörseparerade värden inom en enskild cell.
- **Filterchippar eller statusindikatorer** som grupperar flera värden på en plats för läsbarhet.
...
- **Plattformsoberoende kompatibilitet**, där vissa konsumenter inte kan tolerera arrayer som sprider sig över flera celler.

### **Luckan den fyller**

Utan en inbyggd mekanism skulle utvecklare tvingas förbehandla data i JavaScript — sammanfoga arrayer till avgränsade strängar innan de binds till arbetsboksdesignern. Detta duplicerar logik, komplicerar datamodeller och ökar risken för fel. Attributen `ArrayAsSingle` och `ExtraDelimiter` eliminerar denna lösning genom att hantera formateringen deklarativt inom själva Smart Markern.
...
- **Anpassad avgränsarkontroll**: Ange valfri separatorsträng du vill — komma, semikolon, bindestreck, rör, nyrad eller valfri anpassad text.
- **Mallsdriven formatering**: Ingen ytterligare kod krävs för att förbehandla datan; formateringsregler finns inom Smart Marker-taggen.
...
- `arrayasSingle=true` — instruerar motorn att rendera hela arrayen i en enskild cell. Endast värdet `true` utlöser encellsbeteendet.
...
2. **Skapa en designrarbetsbok**: Skapa en ny `Workbook`, lägg till en rubrikrad och placera en Smart Marker-cell som refererar till array-egenskapen med attributen `arrayasSingle` och `extraDelimiter`.
3. **Instansiera WorkbookDesigner**: Skapa ett `WorkbookDesigner`-objekt, koppla designrarbetsboken till det och bind din datakälla med metoden `setDataSource`.
...
### **Kodexempel 3 — Jämföra standard- kontra ArrayAsSingle-beteende**
...
function main() {
    ...
    // Avsnitt 2: Ny rendering i en cell med hjälp av arrayasSingle och extraDelimiter
...
- Placera Smart Markern i en cell som har tillräcklig bredd för att visa den resulterande sammanfogade strängen; annars kan innehållet visuellt flöda över till intilliggande celler beroende på formatet.
